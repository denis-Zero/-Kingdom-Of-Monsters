...
function toggleMobileMode() {
    ...
    function setupBattleUI() {
        ...
        state.activePlayerCreatures.forEach((p, pIndex) => {
            ...
            p.attacks.forEach(atk => {
                ...
                // Adiciona info de cost se houver
                let costInfo = "";
            ...
            // Adiciona botão para Support Skill se a criatura tiver uma
            if (p.supportSkill) {
                const supportBtn = document.createElement('button');
                const skill = p.supportSkill;
                const cd = p.cooldowns[skill.name] || 0;
                const hasActed = state.isBossBattle && p.battleState && p.battleState.hasActed;
                const isParalyzed = p.trapParalysisTurns > 0;

                supportBtn.style.padding = "5px 10px";
                supportBtn.style.fontSize = "0.7rem";
                supportBtn.style.minWidth = "auto";
                supportBtn.style.backgroundColor = "#2a5298";
                supportBtn.style.borderColor = "#4a7bff";

                if (cd > 0 || hasActed || isParalyzed) {
                    supportBtn.innerText = cd > 0 ? `${skill.name} (${cd})` : skill.name;
                    supportBtn.disabled = true;
                    supportBtn.style.opacity = '0.5';
                    supportBtn.style.cursor = 'not-allowed';
                } else {
                    supportBtn.innerText = skill.name;
                }

                supportBtn.title = `${skill.name}\n${skill.desc}\nRecarga: ${skill.cooldown} turnos`;

                supportBtn.onclick = () => {
                    activateOwnSupportSkill(p, pIndex);
                };

                btnGroup.appendChild(supportBtn);
            };
        ...
        if (state.challengeNoReserve) {
            return;
        }
        
        const activeIds = state.activePlayerCreatures.map(c => c.id);
        // Inclui tanto reservas quanto criaturas ativas (exceto a criatura ativa atual em modo não-boss)
        const supportCreatures = state.team.filter(c => c.currentHp > 0);
        supportCreatures.forEach(creature => {
            ...
            if (skills.length > 0) {
                skills.forEach((skill, index) => {
                    ...
                    if (cd > 0) {
                    btn.innerText = `${creature.emoji} ${skill.name} (${cd})`;
                    btn.disabled = true;
                } else {
                    btn.innerText = `${creature.emoji} ${skill.name}`;
                    btn.onclick = () => activateSupportSkill(creature, index);
                }
    ...
    function activateOwnSupportSkill(creature, playerIndex) {
    ...
    function handleChaosEvent() {
        ...
        showModal("Evento Caótico", "Dois efeitos acontecerão aleatoriamente.", [{ text: "Ok", action: () => { picks.forEach(fn => { if (fn()) startedBattle = true; }); if (!startedBattle) endEvent(); } }]);
    }
    ...
    function showAncientAltar() {
        ...
        showModal("Altar Antigo", "Escolha um sacrifício.", [
            ...
            { text: "Sacrificar criatura", action: () => { showCreatureSelection((s) => { state.team.splice(state.team.indexOf(s), 1); if (Math.random() < 0.5) { const item = randomShopItem(); item.type = 'rare'; addItemToInventory(item); addLog("Recebeu item raro."); } else { const c = getRandomEnemy(); captureCreature(c); addLog("Recebeu ovo."); endEvent(); } }, "Quem será sacrificado?"); } },
            { text: "Ignorar", action: endEvent }
        ]);
    }
    ...
    function addLog(m) {
        ...
        if (!l) return; // Segurança caso o elemento não exista
        const d = document.createElement('div'); 
        d.innerHTML = m; 
