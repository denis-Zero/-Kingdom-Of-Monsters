...
{ id: 'mago', clan: 'primordiais', name: "Mago", emoji: "🧙‍♀️", image: "mago.png", hp: 90,
  ...
  supportSkill: { name: "Encantamento Ígneo", desc: "Adiciona +15 de dano fixo ao próximo ataque do aliado.", effect: "fixed_dmg_buff", value: 15, cooldown: 3 },
...
{ id: 'nautilus_ancestral', clan: 'abissais', name: "Nautilus Ancestral", emoji: "🐚", image: "nautilus_ancestral.png", hp: 195,
  ...
  evo: { level: 10, name: "Nautilus Colossal", emoji: "🌀", image: "nautilus_colossal.png", hpBoost: 95, dmgBoost: 45 }
    ...
    const BOSS = { name: "REI DEMÔNIO", clan: 'primordiais', emoji: "👺", image: "boss.png", hp: 1200, attacks: [{name: "Apocalipse", min: 0, max: 500, cooldown: 0, ignoreClans: true}, {name: "Garras do Caos", min: 120, max: 180}, {name: "Chama Negra", min: 200, max: 360}] }; 
    ...
    const ITEMS = {
        ...
        'cajado_runico': { id: 'cajado_runico', name: "Cajado Rúnico", desc: "+50% de dano para evoluções finais.", cost: 200, type: 'offense', icon: "🪄", image: "cajado_runico.png" },
        // Defensiva
    ...
    function initCreature(base) {
        const c = {
            ...
            currentHp: base.hp,
            isEvolved: false,
            items: base.items ? JSON.parse(JSON.stringify(base.items)) : [], // Mantém itens se a base já tiver
            cooldowns: {} // Adicionado para rastrear cooldowns de ataques
        };
    ...
    function setupBattleUI() {
        ...
        state.activePlayerCreatures.forEach((p, index) => {
            ...
            playerFighter.innerHTML = `
                <div class="creature-card tooltip" style="width: 120px; height: 180px; margin: 0 auto; cursor: default; ${hasActed ? 'border-color: #555;' : ''}">
    ...
    function openTreasure() {
        ...
        if (r < 0.3) {
            ...
            // Chance de encontrar item aleatório no baú (20% do r >= 0.8)
            if (Math.random() < 0.3) {
                const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
