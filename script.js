...

{
  ...
  evo: {
    ...
    let state = {
        ...
        // Itens guardados (não equipados)
        inventory: [],
        battleLoot: [], // Fila de loot da batalha atual
        isBossBattle: false,
        swapsUsed: 0,
        selectedIniciais: [],
        diamantes: 10,
    ...
    function normalizeState() {
        ...
        if (!state.options) {
            state.options = { fontSize: 16, isFullScreen: false, muted: false, volume: 0.5 };
              state.options = { fontSize: 16, isFullScreen: false, muted: false, volume: 0.5, mobileMode: false };         
        }
        // Garante estruturas nas criaturas do time
...
function toggleMobileMode() {
    ...
    function handleTrap() {
        // Efeito Visual de Tela Tremendo
        const body = document.body;
        body.classList.add('anim-trap-shake');
        playSound('defeat'); // Som de impacto/erro

        setTimeout(() => {
            body.classList.remove('anim-trap-shake');

            // 50% de chance para cada tipo de armadilha
            if (Math.random() < 0.2) {
                // --- TIPO 1: Escolher quem perder (Sacrifício) ---
                ...
                state.team.forEach(c => {
                    ...
                    d.onclick = () => {
                        ...
                        // Se morrer, solta os itens no inventário
                        ...
                        // Verifica Game Over se ficou sem time
                        if (state.team.length === 0) {
                            gameOver();
                        } else {
                            endEvent();
                ...
                // --- TIPO 2: Drenagem de Nível e Devolução ---
                ...
                state.team.forEach(c => {
                    ...
                    if (levelsLost > 0) {
                        ...
                        // Reduz status proporcionalmente (20 HP e 2 Dano por nível, valores base do levelUp)
                        ...
                        // Lógica de Devolução (Devolution)
                        if (c.isEvolved && c.evo) {
                            // Encontra a base original no CREATURE_POOL pelo ID
                            const baseCreature = CREATURE_POOL.find(poolItem => poolItem.id === c.id);
                            
                            if (baseCreature) {
                                // Remove os bônus da evolução que foram somados
        ...
        }, 600); // Espera um pouco a tela tremer antes de mostrar o modal
    }
