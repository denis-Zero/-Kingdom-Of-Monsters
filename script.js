...
{ id: 'dragao', clan: 'primordiais', name: "Dragão", emoji: "🦖", image: "dragao.png", hp: 125,
  ...
  supportSkill: { name: "Golpe Duplo", desc: "Permite que o aliado ataque duas vezes no próximo turno.", effect: "multi_hit_buff", turns: 1, cooldown: 5 },
...
{ id: 'golem', clan: 'primordiais', name: "Golem", emoji: "🗿", image: "golem.png", hp: 155,
  ...
  supportSkill: { name: "Barreira", desc: "Cria um escudo que absorve 18 de dano por 2 turnos.", effect: "shield_flat", value: 18, turns: 2, cooldown: 3 },
...
{ id: 'esquilo', clan: 'selvagens', name: "Esquilo", emoji: "🐿️", image: "esquilo.png", hp: 70,
  ...
  supportSkill: { name: "Acelerar", desc: "Reduz o cooldown das habilidades do aliado em 1 turno.", effect: "cooldown_reduce_one", value: 1, cooldown: 5 },
...
{ id: 'zumbi', clan: 'sombras', name: "Zumbi", emoji: "🧟", image: "zumbi.png", hp: 90,
  ...
  supportSkill: { name: "Carne Imortal", desc: "Recupera 5% de HP por turno durante 3 turnos.", effect: "regen_turns", value: 0.05, turns: 3, cooldown: 5 },
...
function toggleMobileMode() {
    ...
    function startPvP(playerCount, enemyCount) {
        ...
        if (state.team.length === 0) {
            ...
            state.team.forEach(c => c.currentHp = c.hp);
        }
        const enemies = [];
    ...
    function generateLevel() {
        ...
        for (let i = 0; i < cardCount; i++) {
            ...
            // 40% Combate, 20% Encontro, 20% Tesouro, 20% Armadilha
            let type = 'COMBAT';
            if (rand < 0.40) type = 'COMBAT';
            else if (rand < 0.60) type = 'ENCOUNTER';
            else if (rand < 0.80) type = 'TREASURE';
            else type = 'TRAP'; // Nova carta
    ...
    function startBattle(enemyOrEnemies, isBoss = false) {
        ...
        state.battleLoot = []; // Reseta o loot no início
    ...
    function activateSupportSkill(creature) {
        ...
        targets.forEach((active, tIdx) => {
            ...
            switch (skill.effect) {
                ...
                case 'cleanse_debuffs':
                    if (active.trapPoisonTurns) active.trapPoisonTurns = 0;
                    if (active.trapParalysisTurns) active.trapParalysisTurns = 0;
                    // Reset debuffs in battleState
                    active.battleState.missChance = 0; // Se houver
                    active.battleState.defenseBuff = Math.max(0, active.battleState.defenseBuff); // Remove negative defense if any
                    addLog(`✨ PURIFICAR: ${active.name} foi purificado de todos os efeitos negativos!`);
                    playSound('heal');
                    break;
        ...
        switch (skill.effect) {
            ...
            case 'revive_weak':
                // Procura um aliado morto
                const deadAlly = state.activePlayerCreatures.find(c => c.currentHp <= 0);
                if (deadAlly) {
    ...
    function activateOwnSupportSkill(creature, playerIndex) {
        ...
        targets.forEach((target, tIdx) => {
            ...
            switch (skill.effect) {
                ...
                case 'cleanse_debuffs':
                    if (target.trapPoisonTurns) target.trapPoisonTurns = 0;
                    if (target.trapParalysisTurns) target.trapParalysisTurns = 0;
                    target.battleState.missChance = 0;
                    target.battleState.defenseBuff = Math.max(0, target.battleState.defenseBuff);
                    addLog(`✨ PURIFICAR: ${target.name} foi purificado de todos os efeitos negativos!`);
                    playSound('heal');
                    break;
    ...
    function executePlayerAttack(atk, enemyIndex, playerIndex, skipTurnPass = false) {
        ...
        // Define enemyEl e enemyCard para animações
