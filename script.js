 // --- Data Definitions ---
    const CLANS = {
        primordiais: { 
            name: "Primordiais", emoji: "🔥", color: "#ff4d4d", bonus: "+20% Dano",
            strongAgainst: ['selvagens'], weakAgainst: ['abissais'],
            desc: "Poder bruto e magia antiga.",
            initialCreature: 'dragao'
        },
        selvagens: { 
            name: "Selvagens", emoji: "🌿", color: "#4CAF50", bonus: "+15% HP",
            strongAgainst: ['sombras'], weakAgainst: ['primordiais'],
            desc: "Instinto, resistência e adaptação.",
            initialCreature: 'lobo'
        },
        sagrados: { 
            name: "Sagrados", emoji: "✨", color: "#ffd700", bonus: "+10% HP/Turno",
            strongAgainst: ['sombras'], weakAgainst: ['primordiais'],
            desc: "Luz, cura e pureza.",
            initialCreature: 'unicornio'
        },
        sombras: { 
            name: "Sombras", emoji: "🌙", color: "#9932cc", bonus: "+20% Esquiva",
            strongAgainst: ['abissais'], weakAgainst: ['sagrados'],
            desc: "Noite, caos e corrupção.",
            initialCreature: 'morcego'
        },
        abissais: { 
            name: "Abissais", emoji: "🌊", color: "#00bfff", bonus: "+25% Refletir",
            strongAgainst: ['primordiais'], weakAgainst: ['selvagens'],
            desc: "Domínio, controle e profundezas.",
            initialCreature: 'kraken'
        },
        primordiais: { 
            name: "Primordiais", emoji: "🔥", color: "#ff4d4d", bonus: "+20% Dano",
            strongAgainst: ['selvagens'], weakAgainst: ['abissais'],
            desc: "Poder bruto e magia antiga.",
            initialCreature: 'O Primeiro Guardião'
        },
    };

    const CREATURE_POOL = [
        
    // 🔥 PRIMORDIAIS
{ id: 'dragao', clan: 'primordiais', name: "Dragão", emoji: "🦖", image: "dragao.png", hp: 125,
  attacks: [
    { name: "Sopro de Fogo", min: 22, max: 32 },
    { name: "Mordida Flamejante", min: 34, max: 44, cooldown: 2 }
  ],
  supportSkills: [
    { name: "Golpe Duplo", desc: "Permite que o aliado ataque duas vezes no próximo turno.", effect: "multi_hit_buff", turns: 1, cooldown: 5 }
  ],
  evo: { level: 7, name: "Rei Dragão", emoji: "🐉", image: "rei_dragao.png", hpBoost: 95, dmgBoost: 80 }
},

{ id: 'fenix', clan: 'primordiais', name: "Fênix", emoji: "🐦‍🔥", image: "fenix.png", hp: 95,
  attacks: [
    { name: "Brasas", min: 14, max: 22 },
    { name: "Supernova", min: 38, max: 52, recoil: 0.25, cooldown: 3 }
  ],
  supportSkills: [
    { name: "Cinzas Renascidas", desc: "Cura 25% do HP da criatura ativa instantaneamente.", effect: "heal_percent", value: 0.25, cooldown: 5 },
    { name: "Renascimento", desc: "Revive a criatura com 50% de HP se ela morrer no próximo turno.", effect: "auto_revive", value: 0.50, cooldown: 8 },
    { name: "Purificar", desc: "Remove todos os debuffs negativos do aliado.", effect: "cleanse_debuffs", value: 1, cooldown: 4 }
],
  evo: { level: 8, name: "Fênix Ancestral", emoji: "🦅", image: "fenix_ancestral.png", hpBoost: 85, dmgBoost: 75 }
},

{ id: 'golem', clan: 'primordiais', name: "Golem", emoji: "🗿", image: "golem.png", hp: 155,
  attacks: [
    { name: "Soco de Pedra", min: 26, max: 36 },
    { name: "Terremoto", min: 46, max: 72, enemyBoost: { dmg: 0.9}, cooldown: 3 }
  ],
  supportSkill: { name: "Barreira", desc: "Cria um escudo que absorve 35 de dano por 2 turnos.", effect: "shield_flat", value: 35, turns: 2, cooldown: 3 },
  evo: { level: 10, name: "Golem de Ferro", emoji: "🤖", image: "golem_ferro.png", hpBoost: 90, dmgBoost: 70 }
},

{ id: 'mago', clan: 'primordiais', name: "Mago", emoji: "🧙‍♀️", image: "mago.png", hp: 90,
  attacks: [
    { name: "Faisca", min: 16, max: 24 },
    { name: "Faisca Poderosa", min: 36, max: 50, cooldown: 2 }
  ],
  supportSkill: { name: "Encantamento Ígneo", desc: "Adiciona +15 de dano fixo ao próximo ataque do aliado.", effect: "fixed_dmg_buff", value: 15, cooldown: 2 },
  evo: { level: 5, name: "Rei Mago", emoji: "🧙‍♂️", image: "rei_mago.png", hpBoost: 70, dmgBoost: 75 }
},

{ id: 'tita', clan: 'primordiais', name: "Titã", emoji: "🗻", image: "tita.png", hp: 175,
  attacks: [
    { name: "Punho Colossal", min: 18, max: 26 },
    { name: "Impacto Titânico", min: 32, max: 44, cooldown: 3 }
  ],
  supportSkill: { name: "Pele de Titã", desc: "Reduz o dano recebido pelo aliado em 30% no próximo turno.", effect: "dmg_reduction_buff", value: 0.30, turns: 1, cooldown: 4 },
  evo: { level: 9, name: "Titã Primordial", emoji: "🗿", image: "tita_primordial.png", hpBoost: 85, dmgBoost: 60 }
},
{ id: 'colosso_lava', clan: 'primordiais', name: "Colosso de Lava", emoji: "🌋", image: "colosso_lava.png", hp: 135,
  attacks: [
    { name: "Punho Magmático", min: 18, max: 26 },
    { name: "Erupção Devastadora", min: 36, max: 50, cooldown: 3 }
  ],
  supportSkill: { name: "Enfraquecer", desc: "Reduz o dano causado pelo inimigo em 18% por 2 turnos.", effect: "enemy_damage_down", value: 0.18, turns: 2, cooldown: 4 },
  evo: {
    level: 15,
    name: "Titã Vulcânico",
    emoji: "🌋",
    image: "tita_vulcanico.png",
    hpBoost: 200,
    dmgBoost: 160
  }
},

{
  id: 'A Devoradora da Luz',
  clan:'primordiais',
  name: "A Devoradora da Luz",
  emoji: "🗡️",
  image: "A Devoradora da Luz.png",
  hp: 140,
  attacks: [
    { name: "Asas do Crepúsculo", min: 52, max: 80, cooldown: 2  },
    { name: "Ilusão do Bosque Eterno", min: 85, max: 120, cooldown: 5 }
  ],
  supportSkill: { name: "Encanto da Floresta Morta", desc: "Adiciona +80 de dano fixo ao próximo ataque do aliado.", effect: "fixed_dmg_buff", value: 80, cooldown: 4 },
  evo: {
    level: 10,
    name: "Eclipse Eterno",
    emoji: "🩸",
    image: "Eclipse Eterno.png",
    hpBoost: 50,
    dmgBoost: 90
  }
},




// 🌿 SELVAGENS
{ id: 'lobo', clan: 'selvagens', name: "Lobo", emoji: "🐺", image: "lobo.png", hp: 85,
  attacks: [
    { name: "Arranhão", min: 12, max: 18 },
    { name: "Uivo Mortal", min: 24, max: 38, recoil: 0.15 }
  ],
  supportSkill: { name: "Uivo de Guerra", desc: "Aumenta o dano do próximo ataque do aliado em 20 pontos.", effect: "fixed_dmg_buff", value: 20, cooldown: 3 },
  evo: { level: 5, name: "Lobisomem", emoji: "🐺", image: "lobisomem.png", hpBoost: 75, dmgBoost: 70 }
},

{ id: 'tartaruga', clan: 'selvagens', name: "Tartaruga", emoji: "🐢", image: "tartaruga.png", hp: 190,
  attacks: [
    { name: "Casco Pesado", min: 6, max: 14 },
    { name: "Jato d'água", min: 26, max: 36, cooldown: 3 }
  ],
  supportSkill: { name: "Proteção de Casco", desc: "Reduz o dano recebido pelo aliado ativo em 40% no próximo turno.", effect: "dmg_reduction_buff", value: 0.40, turns: 1, cooldown: 4 },
  evo: { level: 5, name: "Tartaruga Gigante", emoji: "🐢", image: "tartaruga_gigante.png", hpBoost: 95, dmgBoost: 50 }
},

{ id: 'esquilo', clan: 'selvagens', name: "Esquilo", emoji: "🐿️", image: "esquilo.png", hp: 70,
  attacks: [
    { name: "Mordida Rápida", min: 8, max: 12 },
    { name: "Noz Explosiva", min: 22, max: 32, cooldown: 2 }
  ],
  supportSkills: [
    { name: "Acelerar", desc: "Reduz o cooldown das habilidades do aliado em 1 turno e remove todos os debuffs negativos.", effect: "cooldown_reduce_one", value: 1, cooldown: 2 },
    { name: "Respiração da Floresta", desc: "Garante que o próximo ataque do aliado cause o dano máximo possível.", effect: "guaranteed_max_dmg", cooldown: 5 },
  ],   
  evo: { level: 5, name: "Esquilo Guerreiro", emoji: "🛡️", image: "esquilo_guerreiro.png", hpBoost: 90, dmgBoost: 65 }
},

{ id: 'urso', clan: 'selvagens', name: "Urso", emoji: "🐻", image: "urso.png", hp: 145,
  attacks: [
    { name: "Garra Pesada", min: 16, max: 24 },
    { name: "Fúria Selvagem", min: 30, max: 38, recoil: 0.2 }
  ],
  supportSkill: { name: "Foco Letal", desc: "Aumenta a chance de crítico em 25% no próximo turno.", effect: "crit_buff", value: 0.25, turns: 1, cooldown: 3 },
  evo: { level: 6, name: "Urso Ancestral", emoji: "🐻‍❄️", image: "urso_ancestral.png", hpBoost: 70, dmgBoost: 55 }
},

{ id: 'serpente', clan: 'selvagens', name: "Serpente", emoji: "🐍", image: "serpente.png", hp: 90,
  attacks: [
    { name: "Bote Rápido", min: 18, max: 28 },
    { name: "Veneno Paralizante", min: 24, max: 34, cooldown: 3 }
  ],
  supportSkill: { name: "Toxina Debilitante", desc: "Reduz o dano máximo do inimigo em 10 durante 2 turnos.", effect: "enemy_max_dmg_debuff", value: 10, turns: 2, cooldown: 5 },
  evo: { level: 6, name: "Serpente Alfa", emoji: "🐲", image: "serpente_alfa.png", hpBoost: 50, dmgBoost: 65 }
},

{
  id: 'cervo_ancestral',
  clan: 'selvagens',
  name: "Cervo Ancestral",
  emoji: "🦌",
  image: "cervo_ancestral.png",
  hp: 125,
  attacks: [
    { name: "Chifrada Selvagem", min: 17, max: 25 },
    { name: "Espírito da Floresta", min: 26, max: 38, cooldown: 2 }
  ],
  supportSkill: { name: "Regeneração", desc: "Recupera 6% de HP por turno durante 3 turnos.", effect: "regen_turns", value: 0.06, turns: 3, cooldown: 4 },
  evo: {
    level: 6,
    name: "Guardião do Bosque",
    emoji: "🌳",
    image: "guardiao_bosque.png",
    hpBoost: 105,
    dmgBoost: 70
  }
},

// ✨ SAGRADOS
{ id: 'unicornio', clan: 'sagrados', name: "Unicórnio", emoji: "🦄", image: "unicornio.png", hp: 95,
  attacks: [
    { name: "Raio Místico", min: 18, max: 24 },
    { name: "Benção Lunar", min: 28, max: 38, cooldown: 3 }
  ],
  supportSkill: { name: "Purificar", desc: "Remove todos os debuffs negativos do aliado.", effect: "cleanse_debuffs", value: 1, cooldown: 4 },
  evo: { level: 5, name: "pegasus", emoji: "🫎", image: "pegasus.png", hpBoost: 60, dmgBoost: 60 }
},

{ id: 'fada', clan: 'sagrados', name: "Fada", emoji: "🧚‍♀️", image: "fada.png", hp: 80,
  attacks: [
    { name: "Pó de Estrela", min: 14, max: 22 },
    { name: "Explosão Mágica", min: 28, max: 40, recoil: 0.1 }
  ],
  supportSkill: { name: "Pó de Estrela", desc: "Limpa qualquer efeito de recoil (recuo) e cura 15 HP.", effect: "clean_recoil_heal", value: 15, cooldown: 3 },
  evo: { level: 6, name: "Rainha Fada", emoji: "👸", image: "rainha_fada.png", hpBoost: 55, dmgBoost: 45 }
},

{ id: 'anjo', clan: 'sagrados', name: "Anjo", emoji: "😇", image: "anjo.png", hp: 105,
  attacks: [
    { name: "Lâmina Celestial", min: 18, max: 26 },
    { name: "Julgamento Divino", min: 34, max: 46, cooldown: 3, recoil: 0.2 }
  ],
  supportSkill: { name: "Reanimar", desc: "Ressuscita um aliado derrotado com 20% de HP.", effect: "revive_weak", value: 0.20, cooldown: 8 },
  evo: { level: 8, name: "Arcanjo", emoji: "🪽", image: "arcanjo.png", hpBoost: 65, dmgBoost: 75 }
},

{ id: 'guardiao', clan: 'sagrados', name: "Guardião", emoji: "🛡️", image: "guardiao.png", hp: 165,
  attacks: [
    { name: "Golpe Protetor", min: 12, max: 22 },
    { name: "Muralha Sagrada", min: 24, max: 34, cooldown: 3 }
  ],
  supportSkill: { name: "Provocar", desc: "Atrai todos os ataques inimigos por 2 turnos.", effect: "taunt_support", value: 0, turns: 2, cooldown: 5 },
  evo: { level: 7, name: "Guardião Eterno", emoji: "🗡️", image: "guardiao_eterno.png", hpBoost: 85, dmgBoost: 40 }
},

{ id: 'oraculo', clan: 'sagrados', name: "Oráculo", emoji: "🔮", image: "oraculo.png", hp: 85,
  attacks: [
    { name: "Visão Arcana", min: 14, max: 22 },
    { name: "Profecia Ruinosa", min: 28, max: 40, cooldown: 4 }
  ],
  supportSkill: { name: "Previsão Arcana", desc: "Garante que o próximo ataque do aliado cause o dano máximo possível.", effect: "guaranteed_max_dmg", cooldown: 4 },
  evo: { level: 7, name: "Oráculo Supremo", emoji: "✨", image: "oraculo_supremo.png", hpBoost: 50, dmgBoost: 65 }
},

{
  id: 'templario_luz',
  clan: 'sagrados',
  name: "Templário da Luz",
  emoji: "🛡️",
  image: "templario_luz.png",
  hp: 140,
  attacks: [
    { name: "Lâmina Consagrada", min: 16, max: 24 },
    { name: "Veredito Celestial", min: 28, max: 42, cooldown: 2 }
  ],
  supportSkill: { name: "Escudo da Fé", desc: "Cria um escudo que absorve 25 de dano por 2 turnos.", effect: "shield_flat", value: 25, turns: 2, cooldown: 4 },
  evo: {
    level: 12,
    name: "Paladino Solar",
    emoji: "☀️",
    image: "paladino_solar.png",
    hpBoost: 130,
    dmgBoost: 65
  }
},

{ id: 'azul_eterno', clan: 'sagrados', name: "Azul Eterno", emoji: "💙", image: "Azul Eterno.png", hp: 198,
  attacks: [
    { name: "Corte Iluminado", min: 28, max: 48 },
    { name: "Retaliação Eterna", min: 28, max: 78, recoil: 0.3 }
  ],
  supportSkill: { name: "Relâmpago Azul", desc: "Concede +60% de Esquiva para o aliado no próximo turno.", effect: "dodge_buff", value: 0.60, turns: 1, cooldown: 5},
  evo: { level: 12, name: "Paladino do Trono Azul", emoji: "🌀", image: "Paladino do Azul.png", hpBoost: 100, dmgBoost: 40 }
}, 

// 🌙 SOMBRAS
{ id: 'goblin', clan: 'sombras', name: "Goblin", emoji: "🧌", image: "goblin.png", hp: 100,
  attacks: [
    { name: "Dilacerar", min: 16, max: 22 },
    { name: "Carnificina", min: 30, max: 42, cooldown: 2 }
  ],
  supportSkill: { name: "Trapaça", desc: "Reduz o cooldown das habilidades do aliado em 1 turno.", effect: "cooldown_reduce_one", value: 1, cooldown: 5 },
  evo: { level: 10, name: "Goblin Supremo", emoji: "🧌", image: "goblin_supremo.png", hpBoost: 95, dmgBoost: 75 }
},

{ id: 'morcego', clan: 'sombras', name: "Morcego", emoji: "🦇", image: "morcego.png", hp: 75,
  attacks: [
    { name: "Sugada", min: 8, max: 12 },
    { name: "Eco-ataque", min: 22, max: 32, recoil: 0.2 }
  ],
  supportSkill: { name: "Névoa", desc: "Reduz a precisão dos inimigos em 20% por 2 turnos.", effect: "enemy_accuracy_down", value: 0.20, cooldown: 4, turns: 2 },
  evo: { level: 9, name: "Vampiro", emoji: "🧛", image: "vampiro.png", hpBoost: 55, dmgBoost: 75 }
},

{ id: 'zumbi', clan: 'sombras', name: "Zumbi", emoji: "🧟", image: "zumbi.png", hp: 90,
  attacks: [
    { name: "Mordida", min: 26, max: 35 },
    { name: "Desmembramento", min: 46, max: 76, cooldown: 3 }
  ],
  supportSkill: { name: "Carne Imortal", desc: "Recupera 15% de HP por turno durante 3 turnos.", effect: "regen_turns", value: 0.15, turns: 3, cooldown: 4 },
  evo: { level: 7, name: "Zumbi Forte", emoji: "🧟‍♂️", image: "zumbi_forte.png", hpBoost: 50, dmgBoost: 45 }
},

{ id: 'espectro', clan: 'sombras', name: "Espectro", emoji: "👻", image: "espectro.png", hp: 80,
  attacks: [
    { name: "Toque Etéreo", min: 18, max: 28 },
    { name: "Assombração", min: 28, max: 38, recoil: 0.25 }
  ],
  supportSkill: { name: "Velo das Sombras", desc: "Concede +30% de Esquiva para o aliado no próximo turno.", effect: "dodge_buff", value: 0.30, turns: 1, cooldown: 4 },
  evo: { level: 7, name: "Espectro Ancião", emoji: "☠️", image: "espectro_anciao.png", hpBoost: 45, dmgBoost: 60 }
},

{ id: 'demonio', clan: 'sombras', name: "Demônio", emoji: "😈", image: "demonio.png", hp: 125,
  attacks: [
    { name: "Chamas Infernais", min: 18, max: 28 },
    { name: "Pacto Profano", min: 36, max: 50, recoil: 0.3, cooldown: 3 }
  ],
  supportSkill: { name: "Aura de Medo", desc: "Reduz o dano causado pelo inimigo em 15% por 2 turnos.", effect: "enemy_damage_down", value: 0.15, turns: 2, cooldown: 5 },
  evo: { level: 9, name: "Demônio Abissal", emoji: "👹", image: "demonio_abissal.png", hpBoost: 75, dmgBoost: 80 }
},

{
  id: 'assassino_vazio',
  clan: 'sombras',
  name: "Assassino do Vazio",
  emoji: "🗡️",
  image: "assassino_vazio.png",
  hp: 90,
  attacks: [
    { name: "Lâmina Oculta", min: 22, max: 30 },
    { name: "Execução Sombria", min: 38, max: 52, cooldown: 3 }
  ],
  supportSkill: { name: "Poder do Abismo", desc: "Adiciona +50 de dano fixo ao próximo ataque do aliado.", effect: "fixed_dmg_buff", value: 50, cooldown: 3 },
  evo: {
    level: 10,
    name: "Carrasco do Abismo",
    emoji: "🩸",
    image: "carrasco_abismo.png",
    hpBoost: 50,
    dmgBoost: 90
  }
},


{
  id: 'O Primeiro Guardião',
  clan:'sombras',
  name: "O Primeiro Guardião",
  emoji: "🦋",
  image: "O Primeiro Guardião.png",
  hp: 125,
  attacks: [
    { name: "Asas do Crepúsculo", min: 52, max: 60, cooldown: 1  },
    { name: "Ilusão do Bosque Eterno", min: 75, max: 85, cooldown: 4 }
  ],
  supportSkills: [
                { name: "Sopro Vital", desc: "Cura 45% do HP da criatura ativa instantaneamente.", effect: "heal_percent", value: 0.45, cooldown: 3 }, 
                { name: "Restalração", desc: "Remove todos os debuffs negativos do aliado.", effect: "cleanse_debuffs", value: 1, cooldown: 2 },
                { name: "Barreira", desc: "Cria um escudo que absorve 120 de dano por 2 turnos.", effect: "shield_flat", value: 120, turns: 2, cooldown: 3 }
  ],
evo: {
    level: 8,
    name: "Guardião Primordial",
    emoji: "🦋",
    image: "Guardião Primordial.png",
    hpBoost: 65,
    dmgBoost: 100
  }
},


// 🌊 ABISSAIS
{ id: 'polvo', clan: 'abissais', name: "Polvo", emoji: "🐙", image: "polvo.png", hp: 115,
  attacks: [
    { name: "Onda de Choque", min: 14, max: 22 },
    { name: "Tsunami", min: 32, max: 44, cooldown: 3 }
  ],
  supportSkill: { name: "Tinta Obscurecedora", desc: "Faz o inimigo perder o próximo turno (50% de chance).", effect: "stun_chance", value: 0.50, cooldown: 6 },
  evo: { level: 8, name: "Polvo_Sombrio", emoji: "🦑", image: "polvo_sombrio.png", hpBoost: 70, dmgBoost: 65 }
},

{ id: 'kraken', clan: 'abissais', name: "Kraken", emoji: "🐙", image: "kraken.png", hp: 165,
  attacks: [
    { name: "Tentáculos", min: 18, max: 26 },
    { name: "Esmagamento Abissal", min: 34, max: 46, cooldown: 3 }
  ],
  supportSkill: { name: "Fúria dos Tentáculos", desc: "Permite que o aliado ataque duas vezes no próximo turno.", effect: "multi_hit_buff", value: 1, turns: 1, cooldown: 6 },
  evo: { level: 9, name: "Kraken Ancestral", emoji: "🦑", image: "kraken_ancestral.png", hpBoost: 85, dmgBoost: 65 }
},

{ id: 'serpente_marinha', clan: 'abissais', name: "Serpente Marinha", emoji: "🐉", image: "serpente_marinha.png", hp: 135,
  attacks: [
    { name: "Mordida Salgada", min: 18, max: 28 },
    { name: "Redemoinho", min: 30, max: 42, cooldown: 3 }
  ],
  evo: { level: 8, name: "Serpente Oceânica", emoji: "🌊", image: "serpente_oceanica.png", hpBoost: 65, dmgBoost: 70 }
},

{ id: 'abissal', clan: 'abissais', name: "Abissal", emoji: "🕳️", image: "abissal.png", hp: 115,
  attacks: [
    { name: "Vácuo Profundo", min: 18, max: 26 },
    { name: "Colapso", min: 32, max: 44, recoil: 0.25 }
  ],
  supportSkill: { name: "Olhar do Abismo", desc: "Faz o inimigo perder o próximo turno (40% de chance).", effect: "stun_chance", value: 0.40, cooldown: 5 },
  evo: { level: 8, name: "Entidade Abissal", emoji: "🌑", image: "entidade_abissal.png", hpBoost: 60, dmgBoost: 65 }
},

{ id: 'nautilus_ancestral', clan: 'abissais', name: "Nautilus Ancestral", emoji: "🐚", image: "nautilus_ancestral.png", hp: 195,
  attacks: [
    { name: "Casco Milenar", min: 10, max: 18 },
    { name: "Pressão Oceânica", min: 28, max: 38, cooldown: 3 }
  ],
  supportSkill: { name: "Purificação das Marés", desc: "Remove todos os debuffs negativos do aliado.", effect: "cleanse_debuffs", value: 1, cooldown: 4 },
  evo: { level: 10, name: "Nautilus Colossal", emoji: "🌀", image: "nautilus_colossal.png", hpBoost: 95, dmgBoost: 45 }
},

{
  id: 'sacerdote_profundo',
  clan: 'abissais',
  name: "Sacerdote das Profundezas",
  emoji: "🌀",
  image: "sacerdote_profundezas.png",
  hp: 120,
  attacks: [
    { name: "Sussurros do Abismo", min: 15, max: 23 },
    { name: "Ritual Profano", min: 34, max: 48, cooldown: 3 }
  ],
  evo: {
    level: 10,
    name: "Arauto do Caos",
    emoji: "🌑",
    image: "arauto_caos.png",
    hpBoost: 100,
    dmgBoost: 90
  }
}



];

    const CLAN_BOSSES = {
        primordiais: {
            name: "TITÃ DO FIM DOS TEMPOS",
            clan: "primordiais",
            emoji: "🐉",
            image: "Titã do Fim dos Tempos.png",
            hp: 1300,
            level: 50,
            attacks: [
                { name: "Sopro Apocalíptico", min: 0, max: 550, cooldown: 0, ignoreClans: true },
                { name: "Garras Vulcânicas", min: 130, max: 190 },
                { name: "Explosão Primordial", min: 220, max: 380 }
            ]
        },
        selvagens: {
            name: "ESPÍRITO ANCESTRAL DA SELVA",
            clan: "selvagens",
            emoji: "🐺",
            image: "Espírito Ancestral da Selva.png",
            hp: 1400,
            level: 50,
            attacks: [
                { name: "Grito do Caos", min: 0, max: 520, cooldown: 0, ignoreClans: true },
                { name: "Vulto da Floresta", min: 120, max: 180 },
                { name: "Respiração Selvagem", min: 200, max: 360 }
            ]
        },
        sagrados: {
            name: "ARCANJO DA ETERNIDAD",
            clan: "sagrados",
            emoji: "☀️",
            image: "Arcanjo da Eternidade.png",
            hp: 1200,
            level: 50,
            attacks: [
                { name: "Veredito Sagrado", min: 0, max: 580, cooldown: 0, ignoreClans: true },
                { name: "Luz Purificadora", min: 140, max: 200 },
                { name: "Explosão de Luz", min: 230, max: 390 }
            ]
        },
        sombras: {
            name: "SOBERANO DO ECLIPSE",
            clan: "sombras",
            emoji: "👺",
            image: "Soberano do Eclipse.png",
            hp: 1200,
            level: 50,
            attacks: [
                { name: "Apocalipse", min: 0, max: 500, cooldown: 0, ignoreClans: true },
                { name: "Garras do Caos", min: 120, max: 180 },
                { name: "Chama Negra", min: 200, max: 360 }
            ]
        },
        abissais: {
            name: "IMPERADOR DO ABISMO",
            clan: "abissais",
            emoji: "🐙",
            image: "Imperador do Abismo.png",
            hp: 1500,
            level: 50,
            attacks: [
                { name: "Vácuo Abissal", min: 0, max: 540, cooldown: 0, ignoreClans: true },
                { name: "Tentáculos da Escuridão", min: 110, max: 170 },
                { name: "Colapso Oceânico", min: 190, max: 350 }
            ]
        }
    };

    const BOSS = CLAN_BOSSES.sombras;

    const ITEMS = {
        // Ofensiva
        'garra_obsidiana': { id: 'garra_obsidiana', name: "Garra de Obsidiana", desc: "+50 de dano fixo em todos os ataques.", cost: 150, type: 'offense', icon: "💎", image: "garra_obsidiana.png" },
        'anel_fogo': { id: 'anel_fogo', name: "Anel de Fogo", desc: "Aumenta o dano máximo em 15.", cost: 120, type: 'offense', icon: "🔥", image: "anel_fogo.png" },
        'amolador_presas': { id: 'amolador_presas', name: "Amolador de Presas", desc: "Dano mínimo nunca é inferior a 80.", cost: 100, type: 'offense', icon: "🦴", image: "amolador_presas.png" },
        'cajado_runico': { id: 'cajado_runico', name: "Cajado Rúnico", desc: "+5% de dano para evoluções finais.", cost: 200, type: 'offense', icon: "🪄", image: "cajado_runico.png" },
        // Defensiva
        'casco_adamante': { id: 'casco_adamante', name: "Casco de Adamante", desc: "+100 de HP máximo.", cost: 130, type: 'defense', icon: "🛡️", image: "casco_adamante.png" },
        'manto_esquiva': { id: 'manto_esquiva', name: "Manto da Esquiva", desc: "25% de chance de desviar de ataques.", cost: 180, type: 'defense', icon: "🧣", image: "manto_esquiva.png" },
        'colar_regeneracao': { id: 'colar_regeneracao', name: "Colar de Regeneração", desc: "Cura uma parte do HP total ao atacar.", cost: 150, type: 'defense', icon: "📿", image: "colar_regeneracao.png" },
        'escudo_espinhos': { id: 'escudo_espinhos', name: "Escudo de Espinhos", desc: "Reflete 45% do dano recebido.", cost: 140, type: 'defense', icon: "🌵", image: "escudo_espinhos.png" },
        // Utilitários
        'grimorio_aprendiz': { id: 'grimorio_aprendiz', name: "Grimório de Aprendiz", desc: "+60% de ganho de XP.", cost: 100, type: 'utility', icon: "📖", image: "grimorio_aprendiz.png" },
        'bolsa_moedas': { id: 'bolsa_moedas', name: "Bolsa de Moedas", desc: "+100 diamantes em baús de tesouro.", cost: 80, type: 'utility', icon: "💰", image: "bolsa_moedas.png" },
        'incenso_atrativo': { id: 'incenso_atrativo', name: "Incenso Atrativo", desc: "Aumenta chance de captura em encontros.", cost: 90, type: 'utility', icon: "💨", image: "incenso_atrativo.png" },
        'pedra_evolucao': { id: 'pedra_evolucao', name: "Pedra da Evolução Precoce", desc: "Evolui 1 nível antes do requisito.", cost: 150, type: 'utility', icon: "💠", image: "pedra_evolucao.png" },

        // --- Itens (Consumíveis) ---
        'cristal_evolucao_primordiais': { id: 'cristal_evolucao_primordiais', name: "Cristal da Evolução (Primordiais)", desc: "Evolui instantaneamente uma criatura do clã Primordiais.", cost: 260, type: 'consumable', icon: "🧿", image: "cristal primordial.gif", minLevel: 6 },
        'cristal_evolucao_selvagens': { id: 'cristal_evolucao_selvagens', name: "Cristal da Evolução (Selvagens)", desc: "Evolui instantaneamente uma criatura do clã Selvagens.", cost: 260, type: 'consumable', icon: "🧿", image: "cristal selvagem.gif", minLevel: 6 },
        'cristal_evolucao_sagrados': { id: 'cristal_evolucao_sagrados', name: "Cristal da Evolução (Sagrados)", desc: "Evolui instantaneamente uma criatura do clã Sagrados.", cost: 260, type: 'consumable', icon: "🧿", image: "cristal sagrado.gif", minLevel: 6 },
        'cristal_evolucao_sombras': { id: 'cristal_evolucao_sombras', name: "Cristal da Evolução (Sombras)", desc: "Evolui instantaneamente uma criatura do clã Sombras.", cost: 260, type: 'consumable', icon: "🧿", image: "cristal sombrio.gif", minLevel: 6 },
        'cristal_evolucao_abissais': { id: 'cristal_evolucao_abissais', name: "Cristal da Evolução (Abissais)", desc: "Evolui instantaneamente uma criatura do clã Abissais.", cost: 260, type: 'consumable', icon: "🧿", image: "cristal abissal.gif", minLevel: 6 },
        'pocao_grupal': { id: 'pocao_grupal', name: "Poção Grupal", desc: "Cura todos os efeitos negativos das criaturas do jogador.", cost: 200, type: 'consumable', icon: "🧪", image: "skill_heal.png", minLevel: 3 },
        // Raros (Nível 30+)
        'coroa_demonio': { id: 'coroa_demonio', name: "Coroa do Rei Demônio", desc: "+20 de dano, mas corta HP pela metade.", cost: 400, type: 'rare', icon: "👑", image: "coroa_demonio.png", minLevel: 30 },
        'fenix_bolso': { id: 'fenix_bolso', name: "Fênix de Bolso", desc: "Revive uma vez com 30% HP (Quebra após o uso).", cost: 500, type: 'rare', icon: "🐣", image: "fenix_bolso.png", minLevel: 30 },
        'espelho_mistico': { id: 'espelho_mistico', name: "Espelho Místico", desc: "Reflete o ataque mais forte do inimigo.", cost: 450, type: 'rare', icon: "🪞", image: "espelho_mistico.png", minLevel: 30 },
        
        // --- Novos Equipamentos (Buffs & Debuffs por Turno) ---
        'amuleto_ritmo': { id: 'amuleto_ritmo', name: "Amuleto do Ritmo", desc: "+10% dano por turno consecutivo sem trocar. Reseta ao trocar ou HP < 30%.", cost: 220, type: 'offense', icon: "🧩", image: "amuleto_ritmo.png" },
        'coracao_ancestral': { id: 'coracao_ancestral', name: "Coração Ancestral", desc: "Regenera 4% HP por turno, mas reduz dano em 6%.", cost: 200, type: 'defense', icon: "🟢", image: "coracao_ancestral.png" },
        'manto_persistencia': { id: 'manto_persistencia', name: "Manto da Persistência", desc: "+5% defesa por turno vivo (máx 25%+). Some ao trocar.", cost: 250, type: 'defense', icon: "🧥", image: "manto_persistencia.png" },
        'olho_predador': { id: 'olho_predador', name: "Olho do Predador", desc: "Após 2 turnos no mesmo alvo, ignora esquiva, defesa e fraqueza.", cost: 280, type: 'offense', icon: "👁️", image: "olho_predador.png" },
        'tonico_guerra': { id: 'tonico_guerra', name: "Tônico de Guerra", desc: "Consumível: +15% dano por 3 turnos. Depois causa exaustão e quebra.", cost: 150, type: 'utility', icon: "🧪", image: "tonico_guerra.png" },
        'correntes_esquecimento': { id: 'correntes_esquecimento', name: "Correntes do Esquecimento", desc: "Inimigo perde 5% dano/turno (acumula 4x). Reseta se ele esquivar.", cost: 210, type: 'utility', icon: "⛓️", image: "correntes_esquecimento.png" },
        'marca_abismo': { id: 'marca_abismo', name: "Marca do Abismo", desc: "Inimigo recebe +6% dano/turno. Explode e elimina se HP < 30%.", cost: 300, type: 'rare', icon: "🔴", image: "marca_abismo.png" },
        'nevoa_entorpecente': { id: 'nevoa_entorpecente', name: "Névoa Entorpecente", desc: "+10% chance de erro do oponente por turno (máx 60%).", cost: 240, type: 'utility', icon: "🌫️", image: "nevoa_entorpecente.png" },
        'sangramento': { id: 'sangramento', name: "Sangramento", desc: "Causa 3% HP/turno (3 turnos) e cura metade do valor ao portador.", cost: 190, type: 'offense', icon: "🩸", image: "sangramento.png" },
        'selo_ruptura': { id: 'selo_ruptura', name: "Selo da Ruptura", desc: "Reduz 1 nível de item inimigo por turno. Nível 0 quebra o item.", cost: 350, type: 'rare', icon: "🔱", image: "selo_ruptura.png" },
        'lamina_instavel': { id: 'lamina_instavel', name: "Lâmina Instável", desc: "+20% dano, mas tem 10% de chance de se ferir por turno.", cost: 180, type: 'offense', icon: "🔪", image: "lamina_instavel.png" },
        'relogio_entropia': { id: 'relogio_entropia', name: "Relógio da Entropia", desc: "Turno ímpar: +15% esquiva. Turno par: -15% defesa.", cost: 230, type: 'utility', icon: "⏳", image: "relogio_entropia.png" },
        'mascara_sacrificio': { id: 'mascara_sacrificio', name: "Máscara do Sacrifício", desc: "+5% dano por debuff no inimigo. -3% HP/turno por buff em você.", cost: 320, type: 'rare', icon: "🎭", image: "mascara_sacrificio.png" }
    };

    const SOUNDS = {
        'click': 'sons/click.mp3',
        'battle_start': 'sons/battle_start.mp3',
        'attack': 'sons/attack.mp3',
        'hit': 'sons/hit.mp3',
        'victory': 'sons/victory.mp3',
        'defeat': 'sons/defeat.mp3',
        'card_flip': 'sons/card_flip.mp3',
        'treasure': 'sons/treasure.mp3',
        'shop': 'sons/shop.mp3',
        'evolution': 'sons/evolution.mp3',
        'boss_battle': 'sons/boss_battle.mp3',
        'background': 'sons/background.mp3'
    };

    let currentAudio = null;
    let backgroundAudio = null;

    function playSound(name) {
        if (state.options.muted) return;
        const audio = new Audio(SOUNDS[name]);
        let vol = state.options.volume || 0.5;
        if (name === 'hit') vol = Math.min(1.0, vol * 3);
        audio.volume = vol;
        
        if (name === 'defeat') currentAudio = audio;
        
        audio.play().catch(e => console.log("Erro ao tocar som: " + e));
    }

    function pickExplorationCardType() {
        normalizeState();
        const r = Math.random();
        const after5 = state.level > 5;
        const teamOk = state.team.length > 1;
        const pool = [];
        pool.push({ t: 'COMBAT', w: 0.28 });
        pool.push({ t: 'ENCOUNTER', w: 0.16 });
        pool.push({ t: 'TREASURE', w: 0.16 });
        pool.push({ t: 'TRAP', w: 0.14 });
        if (after5) pool.push({ t: 'ELITE', w: 0.08 });
        pool.push({ t: 'CHAOS_EVENT', w: 0.10 });
        pool.push({ t: 'WANDERING_MERCHANT', w: 0.08 });
        if (teamOk) pool.push({ t: 'ANCIENT_ALTAR', w: 0.06 });
        pool.push({ t: 'AMBUSH', w: 0.08 });
        pool.push({ t: 'LOST_RELIC', w: 0.04 });
        pool.push({ t: 'CURSE', w: 0.06 });
        pool.push({ t: 'FUTURE_VISION', w: 0.04 });
        pool.push({ t: 'WILD_CALL', w: 0.08 });
        pool.push({ t: 'CHALLENGE', w: 0.06 });
        const total = pool.reduce((s, p) => s + p.w, 0);
        let pick = r * total;
        for (let i = 0; i < pool.length; i++) {
            pick -= pool[i].w;
            if (pick <= 0) return pool[i].t;
        }
        return 'COMBAT';
    }

    function stopSound() {
        if (currentAudio) {
            currentAudio.pause();
            currentAudio.currentTime = 0;
            currentAudio.src = "";
            currentAudio = null;
        }
    }

    function playBackgroundMusic() {
        if (state.options.muted) return;
        if (!SOUNDS.background) return;
        if (backgroundAudio) {
            backgroundAudio.pause();
            backgroundAudio.currentTime = 0;
        }
        backgroundAudio = new Audio(SOUNDS.background);
        backgroundAudio.loop = true;
        backgroundAudio.volume = (state.options.volume || 0.5) * 0.4;
        backgroundAudio.play().catch(e => console.log("Erro ao tocar música: " + e));
    }

    function stopBackgroundMusic() {
        if (backgroundAudio) {
            backgroundAudio.pause();
            backgroundAudio.currentTime = 0;
            backgroundAudio = null;
        }
    }

    // --- Game State ---
    let state = {
        level: 1,
        team: [],
        maxTeamSize: 6,
        cardsRevealed: 0,
        enemies: [],
        activePlayerCreature: null,
        activePlayerCreatures: [],
        inventory: [],
        battleLoot: [],
        isBossBattle: false,
        swapsUsed: 0,
        selectedIniciais: [],
        diamantes: 10,
        lux: 80,
        tcgFreePacks: 5,
        tcgUnlockedIds: [],
        lockedShopItem: null,
        currentShopItems: null,
        options: {
            fontSize: 16,
            isFullScreen: false,
            muted: false,
            volume: 1.0,
            mobileMode: false
        },
        tcgDeck: []
    };

    // --- Inventory Helpers ---
    function normalizeState() {
        if (!state) state = {};
        if (!Array.isArray(state.team)) state.team = [];
        if (!Array.isArray(state.enemies)) state.enemies = [];
        if (!Array.isArray(state.activePlayerCreatures)) state.activePlayerCreatures = [];
        if (!Array.isArray(state.inventory)) state.inventory = [];
        if (!Array.isArray(state.tcgUnlockedIds)) state.tcgUnlockedIds = [];
        if (typeof state.tcgFreePacks !== 'number') state.tcgFreePacks = 0;
        if (typeof state.lux !== 'number') state.lux = 0;
        
        // Inicializa sistema de desbloqueio
        if (!state.unlockedCreatures) {
            loadUnlockedCreatures(); // Tenta carregar do arquivo separado
            if (!state.unlockedCreatures) state.unlockedCreatures = {};
        }

        if (!state.options) {
            state.options = {
                fontSize: 16,
                isFullScreen: false,
                muted: false,
                volume: 1.0,
                mobileMode: false
            };
        } else {
            if (state.options.fontSize == null) state.options.fontSize = 16;
            if (typeof state.options.isFullScreen !== 'boolean') state.options.isFullScreen = false;
            if (typeof state.options.muted !== 'boolean') state.options.muted = false;
            if (typeof state.options.volume !== 'number') state.options.volume = 0.5;
            if (typeof state.options.mobileMode !== 'boolean') state.options.mobileMode = false;
        }

        state.team.forEach(c => {
            if (!c.items) c.items = [];
            if (!c.cooldowns) c.cooldowns = {};
            if (c.currentHp === undefined) c.currentHp = c.hp;
            if (c.level === undefined) c.level = 1;
            if (c.defeated === undefined) c.defeated = false;
        });
    }

    function makeUID() {
        return `${Date.now()}_${Math.floor(Math.random() * 1e9)}`;
    }

    function createItemInstance(baseItem, level = 1) {
        const it = JSON.parse(JSON.stringify(baseItem));
        it.level = it.level || level;
        it.uid = it.uid || makeUID();
        return it;
    }

    function addItemToInventory(item) {
        normalizeState();
        const it = createItemInstance(item, item.level || 1);
        state.inventory.push(it);
        saveGame();
    }

    function removeItemFromInventory(uid) {
        normalizeState();
        const idx = state.inventory.findIndex(i => i.uid === uid);
        if (idx >= 0) return state.inventory.splice(idx, 1)[0];
        return null;
    }

    // --- Consumables (Itens) ---
    function isEvolutionCrystal(item) {
        return !!(item && item.id && item.id.startsWith('cristal_evolucao_'));
    }

    function getEvolutionCrystalClan(item) {
        if (!item || !item.id) return null;
        return item.id.replace('cristal_evolucao_', '');
    }

    function tryUseEvolutionCrystal(itemInstance, onDone, onFail) {
        if (!itemInstance || !isEvolutionCrystal(itemInstance)) {
            onFail && onFail('Este item não é um Cristal da Evolução.');
            return;
        }

        showCreatureSelection((target) => {
            if (!target) return;

            const clanNeeded = getEvolutionCrystalClan(itemInstance);
            if (!clanNeeded) {
                onFail && onFail('Cristal inválido.');
                return;
            }

            if (target.clan !== clanNeeded) {
                const clanName = CLANS[clanNeeded]?.name || clanNeeded;
                onFail && onFail(`Este cristal só funciona em criaturas do clã ${clanName}.`);
                return;
            }

            if (!target.evo) {
                onFail && onFail('Esta criatura não possui evolução definida.');
                return;
            }

            if (target.isEvolved) {
                onFail && onFail('Esta criatura já está evoluída.');
                return;
            }

            evolve(target, () => {
                saveGame();
                updateUI();
                onDone && onDone(target);
            });
            addLog(`🧿 ${target.name} evoluiu usando ${itemInstance.name}!`);

        }, `Use ${itemInstance.name} em qual criatura?`);
    }

    function useEvolutionCrystalFromInventory(uid, backFilter = 'all') {
        normalizeState();
        const item = state.inventory.find(i => i.uid === uid);
        if (!item) return showInventoryModal(backFilter);

        const fail = (msg) => {
            showModal('Não foi possível usar', msg, [
                { text: 'Ok', action: () => showInventoryItemModal(uid, backFilter) }
            ]);
        };

        tryUseEvolutionCrystal(item, () => {
            // Consome o item apenas se funcionou
            removeItemFromInventory(uid);
            saveGame();
            showInventoryModal(backFilter);
        }, fail);
    }

    function usePocaoGrupalFromInventory(uid, backFilter = 'all') {
        normalizeState();
        const item = state.inventory.find(i => i.uid === uid);
        if (!item) return showInventoryModal(backFilter);

        // Cura todos os efeitos negativos das criaturas do jogador
        let curedAny = false;
        state.team.forEach(creature => {
            if (creature.battleState) {
                // Remove todos os efeitos negativos do battleState
                if (creature.battleState.damageReduction > 0) {
                    creature.battleState.damageReduction = 0;
                    curedAny = true;
                }
                if (creature.battleState.takenDamageIncrease > 0) {
                    creature.battleState.takenDamageIncrease = 0;
                    curedAny = true;
                }
                if (creature.battleState.missChance > 0) {
                    creature.battleState.missChance = 0;
                    curedAny = true;
                }
                if (creature.battleState.bleedTurns > 0) {
                    creature.battleState.bleedTurns = 0;
                    curedAny = true;
                }
                // Remove paralisia de armadilha
                if (creature.trapParalysisTurns > 0) {
                    creature.trapParalysisTurns = 0;
                    curedAny = true;
                }
            }
        });

        if (curedAny) {
            addLog("🧪 Poção Grupal: Todos os efeitos negativos foram curados!");
        } else {
            addLog("🧪 Poção Grupal: Nenhum efeito negativo encontrado para curar.");
        }

        // Consome o item
        removeItemFromInventory(uid);
        saveGame();
        updateUI();
        showInventoryModal(backFilter);
    }

    function applyImmediateItemEffects(creature, item) {
        if (!creature || !item) return;
        // Casco de Adamante (+100 HP)
        if (item.id === 'casco_adamante' && !item.appliedHpBonus) {
            const bonus = 100;
            creature.hp += bonus;
            creature.currentHp += bonus;
            item.appliedHpBonus = bonus;
        }
        // Coroa do Demônio (HP pela metade)
        if (item.id === 'coroa_demonio' && !item.appliedCrownOriginalHp) {
            item.appliedCrownOriginalHp = creature.hp;
            creature.hp = Math.floor(creature.hp / 2);
            creature.currentHp = Math.min(creature.currentHp, creature.hp);
        }
    }

    function removeImmediateItemEffects(creature, item) {
        if (!creature || !item) return;
        if (item.id === 'casco_adamante' && item.appliedHpBonus) {
            creature.hp = Math.max(1, creature.hp - item.appliedHpBonus);
            creature.currentHp = Math.min(creature.currentHp, creature.hp);
            item.appliedHpBonus = 0;
        }
        if (item.id === 'coroa_demonio' && item.appliedCrownOriginalHp) {
            creature.hp = item.appliedCrownOriginalHp;
            creature.currentHp = Math.min(creature.currentHp, creature.hp);
            item.appliedCrownOriginalHp = 0;
        }
    }

    // --- Core Functions ---

    function startNewGame() {
        playSound('click');
        playBackgroundMusic();
        document.getElementById('main-menu').style.display = 'none';
        document.getElementById('game-container').style.display = 'block';
        restoreStandardBattleUI();
        initGame();
        autoScaleGame();
    }

    function continueGame() {
        const saved = localStorage.getItem('aventura_save');
        if (saved) {
            state = JSON.parse(saved);
            normalizeState();
            applyAccountSaveToState(state);
            document.getElementById('main-menu').style.display = 'none';
            document.getElementById('game-container').style.display = 'block';
            restoreStandardBattleUI();
            playBackgroundMusic();
            updateUI();
            generateLevel();
            autoScaleGame();
        }
    }

    function saveGame() {
        localStorage.setItem('aventura_save', JSON.stringify(state));
        document.getElementById('continue-btn').disabled = false;
        saveAccountFromState(state);
        // Salva também as criaturas desbloqueadas em arquivo separado
        saveUnlockedCreatures();
    }

    function saveUnlockedCreatures() {
        // Salva as criaturas desbloqueadas em arquivo JSON separado
        const unlockedData = {
            unlockedCreatures: state.unlockedCreatures || {}
        };
        
        // Converte para JSON e salva no localStorage como fallback
        localStorage.setItem('unlocked_creatures', JSON.stringify(unlockedData));
        
        // Tenta salvar em arquivo (para uso futuro com servidor)
        try {
            const blob = new Blob([JSON.stringify(unlockedData, null, 2)], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            
            // Cria um link de download (para debug/backup)
            const a = document.createElement('a');
            a.href = url;
            a.download = 'unlocked-creatures.json';
            a.style.display = 'none';
            document.body.appendChild(a);
            
            // Remove após um tempo
            setTimeout(() => {
                document.body.removeChild(a);
                URL.revokeObjectURL(url);
            }, 100);
        } catch (error) {
            console.log('Erro ao salvar arquivo de criaturas desbloqueadas:', error);
        }
    }

    function loadUnlockedCreatures() {
        // Carrega criaturas desbloqueadas do localStorage
        try {
            const saved = localStorage.getItem('unlocked_creatures');
            if (saved) {
                const data = JSON.parse(saved);
                if (data.unlockedCreatures) {
                    state.unlockedCreatures = data.unlockedCreatures;
                }
            }
        } catch (error) {
            console.log('Erro ao carregar criaturas desbloqueadas:', error);
            state.unlockedCreatures = {};
        }
    }

    function applyOptions() {
    // Apply Mobile Mode class if enabled manually
    if (state.options.mobileMode) {
        document.body.classList.add('mobile-mode');
    } else {
        document.body.classList.remove('mobile-mode');
    }
}

function toggleMobileMode() {
    state.options.mobileMode = !state.options.mobileMode;
    applyOptions();
    saveGame();
    showOptionsMenu(); // Refresh menu to show new state
}

    function showOptionsMenu() {
        const volumePercent = Math.round((state.options.volume || 1.0) * 100);
         const mobileText = state.options.mobileMode ? "Modo Mobile: ON" : "Modo Mobile: OFF";
        const body = `
            <div style="display:flex; flex-direction:column; gap:10px;">
                <div>
                    Volume: <span id="volume-label">${volumePercent}%</span>
                </div>
                <input 
                    id="volume-slider"
                    type="range"
                    min="0"
                    max="100"
                    value="${volumePercent}"
                    style="width:100%;"
                />
            </div>
        `;

        showModal("Opções", body, [
            { text: mobileText, action: toggleMobileMode },
            { text: "Aumentar Texto", action: () => changeFontSize(2) },
            { text: "Diminuir Texto", action: () => changeFontSize(-2) },
            { text: "Alternar FullScreen", action: toggleFullScreen },
            { text: state.options.muted ? "Ativar Som" : "Mudar Som", action: toggleMute },
            { text: "Exportar Save", action: exportSave },
            { text: "Importar Save", action: importSave },
            { text: "Apagar Progresso", action: confirmClearAllProgress },
            { text: "Voltar ao Menu", action: () => location.reload() },
            { text: "Fechar", action: hideModal }
        ]);

        const slider = document.getElementById('volume-slider');
        const label = document.getElementById('volume-label');
        if (slider && label) {
            slider.oninput = () => {
                const val = parseInt(slider.value, 10);
                label.innerText = `${val}%`;
                changeVolume(val / 100);
            };
        }
    }

    function toggleMute() {
        state.options.muted = !state.options.muted;
        if (state.options.muted) {
            stopBackgroundMusic();
        } else {
            playBackgroundMusic();
        }
        saveGame();
        showOptionsMenu();
    }

    function exportSave() {
        let accountData = null;
        if (typeof loadAccountSave === 'function') accountData = loadAccountSave();
        if (!accountData) {
            accountData = {
                accountLevel: state.accountLevel || 1,
                lux: typeof state.lux === 'number' ? state.lux : 0,
                tcgUnlockedIds: Array.isArray(state.tcgUnlockedIds) ? state.tcgUnlockedIds : [],
                tcgDecks: Array.isArray(state.tcgDecks) ? state.tcgDecks : []
            };
        }
        const payload = { adventure: state, account: accountData };
        const dataStr = JSON.stringify(payload);
        const dataUri = 'data:application/json;charset=utf-8,' + encodeURIComponent(dataStr);
        
        const exportFileDefaultName = 'progresso_kom.json';
        
        const linkElement = document.createElement('a');
        linkElement.setAttribute('href', dataUri);
        linkElement.setAttribute('download', exportFileDefaultName);
        linkElement.click();
        addLog("Save exportado com sucesso!");
    }

    function showCreatureCollection() {
        // Inicializa os sistemas se não existirem
        if (!state.unlockedCreatures) {
            state.unlockedCreatures = {};
        }
        
        // Garante que as criaturas iniciais de cada clã estão desbloqueadas
        Object.values(CLANS).forEach(clan => {
            if (clan.initialCreature && !state.unlockedCreatures[clan.initialCreature]) {
                state.unlockedCreatures[clan.initialCreature] = true;
            }
        });
        
        const body = document.createElement('div');
        body.style.display = 'flex';
        body.style.flexWrap = 'wrap';
        body.style.gap = '10px';
        body.style.justifyContent = 'center';
        body.style.maxHeight = '70vh';
        body.style.overflowY = 'auto';
        body.style.padding = '10px';
        
        // Agrupa criaturas por clã
        const creaturesByClan = {};
        CREATURE_POOL.forEach(creature => {
            if (!creaturesByClan[creature.clan]) {
                creaturesByClan[creature.clan] = [];
            }
            creaturesByClan[creature.clan].push(creature);
        });
        
        // Cria seções para cada clã
        Object.entries(creaturesByClan).forEach(([clanId, creatures]) => {
            const clanSection = document.createElement('div');
            clanSection.style.width = '100%';
            clanSection.style.marginBottom = '20px';
            
            const clanName = document.createElement('h3');
            clanName.textContent = CLANS[clanId]?.name || clanId;
            clanName.style.margin = '0 0 10px 0';
            clanName.style.color = 'var(--accent-color)';
            clanName.style.textAlign = 'center';
            clanSection.appendChild(clanName);
            
            const creaturesContainer = document.createElement('div');
            creaturesContainer.style.display = 'flex';
            creaturesContainer.style.flexWrap = 'wrap';
            creaturesContainer.style.gap = '10px';
            creaturesContainer.style.justifyContent = 'center';
            
            creatures.forEach(creature => {
                const isUnlocked = state.unlockedCreatures[creature.id];
                
                const card = document.createElement('div');
                card.className = isUnlocked ? 'creature-card tooltip' : 'creature-card tooltip locked';
                card.style.width = '140px';
                card.style.height = '180px';
                
                if (!isUnlocked) {
                    card.style.opacity = '0.7';
                    card.style.cursor = 'pointer';
                    card.title = `Capture ${creature.name} para desbloqueá-la!\nClã: ${CLANS[creature.clan]?.name || creature.clan}`;
                } else {
                    card.title = getCreatureTooltip(creature);
                }
                
                card.innerHTML = `
                    ${getCreatureDisplay(creature)}
                    <div class="creature-card-info">
                        <div class="creature-card-name">${creature.name}</div>
                        <div class="creature-card-stats">
                            ${isUnlocked ? `Lvl ${creature.level}` : `Capture para desbloquear!`}
                        </div>
                    </div>
                `;
                
                if (isUnlocked) {
                    card.ondblclick = (e) => {
                        e.stopPropagation();
                        showModal("Status: " + creature.name, getCreatureTooltip(creature), [
                            { text: "Voltar", action: () => showCreatureCollection() }
                        ]);
                    };
                } else {
                    card.onclick = (e) => {
                        e.stopPropagation();
                        // Criaturas bloqueadas não podem ser clicadas para desbloqueio
                        showModal("Criatura Bloqueada", `
                            <div style="text-align:center;">
                                ${getCreatureImageOnly(creature, "80px")}
                                <strong>${creature.name}</strong> está bloqueada!<br>
                                <span style="color:gold;">Capture esta criatura para desbloqueá-la!</span>
                            </div>
                        `, [{ text: "Entendi", action: hideModal }]);
                    };
                }
                
                creaturesContainer.appendChild(card);
            });
            
            clanSection.appendChild(creaturesContainer);
            body.appendChild(clanSection);
        });
        
        const unlockedCount = Object.values(state.unlockedCreatures).filter(Boolean).length;
        const totalCount = CREATURE_POOL.length;
        
        showModal(
            `Coleção de Criaturas (${unlockedCount}/${totalCount})`, 
            body, 
            [
                { text: "Fechar", action: hideModal }
            ]
        );
    }

    function importSave() {
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = '.json';
        input.onchange = e => {
            const file = e.target.files[0];
            const reader = new FileReader();
            reader.onload = readerEvent => {
                try {
                    const content = readerEvent.target.result;
                    const parsed = JSON.parse(content);
                    if (parsed && parsed.adventure) {
                        state = parsed.adventure;
                        if (parsed.account) {
                            localStorage.setItem('kom_account_save', JSON.stringify(parsed.account));
                        }
                    } else {
                        state = parsed;
                    }
                    normalizeState();
                    if (typeof applyAccountSaveToState === 'function') applyAccountSaveToState(state);
                    saveGame();
                    alert("Save importado com sucesso! O jogo irá recarregar.");
                    location.reload();
                } catch (err) {
                    alert("Erro ao importar o arquivo!");
                }
            }
            reader.readAsText(file);
        }
        input.click();
    }

    function clearAllProgress(doReload = true) {
        localStorage.removeItem('aventura_save');
        localStorage.removeItem('kom_account_save');
        const btn = document.getElementById('continue-btn');
        if (btn) btn.disabled = true;
        if (doReload) location.reload();
    }

    function confirmClearAllProgress() {
        showModal("Apagar Progresso", `
            <div style="text-align:center;">
                Tem certeza? Isso apaga todo o progresso da aventura e do TCG.
            </div>
        `, [
            { text: "Apagar Tudo", action: () => clearAllProgress(true) },
            { text: "Cancelar", action: showOptionsMenu }
        ]);
    }

    if (!localStorage.getItem('kom_reset_done')) {
        if (localStorage.getItem('aventura_save') || localStorage.getItem('kom_account_save')) {
            clearAllProgress(false);
        }
        localStorage.setItem('kom_reset_done', '1');
    }

    function changeFontSize(delta) {
        state.options.fontSize = Math.min(24, Math.max(12, state.options.fontSize + delta));
        document.documentElement.style.setProperty('--base-font-size', state.options.fontSize + 'px');
        saveGame();
    }

    function changeVolume(value) {
        state.options.volume = Math.min(1, Math.max(0, value));
        if (backgroundAudio) {
            backgroundAudio.volume = (state.options.volume || 0.1) * 0.4;
        }
        saveGame();
    }

    function toggleFullScreen() {
        if (!document.fullscreenElement) {
            document.documentElement.requestFullscreen().catch(e => {
                alert(`Erro ao tentar entrar em modo tela cheia: ${e.message}`);
            });
        } else {
            if (document.exitFullscreen) {
                document.exitFullscreen();
            }
        }
    }

    function startInitialSelection() {
        state.team = [];
        state.selectedIniciais = [];
        
        // Carrega criaturas desbloqueadas se existirem
        loadUnlockedCreatures();
        
        // Se não houver criaturas desbloqueadas, desbloqueia as iniciais
        if (Object.keys(state.unlockedCreatures || {}).length === 0) {
            state.unlockedCreatures = {};
            Object.values(CLANS).forEach(clan => {
                if (clan.initialCreature) {
                    state.unlockedCreatures[clan.initialCreature] = true;
                }
            });
        }
        
        const unlockedCount = Object.values(state.unlockedCreatures).filter(Boolean).length;
        showModal("Escolha suas Criaturas", `Selecione de 1 a 3 criaturas desbloqueadas para começar sua jornada:<br><small>Você tem ${unlockedCount} criaturas desbloqueadas</small>`, []);
        
        const footer = document.getElementById('modal-footer');
        footer.innerHTML = '';
        
        // Filtra apenas criaturas desbloqueadas
        const unlockedCreatures = CREATURE_POOL.filter(c => state.unlockedCreatures[c.id]);
        
        // Se não houver criaturas desbloqueadas, mostra mensagem
        if (unlockedCreatures.length === 0) {
            footer.innerHTML = '<div style="text-align:center; padding:20px; color:#aaa;">Nenhuma criatura desbloqueada ainda. Capture criaturas durante o jogo para desbloqueá-las!</div>';
            return;
        }
        
        unlockedCreatures.forEach(c => {
            const card = document.createElement('div');
            const isUnlocked = true; // Todas estão desbloqueadas pois filtramos
            
            card.className = isUnlocked ? 'creature-card tooltip' : 'creature-card tooltip locked';
            card.innerHTML = `
                ${getCreatureDisplay(c)}
                <div class="creature-card-info">
                    <div class="creature-card-name">${c.name}</div>
                    <div class="creature-card-stats">HP: ${c.hp}</div>
                </div>
                <span class="tooltiptext">${getCreatureTooltip({...c, level: 1, currentHp: c.hp})}</span>
            `;
            
            card.onclick = () => {
                if (card.classList.contains('selected')) {
                    card.classList.remove('selected');
                    state.selectedIniciais = state.selectedIniciais.filter(i => i.id !== c.id);
                } else if (state.selectedIniciais.length < 3) {
                    card.classList.add('selected');
                    state.selectedIniciais.push({...c});
                }
                updateSelectionButton();
            };
            
            footer.appendChild(card);
        });

        const actions = document.getElementById('modal-actions');
        actions.innerHTML = '<button id="confirm-iniciais" disabled>Confirmar (0/3)</button>';
        document.getElementById('confirm-iniciais').onclick = () => {
            state.team = state.selectedIniciais.map(c => initCreature(c));
            hideModal();
            initGame(true);
        };
    }

    function updateSelectionButton() {
        const btn = document.getElementById('confirm-iniciais');
        btn.innerText = `Confirmar (${state.selectedIniciais.length}/3)`;
        btn.disabled = state.selectedIniciais.length < 1;
    }

    function initCreature(base) {
        const cloned = JSON.parse(JSON.stringify(base));
        
        // Converte supportSkill antigo para supportSkills (array)
        if (cloned.supportSkill && !cloned.supportSkills) {
            cloned.supportSkills = [cloned.supportSkill];
            delete cloned.supportSkill;
        } else if (!cloned.supportSkills) {
            cloned.supportSkills = [];
        }
        
        const c = {
            ...cloned,
            level: cloned.level || 1,
            xp: 0,
            maxXp: 100,
            currentHp: cloned.hp,
            isEvolved: false,
            items: cloned.items ? JSON.parse(JSON.stringify(cloned.items)) : [],
            cooldowns: {}
        };

        // Bônus Selvagens: +15% HP
        if (c.clan === 'selvagens') {
            c.hp = Math.floor(c.hp * 1.15);
            c.currentHp = c.hp;
        }

        return c;
    }

    function initGame(alreadySelected = false) {
        if (!alreadySelected) {
            startInitialSelection();
            return;
        }
        applyAccountSaveToState(state);
        state.level = 1;
        state.cardsRevealed = 0;
        state.isBossBattle = false;
        state.diamantes = 50;
        state.currentShopItems = null;
        state.inventory = [];
        restoreStandardBattleUI();
        updateUI();
        generateLevel();
        saveGame();
    }

    // --- PvP & TCG Modes ---

    function showPvPMenu() {
        playSound('click');
        const body = `
            <div style="text-align:center; color:white; margin-bottom:20px;">
                Escolha o modo de batalha PvP:
            </div>
        `;
        showModal("Modo PVP", body, [
            { text: "Duelo 1x1", action: () => startPvP(1, 1) },
            { text: "Batalha 2x3", action: () => startPvP(2, 3) },
            { text: "Guerra 3x3", action: () => startPvP(3, 3) },
            { text: "Voltar", action: hideModal }
        ]);
    }

    function startPvP(playerCount, enemyCount) {
        playSound('click');
        hideModal();
        const adventureState = JSON.parse(JSON.stringify(state));
        if (state.team.length === 0) {
            for (let i = 0; i < 6; i++) {
                const base = CREATURE_POOL[Math.floor(Math.random() * CREATURE_POOL.length)];
                state.team.push(initCreature(base));
            }
        } else {
            state.team.forEach(c => {
                c.currentHp = c.hp;
                c.defeated = false;
            });
        }
        const enemies = [];
        for (let i = 0; i < enemyCount; i++) {
            const enemy = getRandomEnemy();
            const avgLevel = state.team.reduce((sum, c) => sum + c.level, 0) / state.team.length;
            enemy.level = Math.floor(avgLevel);
            enemies.push(enemy);
        }
        const originalEndBattle = window.endBattle;
        window.endBattle = function(victory) {
            const currentOptions = state.options;
            state = adventureState;
            state.options = currentOptions;
            window.endBattle = originalEndBattle;
            document.getElementById('battle-screen').style.display = 'none';
            document.getElementById('main-menu').style.display = 'flex';
            document.getElementById('game-container').style.display = 'none';
            alert(victory ? "Vitória no PvP!" : "Derrota no PvP!");
            updateUI();
        };
        document.getElementById('main-menu').style.display = 'none';
        document.getElementById('game-container').style.display = 'block';
        startBattle(enemies, false); // startBattle handles multiple enemies
    }

    function exitToMainMenu() {
        playSound('click');
        // Esconde todas as interfaces de jogo
        document.getElementById('battle-screen').style.display = 'none';
        document.getElementById('game-container').style.display = 'none';
        document.getElementById('tcg-board').style.display = 'none';
        document.getElementById('exploration-grid').classList.add('hidden');
        
        // Mostra o menu principal
        document.getElementById('main-menu').style.display = 'flex';
        
        // Para a música de batalha se estiver tocando
        stopBattleMusic();
        playBackgroundMusic();
        
        console.log('Voltando ao menu principal');
        document.body.classList.remove('battle-mode');
    }

    function ensureExplorationGrid() {
        const grid = document.getElementById('exploration-grid');
        const battleScreen = document.getElementById('battle-screen');
        if (!grid || !battleScreen) return;
        if (getComputedStyle(battleScreen).display !== 'none') return;
        const mainMenu = document.getElementById('main-menu');
        const gameContainer = document.getElementById('game-container');
        if (mainMenu) mainMenu.style.display = 'none';
        if (gameContainer) gameContainer.style.display = 'block';
        if (!state.team || state.team.length === 0) return;
        if (state.level % 10 === 0) return;
        if (grid.children.length === 0) generateLevel();
    }

    function getGridCardCount() {
        if (state.level >= 20) return 9;
        if (state.level >= 10) return 6;
        return 3;
    }

    function grantFreePackForLevel() {
        if (typeof state.tcgFreePacks !== "number") state.tcgFreePacks = 0;
        if (state.level > 0 && state.level % 5 === 0) {
            state.tcgFreePacks += 1;
        }
    }

    function updateUI() {
        document.getElementById('current-level').innerText = state.level;
        document.getElementById('creature-count').innerText = state.team.length;
        document.getElementById('player-diamantes').innerText = state.diamantes;
        const luxSpan = document.getElementById('player-lux');
        if (luxSpan) luxSpan.innerText = state.lux || 0;
        const cardsRevealedSpan = document.getElementById('cards-revealed');
        const cardsTotalSpan = document.getElementById('cards-total');
        if (cardsRevealedSpan) cardsRevealedSpan.innerText = state.cardsRevealed || 0;
        if (cardsTotalSpan) cardsTotalSpan.innerText = getGridCardCount();
        
        // Atualiza diamante no modal se estiver visível
        const diamondVal = document.getElementById('modal-diamonds-val');
        if (diamondVal) diamondVal.innerText = state.diamantes;

        const footer = document.getElementById('team-footer');
        footer.innerHTML = '';
        state.team.forEach(c => {
            const div = document.createElement('div');
            div.className = 'team-member tooltip';
            const hpPercent = (c.currentHp / c.hp) * 100;
            const xpPercent = (c.xp / c.maxXp) * 100;
            const isDefeated = c.defeated || c.currentHp <= 0;
            
            div.innerHTML = `
                <div class="creature-card mini ${isDefeated ? 'defeated' : ''}">
                    ${getCreatureDisplay(c)}
                    <div class="creature-card-info">
                        <div class="creature-card-name">${c.name}${isDefeated ? ' 💀' : ''}</div>
                    </div>
                </div>
                <div class="mini-bars">
                    <div class="mini-bar"><div class="mini-bar-fill-hp" style="width: ${isDefeated ? '0' : hpPercent}%"></div></div>
                    <div class="mini-bar"><div class="mini-bar-fill-xp" style="width: ${xpPercent}%"></div></div>
                </div>
                <span class="tooltiptext">${getCreatureTooltip(c)}${isDefeated ? '\n💀 DERROTADO - Use "Reanimar" para reviver' : ''}</span>
            `;
            div.ondblclick = () => {
                showModal("Status da Criatura", getCreatureTooltip(c), [{ text: "Fechar", action: hideModal }]);
            };
            footer.appendChild(div);
        });

        if (state.team.length === 0 || state.team.every(c => c.currentHp <= 0 && c.defeated)) {
            gameOver();
        }
        ensureExplorationGrid();
        saveGame();
    }

    function generateLevel() {
        state.cardsRevealed = 0;
        saveGame();
        const mainMenu = document.getElementById('main-menu');
        const gameContainer = document.getElementById('game-container');
        if (mainMenu) mainMenu.style.display = 'none';
        if (gameContainer) gameContainer.style.display = 'block';
        const grid = document.getElementById('exploration-grid');
        grid.innerHTML = '';
        grid.classList.remove('hidden');
        grid.style.display = 'grid';
        grid.style.visibility = 'visible';
        grid.style.opacity = '1';
        document.getElementById('battle-screen').style.display = 'none';

        if (state.level % 10 === 0) {
            startBossBattle();
            return;
        }

        const cardCount = getGridCardCount();
        const rows = cardCount === 3 ? 1 : cardCount === 6 ? 2 : 3;
        grid.style.gridTemplateColumns = "repeat(3, 1fr)";
        grid.style.gridTemplateRows = `repeat(${rows}, auto)`;
        for (let i = 0; i < cardCount; i++) {
            const card = document.createElement('div');
            card.className = 'card pulse';
            card.innerHTML = `
                <div class="card-face card-back">
                    <img src="imagens/card_back.png" class="card-back-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 3rem; opacity: 0.3;">❓</div>
                </div>
                <div class="card-face card-front"></div>
            `;
            
            let type = pickExplorationCardType();
            
            card.dataset.type = type;
            card.onclick = () => openCard(card);
            grid.appendChild(card);
        }
        autoScaleGame();
    }

    function openCard(cardElement) {
        if (cardElement.classList.contains('revealed')) return;
        playSound('card_flip');
        cardElement.classList.remove('pulse');
        cardElement.classList.add('revealed');
        state.cardsRevealed++;
        
        updateUI();

        const type = cardElement.dataset.type;
        const front = cardElement.querySelector('.card-front');
        
        let eventName = "";
        let eventEmoji = "";
        let eventImg = "";
        let eventColor = "";

        if (type === 'COMBAT') {
            eventName = "COMBATE!";
            eventEmoji = "⚔️";
            eventImg = "card_battle.png";
            eventColor = "#ff4d4d";
        } else if (type === 'ENCOUNTER') {
            eventName = "ENCONTRO";
            eventEmoji = "👣";
            eventImg = "card_encounter.png";
            eventColor = "#4dabff";
        } else if (type === 'TREASURE') {
            eventName = "TESOURO";
            eventEmoji = "🍀";
            eventImg = "card_treasure.png";
            eventColor = "#4dff4d";
        } else if (type === 'TRAP') {
            // Configuração da Armadilha
            eventName = "ARMADILHA";
            eventEmoji = "🕸️";
            eventImg = "card_trap.png";
            eventColor = "#a600ff"; // Roxo
        } else if (type === 'ELITE') {
            eventName = "ELITE";
            eventEmoji = "🔥";
            eventImg = "card_elite.png";
            eventColor = "#ff7b00";
        } else if (type === 'CHAOS_EVENT') {
            eventName = "EVENTO CAÓTICO";
            eventEmoji = "🌪️";
            eventImg = "card_chaos.png";
            eventColor = "#c0c0c0";
        } else if (type === 'WANDERING_MERCHANT') {
            eventName = "MERCADOR ERRANTE";
            eventEmoji = "🧙";
            eventImg = "card_merchant.png";
            eventColor = "#ffd700";
        } else if (type === 'ANCIENT_ALTAR') {
            eventName = "ALTAR ANTIGO";
            eventEmoji = "🧬";
            eventImg = "card_altar.png";
            eventColor = "#8a2be2";
        } else if (type === 'AMBUSH') {
            eventName = "EMBOSCADA";
            eventEmoji = "🌑";
            eventImg = "card_ambush.png";
            eventColor = "#ff4d4d";
        } else if (type === 'LOST_RELIC') {
            eventName = "RELÍQUIA PERDIDA";
            eventEmoji = "💠";
            eventImg = "card_relic.png";
            eventColor = "#00ffff";
        } else if (type === 'CURSE') {
            eventName = "MALDIÇÃO";
            eventEmoji = "☠️";
            eventImg = "card_curse.png";
            eventColor = "#9932cc";
        } else if (type === 'FUTURE_VISION') {
            eventName = "VISÃO DO FUTURO";
            eventEmoji = "🧠";
            eventImg = "card_future.png";
            eventColor = "#66ccff";
        } else if (type === 'WILD_CALL') {
            eventName = "CHAMADO SELVAGEM";
            eventEmoji = "🐲";
            eventImg = "card_wild.png";
            eventColor = "#32cd32";
        } else if (type === 'CHALLENGE') {
            eventName = "DESAFIO";
            eventEmoji = "🏆";
            eventImg = "card_challenge.png";
            eventColor = "#ffa500";
        }

        front.innerHTML = `
            <img src="imagens/${eventImg}" class="card-event-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
            <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 4rem; opacity: 0.3;">${eventEmoji}</div>
            <div class="card-event-info" style="color: ${eventColor};">
                ${eventEmoji}<br>${eventName}
            </div>
        `;
        cardElement.style.borderColor = eventColor;
        cardElement.style.boxShadow = `0 0 22px ${eventColor}`;
        
        setTimeout(() => {
            if (type === 'COMBAT') {
                const enemyCount = state.level > 10 ? Math.min(3, 1 + Math.floor(Math.random() * 3)) : 1;
                const enemies = [];
                for (let i = 0; i < enemyCount; i++) enemies.push(getRandomEnemy());
                setTimeout(() => startBattle(enemies), 600);
            } else if (type === 'ENCOUNTER') {
                setTimeout(() => startEncounter(getRandomEnemy()), 600);
            } else if (type === 'TREASURE') {
                setTimeout(() => handleTreasure(), 600);
            } else if (type === 'TRAP') {
                // Aciona a armadilha
                setTimeout(() => handleTrap(), 600);
            } else if (type === 'ELITE') {
                setTimeout(() => startEliteBattle(), 600);
            } else if (type === 'CHAOS_EVENT') {
                setTimeout(() => handleChaosEvent(), 600);
            } else if (type === 'WANDERING_MERCHANT') {
                setTimeout(() => showWanderingMerchant(), 600);
            } else if (type === 'ANCIENT_ALTAR') {
                setTimeout(() => showAncientAltar(), 600);
            } else if (type === 'AMBUSH') {
                setTimeout(() => startAmbush(), 600);
            } else if (type === 'LOST_RELIC') {
                setTimeout(() => handleLostRelic(), 600);
            } else if (type === 'CURSE') {
                setTimeout(() => applyCurse(), 600);
            } else if (type === 'FUTURE_VISION') {
                setTimeout(() => showFutureVision(), 600);
            } else if (type === 'WILD_CALL') {
                setTimeout(() => startWildCall(), 600);
            } else if (type === 'CHALLENGE') {
                setTimeout(() => showChallengeCard(), 600);
            }
        }, 300);
    }

    function handleTrap() {
        const body = document.body;
        body.classList.add('anim-trap-shake');
        playSound('defeat');

        setTimeout(() => {
            body.classList.remove('anim-trap-shake');

            const roll = Math.random();

            if (roll < 0.2) {
                let anyAlive = false;
                state.team.forEach(c => {
                    if (c.currentHp > 0) {
                        anyAlive = true;
                        c.trapPoisonTurns = 4;
                        c.trapPoisonDmg = 40;
                    }
                });

                if (!anyAlive) {
                    gameOver();
                    return;
                }

                updateUI();
                showModal("ARMADILHA DE SANGUE 🕸️", 
                    "Sua equipe foi marcada por um efeito de -40 HP/turno por 4 turnos.", 
                    [{ text: "Continuar", action: endEvent }]
                );

            } else if (roll < 0.4) {
                let anyAlive = false;
                state.team.forEach(c => {
                    if (c.currentHp > 0) {
                        anyAlive = true;
                        c.trapPoisonTurns = 4;
                        c.trapPoisonDmg = 60;
                    }
                });

                if (!anyAlive) {
                    gameOver();
                    return;
                }

                updateUI();
                showModal("VENENO FORTE 🕸️", 
                    "Um veneno forte atingiu sua equipe! Perdem 60 de HP por turno por 4 turnos.", 
                    [{ text: "Continuar", action: endEvent }]
                );

            } else if (roll < 0.7) {
                let anyAlive = false;
                state.team.forEach(c => {
                    if (c.currentHp > 0) {
                        anyAlive = true;
                        c.trapParalysisTurns = 3;
                    }
                });

                if (!anyAlive) {
                    gameOver();
                    return;
                }

                updateUI();
                showModal("ARMADILHA PARALISANTE 🕸️", 
                    "Uma armadilha paralisante afetou sua equipe! Criaturas podem ficar paralisadas por 3 turnos.", 
                    [{ text: "Continuar", action: endEvent }]
                );

            } else if (roll < 0.95) {
                if (state.team.length === 0) return endEvent();

                showModal("ARMADILHA MORTAL! 🕸️", 
                    "Você caiu em um poço de espinhos! Para o resto do time sobreviver, você deve deixar uma criatura para trás.", 
                    []);

                const f = document.getElementById('modal-footer');
                f.innerHTML = '';
                
                state.team.forEach(c => {
                    const d = document.createElement('div');
                    d.className = 'creature-card tooltip';
                    d.innerHTML = `
                        ${getCreatureDisplay(c)}
                        <div class="creature-card-info" style="background: linear-gradient(transparent, #500);">
                            <div class="creature-card-name">${c.name}</div>
                            <div class="creature-card-stats" style="color:red;">SACRIFICAR</div>
                        </div>
                    `;
                    d.onclick = () => {
                        state.team.splice(state.team.indexOf(c), 1);
                        
                        if (c.items && c.items.length > 0) {
                            c.items.forEach(it => {
                                removeImmediateItemEffects(c, it);
                                addItemToInventory(it);
                            });
                            addLog(`Os itens de ${c.name} foram recuperados.`);
                        }

                        addLog(`💀 ${c.name} foi sacrificado na armadilha.`);
                        updateUI();
                        
                        if (state.team.length === 0) {
                            gameOver();
                        } else {
                            endEvent();
                        }
                    };
                    f.appendChild(d);
                });

            } else {
                let logMsg = "";
                state.team.forEach(c => {
                    const oldLevel = c.level;
                    const newLevel = Math.max(1, Math.floor(oldLevel / 2));
                    const levelsLost = oldLevel - newLevel;

                    if (levelsLost > 0) {
                        c.level = newLevel;
                        
                        c.hp = Math.max(10, c.hp - (levelsLost * 20));
                        c.currentHp = Math.min(c.currentHp, c.hp);
                        c.attacks.forEach(a => {
                            a.min = Math.max(1, a.min - (levelsLost * 2));
                            a.max = Math.max(2, a.max - (levelsLost * 4));
                        });

                        if (c.isEvolved && c.evo) {
                            const baseCreature = CREATURE_POOL.find(poolItem => poolItem.id === c.id);
                            
                            if (baseCreature) {
                                c.hp = Math.max(10, c.hp - c.evo.hpBoost);
                                c.currentHp = Math.min(c.currentHp, c.hp);
                                c.attacks.forEach(a => {
                                    a.min = Math.max(1, a.min - c.evo.dmgBoost);
                                    a.max = Math.max(2, a.max - c.evo.dmgBoost);
                                });

                                c.name = baseCreature.name;
                                c.emoji = baseCreature.emoji;
                                c.image = baseCreature.image;
                                c.isEvolved = false;
                                
                                logMsg += `<br>${c.evo.name} regrediu para ${c.name}!`;
                            }
                        }
                    }
                });

                updateUI();
                showModal("DRENAGEM DE ENERGIA! ⚡", 
                    `Uma maldição antiga atingiu sua equipe!<br><br>
                     <strong>Todos perderam metade do nível.</strong>
                     ${logMsg ? '<br><br><span style="color:orange;">Criaturas evoluídas voltaram à forma original!</span>' : ''}`, 
                    [{ text: "Que desastre...", action: endEvent }]
                );
            }
        }, 600);
    }

    function getRandomEnemy() {
        const base = CREATURE_POOL[Math.floor(Math.random() * CREATURE_POOL.length)];
        let enemy = initCreature(base);
        
        // Escala nível do inimigo com base no nível do jogador
        const levelBoost = Math.floor(state.level / 2);
        enemy.level += levelBoost;
        enemy.hp += levelBoost * 20;
        enemy.attacks.forEach(a => { a.min += levelBoost * 2; a.max += levelBoost * 3; });

        // A partir do nível 10, inimigos podem vir evoluídos ou com itens
        if (state.level > 10) {
            // Chance de vir evoluído (se tiver evolução)
            if (enemy.evo && Math.random() < 0.4) {
                const oldName = enemy.name;
                enemy.name = enemy.evo.name;
                enemy.emoji = enemy.evo.emoji;
                if (enemy.evo.image) enemy.image = enemy.evo.image;
                enemy.hp += enemy.evo.hpBoost;
                enemy.attacks.forEach(a => { a.min += enemy.evo.dmgBoost; a.max += enemy.evo.dmgBoost; });
                enemy.isEvolved = true;
            }

            // Chance de vir com equipamento (1 a 2 itens)
            if (Math.random() < 0.5) {
                const availableItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
                const itemCount = Math.random() < 0.8 ? 1 : 2;
                for (let i = 0; i < itemCount; i++) {
                    const randomItem = JSON.parse(JSON.stringify(availableItems[Math.floor(Math.random() * availableItems.length)]));
                    randomItem.level = Math.max(1, Math.floor(state.level / 15)); // Itens podem vir com nível maior
                    enemy.items.push(randomItem);
                    
                    // Aplica efeitos imediatos se necessário (ex: Casco de Adamante)
                    if (randomItem.id === 'casco_adamante') enemy.hp += 100;
                }
            }
        }

        // Buff global: inimigos têm 30% a mais de HP e ataque que as criaturas base
        const enemyStatMultiplier = 1.3;
        enemy.hp = Math.floor(enemy.hp * enemyStatMultiplier);
        enemy.attacks.forEach(a => {
            a.min = Math.max(1, Math.floor(a.min * enemyStatMultiplier));
            a.max = Math.max(a.min, Math.floor(a.max * enemyStatMultiplier));
        });

        enemy.currentHp = enemy.hp;
        return enemy;
    }

    // --- Battle & XP Logic ---

    // --- Lógica de Morte e Loot ---

    function handlePlayerDeath(creature) {
        if (!creature.items || creature.items.length === 0) return;

        // Move todos os itens para o inventário
        creature.items.forEach(item => {
            // Remove efeitos passivos (ex: HP extra do Casco) para não bugar os status base
            removeImmediateItemEffects(creature, item);
            
            // Adiciona ao inventário
            addItemToInventory(item);
        });

        addLog(`🎒 Os itens de ${creature.name} foram salvos no inventário!`);
        creature.items = []; // Limpa os itens da criatura morta
    }

    function handleEnemyDeath(enemy) {
        if (!enemy.items || enemy.items.length === 0) return;

        // Inicializa o array de loot da batalha se não existir
        if (!state.battleLoot) state.battleLoot = [];

        // Adiciona os itens do inimigo à pilha de loot
        enemy.items.forEach(item => {
            // Cria uma cópia nova para garantir que é um item "dropável" limpo
            const lootItem = createItemInstance(item, item.level || 1);
            state.battleLoot.push(lootItem);
        });

        addLog(`💰 ${enemy.name} derrubou ${enemy.items.length} item(ns)!`);
        enemy.items = []; // Remove os itens do inimigo
    }

    // Função recursiva para mostrar o loot um por um ao final da batalha
    function processBattleLoot(lootQueue, index, onAllDone) {
        if (!lootQueue || index >= lootQueue.length) {
            onAllDone();
            return;
        }

        const item = lootQueue[index];
        const isLast = index === lootQueue.length - 1;

        showModal("LOOT DE BATALHA", `
            <div style="text-align:center;">
                <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                <div style="font-weight:900; color:gold;">${item.name}</div>
                <div style="font-size:0.85rem; color:#ddd; margin-top:6px;">${item.desc}</div>
                <div style="margin-top:10px; font-size:0.75rem; color:#aaa;">Item ${index + 1} de ${lootQueue.length}</div>
            </div>
        `, [
            { text: "Equipar", action: () => {
                showCreatureSelection((target) => {
                    if (!target.items) target.items = [];
                    const maxSlots = getMaxItemSlotsForLevel(target.level);
                    
                    if (target.items.length >= maxSlots) {
                        // Lógica de substituição simplificada para loot
                        showModal("Espaços Cheios", "Inventário da criatura cheio. Guardar no inventário geral?", [
                            { text: "Sim, Guardar", action: () => {
                                addItemToInventory(item);
                                addLog(`Item guardado.`);
                                processBattleLoot(lootQueue, index + 1, onAllDone);
                            }},
                            { text: "Descartar Novo", action: () => processBattleLoot(lootQueue, index + 1, onAllDone) }
                        ]);
                    } else {
                        target.items.push(item);
                        applyImmediateItemEffects(target, item); // Aplica efeitos passivos ao equipar
                        addLog(`${target.name} equipou ${item.name}!`);
                        processBattleLoot(lootQueue, index + 1, onAllDone);
                    }
                }, "Quem vai equipar?");
            }},
            { text: "Guardar na Mochila", action: () => {
                addItemToInventory(item);
                addLog(`${item.name} guardado.`);
                processBattleLoot(lootQueue, index + 1, onAllDone);
            }},
            { text: "Ignorar (Lixo)", action: () => {
                addLog(`${item.name} foi deixado para trás.`);
                processBattleLoot(lootQueue, index + 1, onAllDone);
            }}
        ]);
    }

    function startBattle(enemyOrEnemies, isBoss = false) {
        state.enemies = Array.isArray(enemyOrEnemies) ? enemyOrEnemies : [enemyOrEnemies];
        state.isBossBattle = isBoss;
        if (state.curse && state.curse.hpPenalty) {
            state.team.forEach(c => {
                if (!c.battleState) c.battleState = {};
                if (!c.battleState.hpOriginal && c.currentHp > 0) {
                    c.battleState.hpOriginal = c.hp;
                    const limit = Math.max(10, Math.floor(c.hp * (1 - state.curse.hpPenalty)));
                    c.hp = limit;
                    c.currentHp = Math.min(c.currentHp, c.hp);
                }
            });
        }
        state.swapsUsed = 0;
        state.battleLoot = [];
        playSound(isBoss ? 'boss_battle' : 'battle_start');
        
        state.activePlayerCreatures = [];

        if (isBoss) {
            const alive = state.team.filter(c => c.currentHp > 0);
            const countToPick = Math.min(3, alive.length);
            let selected = [];

            function pickNext() {
                if (selected.length < countToPick) {
                    const selectedIds = selected.map(s => s.id);
                    showCreatureSelection((c) => {
                        selected.push(c);
                        pickNext();
                    }, `Escolha a criatura ${selected.length + 1} de ${countToPick} para o Boss:`, true, selectedIds);
                } else {
                    finishStartBattle(selected);
                }
            }
            pickNext();
        } else {
            showCreatureSelection((selected) => {
                finishStartBattle([selected]);
            }, "Escolha quem vai lutar:", true);
        }

        function finishStartBattle(creatures) {
            state.activePlayerCreatures = creatures;
            state.activePlayerCreature = creatures[0]; // Mantém para compatibilidade legado

            restoreStandardBattleUI();

            creatures.forEach(selected => {
                selected.cooldowns = selected.cooldowns || {};
                selected.battleState = {
                    consecutiveTurns: 0,
                    defenseBuff: 0,
                    tonicoTurns: selected.items && selected.items.some(i => i.id === 'tonico_guerra') ? 3 : 0,
                    targetLock: { index: -1, turns: 0 },
                    turnCount: 0,
                    hasActed: false
                };
            });

            state.enemies.forEach(e => {
                e.battleState = {
                    damageReduction: 0,
                    takenDamageIncrease: 0,
                    missChance: 0,
                    bleedTurns: 0,
                    turnCount: 0
                };
            });
            
            document.getElementById('exploration-grid').classList.add('hidden');
            document.getElementById('battle-screen').style.display = 'flex';
            document.body.classList.add('battle-mode');
            setupBattleUI();
            
            const playerNames = creatures.map(p => p.name).join(', ');
            addLog(`Batalha: ${playerNames} vs ${state.enemies.map(e => e.name).join(', ')}!`);
            
            let enemyFirst = false;
            if (state.ambushActive || state.challengeEnemiesFast) enemyFirst = true;
            if (!enemyFirst && Math.random() < 0.5) {
                addLog("O inimigo foi mais rápido e ataca primeiro!");
                updateTurnIndicator(false);
                toggleButtons(true);
                setTimeout(enemyTurn, 1000);
            } else {
                addLog("Você foi mais rápido e começa atacando!");
                updateTurnIndicator(true);
                toggleButtons(false);
            }
        }
    }

    function updateTurnIndicator(isPlayerTurn) {
        const indicator = document.getElementById('turn-indicator');
        if (indicator) {
            indicator.innerText = isPlayerTurn ? "▶ Seu Turno" : "▶ Turno do Inimigo";
            indicator.style.color = isPlayerTurn ? "#ffd700" : "#ff4d4d";
        }
    }

    function restoreStandardBattleUI() {
        const header = document.querySelector('header');
        if (header) header.style.display = '';
        const statsBar = document.getElementById('game-stats-bar');
        if (statsBar) statsBar.style.display = '';
        const standardArena = document.getElementById('standard-battle-arena');
        if (standardArena) standardArena.style.display = 'flex';
        const battleLogs = document.getElementById('battle-logs');
        if (battleLogs) battleLogs.style.display = '';
        const battleActions = document.getElementById('battle-actions');
        if (battleActions) battleActions.style.display = '';
        const supportContainer = document.getElementById('support-container');
        if (supportContainer) supportContainer.style.display = '';
        const turnIndicator = document.getElementById('turn-indicator');
        if (turnIndicator) turnIndicator.style.display = '';
        const tcgBoard = document.getElementById('tcg-board');
        if (tcgBoard) tcgBoard.style.display = 'none';
        
        autoScaleGame();
    }

    function processTurnEnd(side) {
        if (side === 'player') {
            state.activePlayerCreatures.forEach((p, pIdx) => {
            if (!p || p.currentHp <= 0) return;

            // Garante que o battleState existe
            if (!p.battleState) {
                p.battleState = {
                    consecutiveTurns: 0,
                    defenseBuff: 0,
                    tonicoTurns: (p.items && p.items.some(i => i.id === 'tonico_guerra')) ? 3 : 0,
                    targetLock: { index: -1, turns: 0 },
                    turnCount: 0
                };
            }

            p.battleState.turnCount++;
            p.battleState.consecutiveTurns++;

            // --- NOVOS BUFFS DE SUPORTE (OFFENSIVE/END-TURN) ---
            if (p.battleState.regen && p.battleState.regen.turns > 0) {
                const heal = Math.floor(p.hp * p.battleState.regen.value);
                p.currentHp = Math.min(p.hp, p.currentHp + heal);
                showFloatingValue(`player-fighter-${pIdx}`, heal, 'heal');
                addLog(`🌿 REGENERAÇÃO: ${p.name} recuperou ${heal} HP.`);
                updateHpUI(`player-${pIdx}`, p);
                p.battleState.regen.turns--;
                if (p.battleState.regen.turns <= 0) p.battleState.regen = null;
            }

            if (p.battleState.multiHit && p.battleState.multiHit.turns > 0) {
                p.battleState.multiHit.turns--;
                if (p.battleState.multiHit.turns <= 0) {
                    p.battleState.multiHit = null;
                    addLog(`⚔️ GOLPE DUPLO: O efeito em ${p.name} acabou.`);
                }
            }

            if (p.battleState.critBuff && p.battleState.critBuff.turns > 0) {
                p.battleState.critBuff.turns--;
                if (p.battleState.critBuff.turns <= 0) {
                    p.battleState.critBuff = null;
                    addLog(`🎯 FOCO: A concentração de ${p.name} voltou ao normal.`);
                }
            }

            if (p.battleState.invulnerability && p.battleState.invulnerability.turns > 0) {
                p.battleState.invulnerability.turns--;
                if (p.battleState.invulnerability.turns <= 0) {
                    p.battleState.invulnerability = null;
                    addLog(`🛡️ ÉGIDE: A invulnerabilidade de ${p.name} terminou.`);
                }
            }

            if (p.battleState.lifesteal && p.battleState.lifesteal.turns > 0) {
                p.battleState.lifesteal.turns--;
                if (p.battleState.lifesteal.turns <= 0) {
                    p.battleState.lifesteal = null;
                    addLog(`🧛 VAMPIRO: O efeito de roubo de vida acabou.`);
                }
            }

            if (p.battleState.reflect && p.battleState.reflect.turns > 0) {
                p.battleState.reflect.turns--;
                if (p.battleState.reflect.turns <= 0) {
                    p.battleState.reflect = null;
                    addLog(`🌵 ESPINHOS: A armadura de espinhos se desfez.`);
                }
            }
            // -----------------------------
            
            if (p.items && p.items.length > 0) {
                p.items.forEach(item => {
                    const lvl = item.level || 1;
                    
                    // 2. Coração Ancestral
                    if (item.id === 'coracao_ancestral') {
                        const heal = Math.floor(p.hp * 0.04);
                        p.currentHp = Math.min(p.hp, p.currentHp + heal);
                        showFloatingValue(`player-fighter-${pIdx}`, heal, 'heal');
                        updateHpUI(`player-${pIdx}`, p);
                    }

                    // 3. Manto da Persistência
                    if (item.id === 'manto_persistencia') {
                        const maxDef = 0.25 + (lvl - 1) * 0.05;
                        p.battleState.defenseBuff = Math.min(maxDef, (p.battleState.defenseBuff || 0) + 0.05);
                    }

                    // 5. Tônico de Guerra
                    if (item.id === 'tonico_guerra') {
                        if (p.battleState.tonicoTurns > 0) {
                            p.battleState.tonicoTurns--;
                            if (p.battleState.tonicoTurns === 0) {
                                p.battleState.tonicoTurns = -1; // Exaustão
                                addLog(`🧪 TÔNICO: O efeito acabou! ${p.name} está exausto.`);
                            }
                        } else if (p.battleState.tonicoTurns === -1) {
                            p.battleState.tonicoTurns = 0; // Fim da exaustão e quebra
                            p.items = p.items.filter(i => i.id !== 'tonico_guerra');
                            addLog(`🧪 TÔNICO: O frasco se quebrou.`);
                        }
                    }

                    // 11. Lâmina Instável
                    if (item.id === 'lamina_instavel' && Math.random() < 0.1) {
                        const selfDmg = Math.floor(p.hp * 0.05);
                        p.currentHp -= selfDmg;
                        showFloatingValue(`player-fighter-${pIdx}`, selfDmg, 'damage');
                        addLog(`🔪 LÂMINA: ${p.name} se feriu com a lâmina instável!`);
                        updateHpUI(`player-${pIdx}`, p);
                    }

                    // 12. Relógio da Entropia
                    if (item.id === 'relogio_entropia') {
                        if (p.battleState.turnCount % 2 === 1) {
                            addLog(`⏳ RELÓGIO: Turno ímpar! +15% velocidade.`);
                        } else {
                            addLog(`⏳ RELÓGIO: Turno par! -15% defesa.`);
                        }
                    }

                    // 13. Máscara do Sacrifício
                    if (item.id === 'mascara_sacrificio') {
                        let buffs = 0;
                        if (p.battleState.consecutiveTurns > 1) buffs++;
                        if (p.battleState.defenseBuff > 0) buffs++;
                        if (p.battleState.tonicoTurns > 0) buffs++;
                        if (buffs > 0) {
                            const penalty = Math.floor(p.hp * (buffs * 0.03));
                            p.currentHp -= penalty;
                            showFloatingValue(`player-fighter-${pIdx}`, penalty, 'damage');
                            updateHpUI(`player-${pIdx}`, p);
                        }
                    }
                });

                // Verifica morte do jogador por efeitos de itens
                if (p.currentHp <= 0) handlePlayerDeath(p);
            }

            if (p.trapPoisonTurns && p.trapPoisonTurns > 0 && p.currentHp > 0) {
                const poisonDmg = p.trapPoisonDmg || 40;
                p.currentHp -= poisonDmg;
                showFloatingValue(`player-fighter-${pIdx}`, poisonDmg, 'damage');
                addLog(`☠️ VENENO: ${p.name} sofreu ${poisonDmg} de dano da armadilha.`);
                p.trapPoisonTurns--;
                updateHpUI(`player-${pIdx}`, p);
                if (p.currentHp <= 0) handlePlayerDeath(p);
            }
        });
    }

    if (side === 'enemy') {
        // Aplica debuffs nos inimigos
        state.enemies.forEach((e, idx) => {
            if (e.currentHp <= 0) return;

            // Garante que o battleState existe no inimigo
            if (!e.battleState) {
                e.battleState = {
                    damageReduction: 0,
                    takenDamageIncrease: 0,
                    missChance: 0,
                    bleedTurns: 0,
                    turnCount: 0
                };
            }
            
            e.battleState.turnCount++;
            
            // Verifica itens de TODOS os jogadores ativos para aplicar efeitos no inimigo
            state.activePlayerCreatures.forEach((p, pIdx) => {
                if (p.currentHp <= 0) return;
                
                if (p.items && p.items.length > 0) {
                    p.items.forEach(item => {
                        // 6. Correntes do Esquecimento
                        if (item.id === 'correntes_esquecimento') {
                            e.battleState.damageReduction = Math.min(0.20, (e.battleState.damageReduction || 0) + 0.05);
                        }
                        // 7. Marca do Abismo
                        if (item.id === 'marca_abismo') {
                            e.battleState.takenDamageIncrease = (e.battleState.takenDamageIncrease || 0) + 0.06;
                            if (e.currentHp < e.hp * 0.3) {
                                e.currentHp = 0;
                                showFloatingValue(`enemy-fighter-${idx}`, "ELIMINADO", 'damage');
                                addLog(`🔴 MARCA: A marca do abismo explodiu e eliminou ${e.name}!`);
                                
                                const enemyEl = document.getElementById(`enemy-fighter-${idx}`);
                                const enemyCard = enemyEl ? enemyEl.querySelector('.creature-card') : null;
                                if (enemyCard) enemyCard.classList.add('anim-death');
                                else if (enemyEl) enemyEl.style.opacity = '0.3';
                                
                                handleEnemyDeath(e); // Morte por marca

                                updateHpUI(`enemy-${idx}`, e);
                            }
                        }
                        // 8. Névoa Entorpecente
                        if (item.id === 'nevoa_entorpecente') {
                            e.battleState.missChance = (e.battleState.missChance || 0) + 0.10;
                            if (e.battleState.missChance >= 0.60) {
                                addLog(`🌫️ NÉVOA: ${e.name} está totalmente desorientado pela névoa!`);
                            }
                        }
                        // 9. Sangramento
                        if (item.id === 'sangramento' && e.battleState.bleedTurns > 0) {
                            const bleedDmg = Math.floor(e.hp * 0.03);
                            e.currentHp -= bleedDmg;
                            const heal = Math.floor(bleedDmg / 2);
                            p.currentHp = Math.min(p.hp, p.currentHp + heal);
                            
                            showFloatingValue(`enemy-fighter-${idx}`, bleedDmg, 'damage');
                            showFloatingValue(`player-fighter-${pIdx}`, heal, 'heal');
                            addLog(`🩸 SANGRAMENTO: ${e.name} perdeu ${bleedDmg} HP e curou ${p.name} em ${heal}!`);
                            
                            e.battleState.bleedTurns--;
                            if (e.battleState.bleedTurns === 0) {
                                addLog(`🩸 SANGRAMENTO: O sangramento de ${e.name} parou.`);
                            }
                            
                            if (e.currentHp <= 0) handleEnemyDeath(e); // Morte por sangramento

                            updateHpUI(`enemy-${idx}`, e);
                            updateHpUI(`player-${pIdx}`, p);
                        }
                        // 10. Selo da Ruptura
                        if (item.id === 'selo_ruptura' && e.items && e.items.length > 0) {
                            const randomItem = e.items[Math.floor(Math.random() * e.items.length)];
                            randomItem.level = (randomItem.level || 1) - 1;
                            if (randomItem.level <= 0) {
                                e.items = e.items.filter(i => i !== randomItem);
                                addLog(`🔱 SELO: O item ${randomItem.name} de ${e.name} se quebrou!`);
                            } else {
                                addLog(`🔱 SELO: Reduziu nível de ${randomItem.name} de ${e.name}.`);
                            }
                        }
                    });
                }
            });

            // --- DECREMENTA DEBUFFS DE SUPORTE (INIMIGO) ---
            if (e.battleState.maxDmgDebuff && e.battleState.maxDmgDebuff.turns > 0) {
                e.battleState.maxDmgDebuff.turns--;
                if (e.battleState.maxDmgDebuff.turns === 0) {
                    addLog(`✨ TOXINA: O efeito em ${e.name} acabou.`);
                }
            }
            // -----------------------------------------------

            // --- REDUZ COOLDOWNS DO INIMIGO ---
            if (e.cooldowns) {
                for (let atkName in e.cooldowns) {
                    if (e.cooldowns[atkName] > 0) e.cooldowns[atkName]--;
                }
            }
            // ----------------------------------
        });

        // --- REDUZ COOLDOWNS DA RESERVA ---
        state.team.forEach(c => {
            if (!state.activePlayerCreatures.includes(c) && c.cooldowns) {
                for (let skillName in c.cooldowns) {
                    if (c.cooldowns[skillName] > 0) c.cooldowns[skillName]--;
                }
            }
        });

        // --- PROCESSAMENTO DE FIM DE TURNO PARA JOGADORES (Reset e Cooldowns) ---
        state.activePlayerCreatures.forEach((p, idx) => {
            if (p.currentHp <= 0) return;

            // Reseta hasActed para o próximo turno do jogador
            if (p.battleState) p.battleState.hasActed = false;

            // Reduz cooldowns do jogador
            if (p.cooldowns) {
                for (let atkName in p.cooldowns) {
                    if (p.cooldowns[atkName] > 0) p.cooldowns[atkName]--;
                }
            }

            // Bônus Sagrados: Cura 10% ao final do turno global
            if (p.clan === 'sagrados') {
                const heal = Math.floor(p.hp * 0.1);
                p.currentHp = Math.min(p.hp, p.currentHp + heal);
                showFloatingValue(`player-fighter-${idx}`, heal, 'heal');
                addLog(`✨ SAGRADOS: ${p.name} regenerou ${heal} HP!`);
                updateHpUI(`player-${idx}`, p);
            }

            // Reseta buffs temporários de suporte (DEFENSIVE/ROUND-END)
            if (p.battleState) {
                // Decrementa ou reseta Redução de Dano
                if (p.battleState.damageReductionBuff && typeof p.battleState.damageReductionBuff === 'object') {
                    p.battleState.damageReductionBuff.turns--;
                    if (p.battleState.damageReductionBuff.turns <= 0) p.battleState.damageReductionBuff = 0;
                } else {
                    p.battleState.damageReductionBuff = 0;
                }

                // Decrementa ou reseta Esquiva
                if (p.battleState.dodgeBuff && typeof p.battleState.dodgeBuff === 'object') {
                    p.battleState.dodgeBuff.turns--;
                    if (p.battleState.dodgeBuff.turns <= 0) p.battleState.dodgeBuff = 0;
                } else {
                    p.battleState.dodgeBuff = 0;
                }
                
                // Decrementa Taunt
                if (p.battleState.taunt && p.battleState.taunt.turns > 0) {
                    p.battleState.taunt.turns--;
                    if (p.battleState.taunt.turns === 0) p.battleState.taunt = null;
                }

                // Decrementa Escudo
                if (p.battleState.shield && p.battleState.shield.turns > 0) {
                    p.battleState.shield.turns--;
                    if (p.battleState.shield.turns === 0) {
                        p.battleState.shield = null;
                        addLog(`🛡️ O escudo de ${p.name} expirou.`);
                    }
                }
            }
        });
    }
}

    function passPlayerTurn() {
        if (!state.activePlayerCreatures || state.activePlayerCreatures.length === 0) return;
        state.activePlayerCreatures.forEach(p => {
            if (!p || p.currentHp <= 0) return;
            if (!p.battleState) {
                p.battleState = {
                    consecutiveTurns: 0,
                    defenseBuff: 0,
                    tonicoTurns: (p.items && p.items.some(i => i.id === 'tonico_guerra')) ? 3 : 0,
                    targetLock: { index: -1, turns: 0 },
                    turnCount: 0,
                    hasActed: false
                };
            }
            p.battleState.hasActed = true;
        });

        processTurnEnd('player');

        const alivePlayers = state.activePlayerCreatures.filter(c => c.currentHp > 0);
        alivePlayers.forEach((player, idx) => {
            if (player.currentHp <= 0) {
                addLog(`${player.name} sucumbiu aos efeitos colaterais.`);
                handlePlayerDeath(player);
                const pCard = document.querySelector(`#player-fighter-${idx} .creature-card`);
                if (pCard) pCard.classList.add('anim-death');
            }
        });

        const teamDefeated = state.activePlayerCreatures.every(c => c.currentHp <= 0);
        if (teamDefeated) {
            toggleButtons(true);
            setTimeout(loseBattle, 1000);
            return;
        }

        toggleButtons(true);
        const allEnemiesDead = state.enemies.every(e => e.currentHp <= 0);
        if (allEnemiesDead) {
            setTimeout(winBattle, 1000);
        } else {
            updateTurnIndicator(false);
            setTimeout(enemyTurn, 1000);
        }
    }


    function queuePlayerAttackFromUI(atk, playerIndex) {
        if (!state.activePlayerCreatures || state.activePlayerCreatures.length === 0) return;
        const p = state.activePlayerCreatures[playerIndex];
        if (!p) return;

        const cd = p.cooldowns[atk.name] || 0;
        const hasActed = state.isBossBattle && p.battleState && p.battleState.hasActed;
        const isParalyzed = p.trapParalysisTurns > 0;

        if (cd > 0) {
            addLog(`⏰ ${atk.name} está em recarga (${cd} turno${cd > 1 ? 's' : ''} restantes)!`);
            return;
        }
        if (hasActed) {
            addLog(`⏳ ${p.name} já agiu neste turno!`);
            return;
        }
        if (isParalyzed) {
            addLog(`🌀 ${p.name} está paralisado e não pode atacar!`);
            return;
        }

        if (state.enemies.length === 1) {
            playerAttack(atk, 0, playerIndex);
        } else {
            addLog(`Escolha o alvo para ${p.name}!`);
            const aliveEnemies = state.enemies.map((e, i) => e.currentHp > 0 ? i : -1).filter(i => i !== -1);
            aliveEnemies.forEach(i => {
                const enemyEl = document.getElementById(`enemy-fighter-${i}`);
                if (!enemyEl) return;
                enemyEl.style.boxShadow = "0 0 15px red";
                enemyEl.onclick = () => {
                    aliveEnemies.forEach(idx => {
                        const el = document.getElementById(`enemy-fighter-${idx}`);
                        if (el) {
                            el.style.boxShadow = "";
                            el.onclick = null;
                        }
                    });
                    playerAttack(atk, i, playerIndex);
                };
            });
        }
    }

    function openAttackSelectionForPlayer(playerIndex) {
        if (!state.activePlayerCreatures || state.activePlayerCreatures.length === 0) return;
        const p = state.activePlayerCreatures[playerIndex];
        if (!p || p.currentHp <= 0) return;

        const body = `
            <div style="text-align:center; margin-bottom:8px; border-bottom: 1px solid rgba(255,215,0,0.3); padding-bottom: 10px;">
                <div style="font-size: 0.85rem; text-align: left;">${getCreatureTooltip(p)}</div>
                <div style="font-weight:600; color:#ffd700; margin-top: 10px;">Escolha uma ação:</div>
            </div>
        `;

        const buttons = [];

        p.attacks.forEach(atk => {
            const cd = p.cooldowns[atk.name] || 0;
            const label = cd > 0 ? `${atk.name} (${cd})` : atk.name;
            buttons.push({
                text: label,
                action: () => {
                    hideModal();
                    queuePlayerAttackFromUI(atk, playerIndex);
                }
            });
        });

        if (!state.isBossBattle && playerIndex === 0 && state.swapsUsed < 2) {
            const remaining = 2 - state.swapsUsed;
            buttons.push({
                text: `Trocar (${remaining})`,
                action: () => {
                    hideModal();
                    const activeIds = state.activePlayerCreatures.map(ac => ac.id);
                    const p = state.activePlayerCreatures[0];
                    showCreatureSelection((newCreature) => {
                        p.restCount = 2;
                        state.swapsUsed++;
                        state.activePlayerCreatures[0] = newCreature;
                        state.activePlayerCreature = newCreature;
                        setupBattleUI();
                        addLog(`${newCreature.name} entrou em campo!`);
                    }, "Escolha quem entra no lugar:", true, activeIds);
                }
            });
        }

        {
            const skills = p.supportSkills || (p.supportSkill ? [p.supportSkill] : []);
            skills.forEach((skill, sIdx) => {
                const cd = p.cooldowns[skill.name] || 0;
                const label = cd > 0 ? `${skill.name} (${cd})` : `✨ ${skill.name}`;
                buttons.push({
                    text: label,
                    action: () => {
                        hideModal();
                        activateOwnSupportSkill(p, playerIndex, sIdx);
                    }
                });
            });
        }

        buttons.push({ text: "Cancelar", action: hideModal });

        showModal("Escolha o ataque", body, buttons);
    }

    function setupBattleUI() {
        const playerContainer = document.getElementById('player-container');
        playerContainer.innerHTML = '';
        
        // Garante que activePlayerCreatures exista
        if (!state.activePlayerCreatures) {
            state.activePlayerCreatures = [state.activePlayerCreature];
        }

        state.activePlayerCreatures.forEach((p, index) => {
            const playerFighter = document.createElement('div');
            playerFighter.className = 'fighter';
            playerFighter.id = `player-fighter-${index}`;
            
            const hasActed = state.isBossBattle && p.battleState && p.battleState.hasActed;
            if (p.currentHp <= 0) playerFighter.style.opacity = '0.3';
            else if (hasActed) playerFighter.style.filter = 'grayscale(0.7) opacity(0.8)';

            playerFighter.innerHTML = `
                <div class="creature-card tooltip" style="width: 140px; height: 210px; margin: 0 auto; cursor: default; ${hasActed ? 'border-color: #555;' : ''}">
                    ${getCreatureDisplay(p)}
                    <div class="creature-card-info" style="padding: 10px 5px 6px 5px;">
                        <div class="creature-card-name" style="font-size: 0.85rem;">${p.name}</div>
                    </div>
                    ${hasActed ? '<div style="position:absolute; top:5px; right:5px; background:rgba(0,0,0,0.6); border-radius:50%; width:20px; height:20px; display:flex; align-items:center; justify-content:center; font-size:10px;">⏳</div>' : ''}
                    <span class="tooltiptext">${getCreatureTooltip(p)}</span>
                </div>
                <div class="hp-bar-container">
                    <div id="player-${index}-hp-bar" class="hp-bar-fill"></div>
                    <div id="player-${index}-hp-text" class="hp-text">${Math.floor(p.currentHp)}/${p.hp}</div>
                </div>
                <div class="xp-bar-container">
                    <div id="player-${index}-xp-bar" class="xp-bar-fill"></div>
                    <div id="player-${index}-xp-text" class="hp-text" style="font-size: 0.6rem;">Lvl ${p.level} - ${p.xp}/${p.maxXp} XP</div>
                </div>
            `;

            const cardEl = playerFighter.querySelector('.creature-card');
            if (cardEl) {
                cardEl.onclick = () => openAttackSelectionForPlayer(index);
            }

            playerFighter.ondblclick = () => {
                showModal("Status: " + p.name, getCreatureTooltip(p), [{ text: "Fechar", action: hideModal }]);
            };
            
            playerContainer.appendChild(playerFighter);
            updateHpUI(`player-${index}`, p);
            updateXpUI(p, index);
        });

        const enemiesContainer = document.getElementById('enemies-container');
        enemiesContainer.innerHTML = '';

        state.enemies.forEach((e, index) => {
            const enemyDiv = document.createElement('div');
            enemyDiv.className = 'fighter enemy-fighter';
            enemyDiv.id = `enemy-fighter-${index}`;
            if (e.currentHp <= 0) enemyDiv.style.opacity = '0.3';
            
            enemyDiv.innerHTML = `
                <div class="creature-card tooltip" style="width: 90px; height: 135px; margin: 0 auto; cursor: crosshair;">
                    ${getCreatureDisplay(e)}
                    <div class="creature-card-info" style="padding: 8px 4px 4px 4px;">
                        <div class="creature-card-name" style="font-size: 0.7rem;">${e.name}</div>
                    </div>
                    <span class="tooltiptext">${getCreatureTooltip(e)}</span>
                </div>
                <div class="hp-bar-container enemy-hp-container">
                    <div id="enemy-${index}-hp-bar" class="hp-bar-fill"></div>
                    <div id="enemy-${index}-hp-text" class="hp-text" style="font-size: 0.6rem;">${Math.floor(e.currentHp)}/${e.hp}</div>
                </div>
            `;
            
            enemyDiv.ondblclick = () => {
                showModal("Status: " + e.name, getCreatureTooltip(e), [{ text: "Fechar", action: hideModal }]);
            };
            
            if (e.currentHp > 0) {
                enemyDiv.style.cursor = 'crosshair';
                enemyDiv.title = "Clique para atacar este inimigo";
            }
            
            enemiesContainer.appendChild(enemyDiv);
            updateHpUI(`enemy-${index}`, e);
        });

        const benchRow = document.querySelector('.playmat-bench-row');
        if (benchRow && state.team) {
            const benchSlots = Array.from(benchRow.querySelectorAll('.battle-slot.bench'));
            const activeCreatures = state.activePlayerCreatures || [];
            const reserves = state.team.filter(c => c.currentHp > 0 && !activeCreatures.includes(c));

            benchSlots.forEach((slot, idx) => {
                slot.classList.remove('locked');
                slot.innerHTML = '';

                const creature = reserves[idx];
                if (creature) {
                    slot.classList.remove('empty');

                    const benchFighter = document.createElement('div');
                    benchFighter.className = 'fighter';

                    benchFighter.innerHTML = `
                        <div class="creature-card tooltip" style="width: 100px; height: 150px; margin: 0 auto; cursor: default;">
                            ${getCreatureDisplay(creature)}
                            <div class="creature-card-info" style="padding: 8px 4px 4px 4px;">
                                <div class="creature-card-name" style="font-size: 0.7rem;">${creature.name}</div>
                            </div>
                            <span class="tooltiptext">${getCreatureTooltip(creature)}</span>
                        </div>
                        <div class="hp-bar-container">
                            <div class="hp-bar-fill" style="width: ${(Math.max(0, creature.currentHp) / creature.hp) * 100}%;"></div>
                        </div>
                    `;

                    const benchCard = benchFighter.querySelector('.creature-card');
                    if (benchCard) {
                        benchCard.onclick = () => openSupportSelectionForCreature(creature);
                    }

                    slot.appendChild(benchFighter);
                } else {
                    slot.classList.add('empty');
                }
            });
        }

        const actions = document.getElementById('battle-actions');
        actions.innerHTML = '';

        const passBtn = document.createElement('button');
        passBtn.innerText = "Passar Turno";
        passBtn.style.backgroundColor = "#444";
        passBtn.onclick = () => passPlayerTurn();
        actions.appendChild(passBtn);

        autoScaleGame();
    }

    function activateSupportSkill(creature, skillIndex = 0) {
        // Suporte para estrutura antiga (supportSkill) e nova (supportSkills)
        const skills = creature.supportSkills || (creature.supportSkill ? [creature.supportSkill] : []);
        if (!skills[skillIndex]) return;
        const skill = skills[skillIndex];

        // No modo boss, habilidades de suporte afetam todos os jogadores ativos (se aplicável)
        const targets = state.isBossBattle ? state.activePlayerCreatures.filter(p => p.currentHp > 0) : [state.activePlayerCreature];
        if (targets.length === 0) return;

        addLog(`✨ SUPORTE: ${creature.name} usa ${skill.name}!`);
        playSound('heal'); // Som genérico de suporte

        let sCd = skill.cooldown;
        if (state.relics && state.relics.cooldownReduction) {
            sCd = Math.max(1, sCd - state.relics.cooldownReduction);
        }
        if (state.curse && state.curse.cooldownPlusOne) {
            sCd += 1;
        }
        creature.cooldowns[skill.name] = sCd;

        targets.forEach((active, tIdx) => {
            const playerIndex = state.isBossBattle ? state.activePlayerCreatures.indexOf(active) : 0;
            const playerCard = document.querySelector(`#player-fighter-${playerIndex} .creature-card`);

            switch (skill.effect) {
                case 'heal_percent':
                    const heal = Math.floor(active.hp * skill.value);
                    active.currentHp = Math.min(active.hp, active.currentHp + heal);
                    showFloatingValue(`player-fighter-${playerIndex}`, heal, 'heal');
                    updateHpUI(`player-${playerIndex}`, active);
                    addLog(`✨ ${active.name} recuperou ${heal} HP!`);
                    if (playerCard) {
                        playerCard.classList.remove('anim-heal');
                        void playerCard.offsetWidth;
                        playerCard.classList.add('anim-heal');
                    }
                    break;
                
                case 'fixed_dmg_buff':
                    active.battleState.nextAttackBonus = (active.battleState.nextAttackBonus || 0) + skill.value;
                    addLog(`✨ O próximo ataque de ${active.name} terá +${skill.value} de dano!`);
                    if (playerCard) {
                        playerCard.classList.remove('anim-buff');
                        void playerCard.offsetWidth;
                        playerCard.classList.add('anim-buff');
                    }
                    break;
                
                case 'dmg_reduction_buff':
                    active.battleState.damageReductionBuff = { value: skill.value, turns: skill.turns || 1 };
                    addLog(`✨ ${active.name} recebeu proteção! -${Math.round(skill.value * 100)}% de dano por ${skill.turns || 1} turnos.`);
                    if (playerCard) {
                        playerCard.classList.remove('anim-buff');
                        void playerCard.offsetWidth;
                        playerCard.classList.add('anim-buff');
                    }
                    break;
                
                case 'clean_recoil_heal':
                    const cleanHeal = skill.value || 15;
                    active.currentHp = Math.min(active.hp, active.currentHp + cleanHeal);
                    showFloatingValue(`player-fighter-${playerIndex}`, cleanHeal, 'heal');
                    updateHpUI(`player-${playerIndex}`, active);
                    addLog(`✨ ${active.name} recuperou ${cleanHeal} HP!`);
                    break;
                
                case 'guaranteed_max_dmg':
                    active.battleState.guaranteedMax = true;
                    addLog(`✨ O próximo ataque de ${active.name} causará dano máximo!`);
                    break;
                
                case 'dodge_buff':
                    active.battleState.dodgeBuff = { value: skill.value, turns: skill.turns || 1 };
                    addLog(`✨ ${active.name} ganhou +${Math.round(skill.value * 100)}% de esquiva por ${skill.turns || 1} turnos!`);
                    break;

                case 'invulnerability':
                    active.battleState.invulnerability = { turns: skill.turns };
                    addLog(`🛡️ ÉGIDE: ${active.name} está invulnerável por ${skill.turns} turno(s)!`);
                    break;

                case 'lifesteal_buff':
                    active.battleState.lifesteal = { value: skill.value, turns: skill.turns };
                    addLog(`🧛 VAMPIRO: ${active.name} curará ${Math.round(skill.value * 100)}% do dano causado!`);
                    break;

                case 'reflect_damage':
                    active.battleState.reflect = { value: skill.value, turns: skill.turns };
                    addLog(`🌵 ESPINHOS: ${active.name} refletirá ${Math.round(skill.value * 100)}% do dano recebido!`);
                    break;
                
                // --- NOVAS SKILLS ---
                case 'shield_flat':
                    active.battleState.shield = { value: skill.value, turns: skill.turns };
                    addLog(`🛡️ BARREIRA: ${active.name} recebeu um escudo de ${skill.value} HP!`);
                    showFloatingValue(`player-fighter-${playerIndex}`, skill.value, 'heal'); // Visual effect (blue?)
                    break;
                
                case 'regen_turns':
                    active.battleState.regen = { value: skill.value, turns: skill.turns };
                    addLog(`🌿 REGENERAÇÃO: ${active.name} vai recuperar HP por ${skill.turns} turnos!`);
                    break;
                
                case 'cleanse_debuffs': {
                    const allies = (state.team || []).filter(c => c.currentHp > 0);
                    allies.forEach(ally => {
                        if (!ally.battleState) ally.battleState = {};
                        if (ally.trapPoisonTurns) ally.trapPoisonTurns = 0;
                        if (ally.trapParalysisTurns) ally.trapParalysisTurns = 0;
                        ally.battleState.missChance = 0;
                        ally.battleState.defenseBuff = Math.max(0, ally.battleState.defenseBuff || 0);
                    });
                    updateUI();
                    addLog('✨ PURIFICAR EM MASSA: Toda a equipe foi purificada!');
                    playSound('heal');
                    break;
                }
                
                case 'crit_buff':
                    active.battleState.critBuff = { value: skill.value, turns: skill.turns };
                    addLog(`🎯 FOCO: ${active.name} aumentou sua chance de crítico!`);
                    break;
                
                case 'multi_hit_buff':
                    active.battleState.multiHit = { value: skill.value, turns: skill.turns };
                    addLog(`⚔️ GOLPE DUPLO: ${active.name} atacará duas vezes no próximo turno!`);
                    break;
                
                case 'taunt_support':
                    active.battleState.taunt = { turns: skill.turns };
                    addLog(`📢 PROVOCAR: ${active.name} está chamando a atenção dos inimigos!`);
                    break;

                case 'cooldown_reduce_one':
                    if (active.cooldowns) {
                        for (let k in active.cooldowns) {
                            if (active.cooldowns[k] > 0) active.cooldowns[k] = Math.max(0, active.cooldowns[k] - skill.value);
                        }
                    }
                    addLog(`⚡ ACELERAR: Os cooldowns de ${active.name} foram reduzidos!`);
                    break;
            }
        });

        // Efeitos globais (afetam inimigos ou estado global)
        switch (skill.effect) {
            case 'enemy_max_dmg_debuff':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.maxDmgDebuff = { value: skill.value, turns: skill.turns || 2 };
                        addLog(`✨ ${e.name} teve o dano máximo reduzido em ${skill.value} por ${skill.turns || 2} turnos!`);
                    }
                });
                break;
            
            case 'enemy_accuracy_down':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.missChance = (e.battleState.missChance || 0) + skill.value;
                        addLog(`🌫️ NÉVOA: A precisão de ${e.name} caiu!`);
                    }
                });
                break;

            case 'enemy_damage_down':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        // Usamos damageReduction no inimigo para significar "redução do dano QUE ELE CAUSA" 
                        // (Isso já é usado por 'Correntes do Esquecimento')
                        e.battleState.damageReduction = (e.battleState.damageReduction || 0) + skill.value;
                        addLog(`📉 ENFRAQUECER: O dano de ${e.name} foi reduzido!`);
                    }
                });
                break;
            
            case 'revive_weak':
                // Procura um aliado morto (nas criaturas ativas ou na equipe completa)
                let deadAlly = state.activePlayerCreatures.find(c => c.currentHp <= 0 || c.defeated);
                if (!deadAlly) {
                    // Se não encontrou nas ativas, procura na equipe completa
                    deadAlly = state.team.find(c => c.currentHp <= 0 || c.defeated);
                }
                if (deadAlly) {
                    deadAlly.currentHp = Math.floor(deadAlly.hp * skill.value);
                    deadAlly.defeated = false; // Remove status de derrotado
                    addLog(`✨ REANIMAR: ${deadAlly.name} voltou à vida com ${deadAlly.currentHp} HP!`);
                    playSound('heal');
                    
                    // Remove animação de morte se existir
                    const pIdx = state.activePlayerCreatures.indexOf(deadAlly);
                    const playerCard = document.querySelector(`#player-fighter-${pIdx} .creature-card`);
                    if (playerCard) playerCard.classList.remove('anim-death');
                    updateHpUI(`player-${pIdx}`, deadAlly);
                } else {
                    addLog(`✨ REANIMAR: Nenhum aliado abatido para reanimar.`);
                }
                break;

            case 'auto_revive':
                // Aplica um buff de auto-revivência na criatura alvo
                targets.forEach(active => {
                    active.battleState.autoRevive = { value: skill.value, turns: 2 };
                    addLog(`✨ RENASCIMENTO: ${active.name} vai renascer com ${Math.floor(active.hp * skill.value)} HP se for derrotado!`);
                });
                break;
            
            case 'clean_recoil_heal':
                // "Limpa recoil" - Vamos interpretar como limpar os boosts que o jogador deu aos inimigos
                state.enemies.forEach(e => {
                    if (e.tempDmgBoost > 1) {
                        e.tempDmgBoost = 1;
                        addLog(`✨ Os inimigos perderam os bônus de dano recebidos!`);
                    }
                });
                break;
            
            case 'stun_chance':
                if (Math.random() < skill.value) {
                    state.enemies.forEach(e => {
                        if (e.currentHp > 0) {
                            e.battleState.stunned = true;
                            addLog(`✨ ${e.name} ficou atordoado e perderá o próximo turno!`);
                        }
                    });
                } else {
                    addLog(`✨ A fumaça falhou em atordoar os inimigos.`);
                }
                break;

            case 'stun_enemy':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.stunned = true;
                        addLog(`🦑 ABRAÇO: ${e.name} foi agarrado e está atordoado!`);
                    }
                });
                break;

            case 'poison_enemy':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.poison = { value: skill.value, turns: skill.turns };
                        addLog(`☠️ VENENO: ${e.name} foi envenenado!`);
                    }
                });
                break;
        }

        setupBattleUI();
    }

    function activateOwnSupportSkill(creature, playerIndex, skillIndex = 0) {
        const skills = creature.supportSkills || (creature.supportSkill ? [creature.supportSkill] : []);
        const skill = skills[skillIndex];
        if (!skill) return;

        // Verifica se a criatura já agiu neste turno (modo boss)
        const hasActed = state.isBossBattle && creature.battleState && creature.battleState.hasActed;
        if (hasActed) {
            addLog(`⏳ ${creature.name} já agiu neste turno!`);
            return;
        }

        // Verifica se está paralisada
        if (creature.trapParalysisTurns > 0) {
            addLog(`🌀 ${creature.name} está paralisada e não pode usar habilidades!`);
            return;
        }

        // Verifica cooldown
        const cd = creature.cooldowns[skill.name] || 0;
        if (cd > 0) {
            addLog(`⏰ ${skill.name} está em recarga (${cd} turnos restantes)!`);
            return;
        }

        // No modo boss, habilidades de suporte afetam todos os jogadores ativos (se aplicável)
        const targets = state.isBossBattle ? state.activePlayerCreatures.filter(p => p.currentHp > 0) : [creature];
        if (targets.length === 0) return;

        addLog(`✨ ${creature.name} usa ${skill.name}!`);
        playSound('heal');

        // Registra o cooldown
        creature.cooldowns[skill.name] = skill.cooldown;

        // Marca que a criatura agiu (modo boss)
        if (state.isBossBattle && creature.battleState) {
            creature.battleState.hasActed = true;
        }

        targets.forEach((target, tIdx) => {
            const targetIndex = state.isBossBattle ? state.activePlayerCreatures.indexOf(target) : playerIndex;

            switch (skill.effect) {
                case 'heal_percent':
                    const heal = Math.floor(target.hp * skill.value);
                    target.currentHp = Math.min(target.hp, target.currentHp + heal);
                    showFloatingValue(`player-fighter-${targetIndex}`, heal, 'heal');
                    updateHpUI(`player-${targetIndex}`, target);
                    addLog(`✨ ${target.name} recuperou ${heal} HP!`);
                    break;
                
                case 'fixed_dmg_buff':
                    target.battleState.nextAttackBonus = (target.battleState.nextAttackBonus || 0) + skill.value;
                    addLog(`✨ O próximo ataque de ${target.name} terá +${skill.value} de dano!`);
                    break;
                
                case 'dmg_reduction_buff':
                    target.battleState.damageReductionBuff = { value: skill.value, turns: skill.turns || 1 };
                    addLog(`✨ ${target.name} recebeu proteção! -${Math.round(skill.value * 100)}% de dano por ${skill.turns || 1} turnos.`);
                    break;
                
                case 'clean_recoil_heal':
                    const cleanHeal = skill.value || 15;
                    target.currentHp = Math.min(target.hp, target.currentHp + cleanHeal);
                    showFloatingValue(`player-fighter-${targetIndex}`, cleanHeal, 'heal');
                    updateHpUI(`player-${targetIndex}`, target);
                    addLog(`✨ ${target.name} recuperou ${cleanHeal} HP!`);
                    break;
                
                case 'guaranteed_max_dmg':
                    target.battleState.guaranteedMax = true;
                    addLog(`✨ O próximo ataque de ${target.name} causará dano máximo!`);
                    break;
                
                case 'dodge_buff':
                    target.battleState.dodgeBuff = { value: skill.value, turns: skill.turns || 1 };
                    addLog(`✨ ${target.name} ganhou +${Math.round(skill.value * 100)}% de esquiva por ${skill.turns || 1} turnos!`);
                    break;

                case 'invulnerability':
                    target.battleState.invulnerability = { turns: skill.turns };
                    addLog(`🛡️ ÉGIDE: ${target.name} está invulnerável por ${skill.turns} turno(s)!`);
                    break;

                case 'lifesteal_buff':
                    target.battleState.lifesteal = { value: skill.value, turns: skill.turns };
                    addLog(`🧛 VAMPIRO: ${target.name} curará ${Math.round(skill.value * 100)}% do dano causado!`);
                    break;

                case 'reflect_damage':
                    target.battleState.reflect = { value: skill.value, turns: skill.turns };
                    addLog(`🌵 ESPINHOS: ${target.name} refletirá ${Math.round(skill.value * 100)}% do dano recebido!`);
                    break;
                
                case 'shield_flat':
                    target.battleState.shield = { value: skill.value, turns: skill.turns };
                    addLog(`🛡️ BARREIRA: ${target.name} recebeu um escudo de ${skill.value} HP!`);
                    showFloatingValue(`player-fighter-${targetIndex}`, skill.value, 'heal');
                    break;
                
                case 'regen_turns':
                    target.battleState.regen = { value: skill.value, turns: skill.turns };
                    addLog(`🌿 REGENERAÇÃO: ${target.name} vai recuperar HP por ${skill.turns} turnos!`);
                    break;
                
                case 'cleanse_debuffs': {
                    const allies = (state.team || []).filter(c => c.currentHp > 0);
                    allies.forEach(ally => {
                        if (!ally.battleState) ally.battleState = {};
                        if (ally.trapPoisonTurns) ally.trapPoisonTurns = 0;
                        if (ally.trapParalysisTurns) ally.trapParalysisTurns = 0;
                        ally.battleState.missChance = 0;
                        ally.battleState.defenseBuff = Math.max(0, ally.battleState.defenseBuff || 0);
                    });
                    updateUI();
                    addLog('✨ PURIFICAR EM MASSA: Toda a equipe foi purificada!');
                    playSound('heal');
                    break;
                }
                
                case 'crit_buff':
                    target.battleState.critBuff = { value: skill.value, turns: skill.turns };
                    addLog(`🎯 FOCO: ${target.name} aumentou sua chance de crítico!`);
                    break;
                
                case 'multi_hit_buff':
                    target.battleState.multiHit = { value: skill.value, turns: skill.turns };
                    addLog(`⚔️ GOLPE DUPLO: ${target.name} atacará duas vezes no próximo turno!`);
                    break;
                
                case 'taunt_support':
                    target.battleState.taunt = { turns: skill.turns };
                    addLog(`📢 PROVOCAR: ${target.name} está chamando a atenção dos inimigos!`);
                    break;

                case 'cooldown_reduce_one':
                    if (target.cooldowns) {
                        for (let k in target.cooldowns) {
                            if (target.cooldowns[k] > 0) target.cooldowns[k] = Math.max(0, target.cooldowns[k] - skill.value);
                        }
                    }
                    addLog(`⚡ ACELERAR: Os cooldowns de ${target.name} foram reduzidos!`);
                    break;
            }
        });

        // Efeitos globais (afetam inimigos ou estado global)
        switch (skill.effect) {
            case 'enemy_max_dmg_debuff':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.maxDmgDebuff = { value: skill.value, turns: skill.turns || 2 };
                        addLog(`✨ ${e.name} teve o dano máximo reduzido em ${skill.value} por ${skill.turns || 2} turnos!`);
                    }
                });
                break;
            
            case 'enemy_accuracy_down':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.missChance = (e.battleState.missChance || 0) + skill.value;
                        addLog(`🌫️ NÉVOA: A precisão de ${e.name} caiu!`);
                    }
                });
                break;

            case 'enemy_damage_down':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.damageReduction = (e.battleState.damageReduction || 0) + skill.value;
                        addLog(`📉 ENFRAQUECER: O dano de ${e.name} foi reduzido!`);
                    }
                });
                break;
            
            case 'revive_weak':
                // Procura um aliado morto (nas criaturas ativas ou na equipe completa)
                let deadAlly = state.activePlayerCreatures.find(c => c.currentHp <= 0 || c.defeated);
                if (!deadAlly) {
                    // Se não encontrou nas ativas, procura na equipe completa
                    deadAlly = state.team.find(c => c.currentHp <= 0 || c.defeated);
                }
                if (deadAlly) {
                    deadAlly.currentHp = Math.floor(deadAlly.hp * skill.value);
                    deadAlly.defeated = false; // Remove status de derrotado
                    addLog(`✨ REANIMAR: ${deadAlly.name} voltou à vida com ${deadAlly.currentHp} HP!`);
                    playSound('heal');
                    
                    const pIdx = state.activePlayerCreatures.indexOf(deadAlly);
                    const playerCard = document.querySelector(`#player-fighter-${pIdx} .creature-card`);
                    if (playerCard) playerCard.classList.remove('anim-death');
                    updateHpUI(`player-${pIdx}`, deadAlly);
                } else {
                    addLog(`✨ REANIMAR: Nenhum aliado abatido para reanimar.`);
                }
                break;
            
            case 'clean_recoil_heal':
                state.enemies.forEach(e => {
                    if (e.tempDmgBoost > 1) {
                        e.tempDmgBoost = 1;
                        addLog(`✨ Os inimigos perderam os bônus de dano recebidos!`);
                    }
                });
                break;
            
            case 'stun_chance':
                if (Math.random() < skill.value) {
                    state.enemies.forEach(e => {
                        if (e.currentHp > 0) {
                            e.battleState.stunned = true;
                            addLog(`✨ ${e.name} ficou atordoado e perderá o próximo turno!`);
                        }
                    });
                } else {
                    addLog(`✨ A fumaça falhou em atordoar os inimigos.`);
                }
                break;

            case 'stun_enemy':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.stunned = true;
                        addLog(`🦑 ABRAÇO: ${e.name} foi agarrado e está atordoado!`);
                    }
                });
                break;

            case 'poison_enemy':
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.battleState.poison = { value: skill.value, turns: skill.turns };
                        addLog(`☠️ VENENO: ${e.name} foi envenenado!`);
                    }
                });
                break;
        }

        setupBattleUI();
    }

    function updateHpUI(who, creature) {
        if (!creature) return;
        const percent = Math.max(0, Math.min(100, (creature.currentHp / creature.hp) * 100));
        
        // Suporte a legado 'player' redirecionando para 'player-0'
        const effectiveWho = who === 'player' ? 'player-0' : who;
        
        const bar = document.getElementById(`${effectiveWho}-hp-bar`);
        const text = document.getElementById(`${effectiveWho}-hp-text`);
        if (bar) bar.style.width = percent + '%';
        if (text) text.innerText = `${Math.max(0, Math.floor(creature.currentHp))}/${creature.hp}`;
        
        // Atualiza Tooltip
        let containerId;
        if (effectiveWho.startsWith('player-')) {
            containerId = `player-fighter-${effectiveWho.split('-')[1]}`;
        } else if (effectiveWho.startsWith('enemy-')) {
            containerId = `enemy-fighter-${effectiveWho.split('-')[1]}`;
        } else {
            containerId = effectiveWho === 'player' ? 'player-fighter-0' : `enemy-fighter-0`;
        }

        const container = document.getElementById(containerId);
        if (container) {
            const tooltip = container.querySelector('.tooltiptext');
            if (tooltip) tooltip.innerHTML = getCreatureTooltip(creature);
        }
    }

    function updateXpUI(creature, index = 0) {
        const percent = (creature.xp / creature.maxXp) * 100;
        const bar = document.getElementById(`player-${index}-xp-bar`);
        const text = document.getElementById(`player-${index}-xp-text`);
        if (bar) bar.style.width = percent + '%';
        if (text) text.innerText = `Lvl ${creature.level} - ${creature.xp}/${creature.maxXp} XP`;
    }

    function playerAttack(atk, enemyIndex, playerIndex = 0) {
        // FIX: Ensure activePlayerCreatures is populated
        if (!state.activePlayerCreatures || state.activePlayerCreatures.length === 0) {
            if (state.activePlayerCreature) {
                state.activePlayerCreatures = [state.activePlayerCreature];
            } else {
                console.error("PlayerAttack: No active player creatures found!");
                return;
            }
        }

        const p = state.activePlayerCreatures[playerIndex];
        if (!p) return;
        
        // --- SUPORTE GOLPE DUPLO (Multi Hit) ---
        let attacksCount = 1;
        if (p.battleState && p.battleState.multiHit) {
            attacksCount = 2;
            // O buff dura X turnos, então não removemos aqui, removemos no processTurnEnd
        }

        for (let i = 0; i < attacksCount; i++) {
            if (i > 0) {
                // Pequeno delay visual para o segundo ataque (se possível, mas síncrono aqui)
                addLog(`⚔️ GOLPE DUPLO: ${p.name} ataca novamente!`);
            }
            const isLast = (i === attacksCount - 1);
            executePlayerAttack(atk, enemyIndex, playerIndex, !isLast);
        }
    }

    function executePlayerAttack(atk, enemyIndex, playerIndex, skipTurnPass = false) {
        const p = state.activePlayerCreatures[playerIndex];
        
        if (p.trapParalysisTurns && p.trapParalysisTurns > 0) {
            addLog(`${p.name} está paralisado pela armadilha e não consegue atacar!`);
            p.trapParalysisTurns--;
            return;
        }

        // Verifica Cooldown (Apenas na primeira execução se for multi-hit, mas simplificamos checando fora se possível)
        // Como o cooldown já foi verificado antes de chamar playerAttack na UI, aqui verificamos novamente por segurança
        // Mas para multi-hit, o cooldown só deve ser aplicado uma vez.
        // Vamos assumir que o cooldown é aplicado apenas na primeira chamada.
        
        // ... (resto da lógica de ataque movida para cá)
        
        // Se já agiu neste turno... (lógica mantida)
        
        playSound('attack');
        const target = state.enemies[enemyIndex];
        
        const enemyEl = document.getElementById(`enemy-fighter-${enemyIndex}`);
        const enemyCard = enemyEl ? enemyEl.querySelector('.creature-card') : null;
        const playerEl = document.getElementById(`player-fighter-${playerIndex}`);
        const playerCard = playerEl ? playerEl.querySelector('.creature-card') : null;
        if (playerCard) {
            const baseClasses = ['anim-attack-p','anim-attack-primordiais','anim-attack-selvagens','anim-attack-sombras','anim-attack-abissais'];
            baseClasses.forEach(cls => playerCard.classList.remove(cls));
            void playerCard.offsetWidth;
            let attackClass = 'anim-attack-p';
            if (p.clan === 'primordiais') attackClass = 'anim-attack-primordiais';
            else if (p.clan === 'selvagens') attackClass = 'anim-attack-selvagens';
            else if (p.clan === 'sombras') attackClass = 'anim-attack-sombras';
            else if (p.clan === 'abissais') attackClass = 'anim-attack-abissais';
            playerCard.classList.add(attackClass);
        }
        // ...
        
        let dmg = Math.floor(Math.random() * (atk.max - atk.min + 1)) + atk.min;
        if (state.permanentBuffs && state.permanentBuffs.atkBonus) {
            dmg = Math.floor(dmg * (1 + state.permanentBuffs.atkBonus));
        }

        // --- SUPORTE FOCO LETAL (Crítico) ---
        if (p.battleState && p.battleState.critBuff) {
            const critChance = p.battleState.critBuff.value; // 0.25
            if (Math.random() < critChance) {
                dmg = Math.floor(dmg * 1.5);
                addLog(`🎯 CRÍTICO! ${p.name} acertou em um ponto vital!`);
            }
        }

        // --- SUPORTE ORÁCULO: Garantia de Dano Máximo ---
        if (p.battleState && p.battleState.guaranteedMax) {
            dmg = atk.max;
            p.battleState.guaranteedMax = false;
            addLog(`✨ PREVISÃO: ${p.name} desferiu um golpe perfeito!`);
        }

        // --- SUPORTE MAGO: Bônus de Próximo Ataque ---
        if (p.battleState && p.battleState.nextAttackBonus) {
            dmg += p.battleState.nextAttackBonus;
            p.battleState.nextAttackBonus = 0;
            addLog(`✨ ENCANTAMENTO: Dano extra aplicado!`);
        }

        // --- APLICA BOOST TEMPORÁRIO (de custos do inimigo) ---
        if (p.tempDmgBoost) {
            dmg = Math.floor(dmg * p.tempDmgBoost);
            p.tempDmgBoost = 1; // Reseta após usar
        }
        // -----------------------------------------------------

        // --- NOVOS BUFFS/DEBUFFS OFFENSIVOS ---
        let ignoreDodge = false;
        let ignoreWeakness = false;
        let ignoreDefense = false;

        if (p.battleState.lifesteal && p.battleState.lifesteal.turns > 0) {
             const lifeStealHeal = Math.floor(dmg * p.battleState.lifesteal.value);
             p.currentHp = Math.min(p.hp, p.currentHp + lifeStealHeal);
             showFloatingValue(`player-fighter-${playerIndex}`, lifeStealHeal, 'heal');
             addLog(`🧛 VAMPIRO: ${p.name} drenou ${lifeStealHeal} HP do inimigo!`);
             updateHpUI(`player-${playerIndex}`, p);
        }

        if (p.items && p.items.length > 0) {
            p.items.forEach(item => {
                const lvl = item.level || 1;
                const itemDourado = (msg) => `<span style="color:gold; font-weight:bold;">${item.icon || '📦'} ${msg}</span>`;

                // 1. Amuleto do Ritmo: +10% dano por turno consecutivo
                if (item.id === 'amuleto_ritmo') {
                    if (p.currentHp < p.hp * 0.3) p.battleState.consecutiveTurns = 0;
                    const bonus = 1 + (p.battleState.consecutiveTurns * 0.1);
                    dmg = Math.floor(dmg * bonus);
                }

                // 2. Coração Ancestral: -6% dano
                if (item.id === 'coracao_ancestral') dmg = Math.floor(dmg * 0.94);

                // 4. Olho do Predador
                if (item.id === 'olho_predador') {
                    if (p.battleState.targetLock.index === enemyIndex) {
                        if (p.battleState.targetLock.turns >= 2) {
                            ignoreDodge = true;
                            ignoreWeakness = true;
                            ignoreDefense = true;
                            addLog(itemDourado(`OLHO DO PREDADOR: Focado! Ignorando defesas.`));
                            p.battleState.targetLock.turns = 0; // Reseta após usar o buff
                        } else {
                            p.battleState.targetLock.turns++;
                        }
                    } else {
                        p.battleState.targetLock = { index: enemyIndex, turns: 1 };
                    }
                }

                // 5. Tônico de Guerra
                if (item.id === 'tonico_guerra') {
                    if (p.battleState.tonicoTurns > 0) {
                        dmg = Math.floor(dmg * 1.15);
                    } else if (p.battleState.tonicoTurns === -1) {
                        dmg = Math.floor(dmg * 0.90); // Exaustão
                    }
                }

                // 11. Lâmina Instável
                if (item.id === 'lamina_instavel') dmg = Math.floor(dmg * 1.2);

                // 13. Máscara do Sacrifício: +5% dmg por debuff no inimigo
                if (item.id === 'mascara_sacrificio' && target.battleState) {
                    let debuffs = 0;
                    if (target.battleState.damageReduction > 0) debuffs++;
                    if (target.battleState.takenDamageIncrease > 0) debuffs++;
                    if (target.battleState.missChance > 0) debuffs++;
                    if (target.battleState.bleedTurns > 0) debuffs++;
                    dmg = Math.floor(dmg * (1 + (debuffs * 0.05)));
                }
            });
        }

        // Marca do Abismo (no alvo)
        if (target.battleState && target.battleState.takenDamageIncrease > 0) {
            dmg = Math.floor(dmg * (1 + target.battleState.takenDamageIncrease));
        }
        // --------------------------------------

        // Bônus Primordiais: +20% Dano
        if (p.clan === 'primordiais') {
            dmg = Math.floor(dmg * 1.2);
        }

        // Vantagem/Desvantagem de Clã
        let multiplier = 1.0;
        if (!atk.ignoreClans && p.clan && target.clan && !ignoreWeakness) {
            const attackerClan = CLANS[p.clan];
            const targetClan = CLANS[target.clan];
            if (attackerClan.strongAgainst.includes(target.clan)) {
                multiplier = 1.2;
                addLog(`✨ VANTAGEM: ${attackerClan.emoji} ${p.name} é forte contra ${targetClan.emoji} ${target.name}! (+20%)`);
            } else if (attackerClan.weakAgainst.includes(target.clan)) {
                multiplier = 0.8;
                addLog(`✖ DESVANTAGEM: ${attackerClan.emoji} ${p.name} é fraco contra ${targetClan.emoji} ${target.name}! (-20%)`);
            }
        }
        dmg = Math.floor(dmg * multiplier);

        // --- CHANCE DE ESQUIVA DO INIMIGO ---
        let enemyDodged = false;
        if (!ignoreDodge) {
            // Bônus Sombras para Inimigos: +20% Esquiva
            if (target.clan === 'sombras' && Math.random() < 0.20) enemyDodged = true;
            
            // Inimigos também podem ter Manto de Esquiva se vierem com itens
            if (target.items && target.items.some(i => i.id === 'manto_esquiva') && Math.random() < 0.25) enemyDodged = true;
        }

        if (enemyDodged) {
            addLog(`<span style="color:#4dabff;">💨 ESQUIVA: ${target.name} desviou do ataque de ${p.name}!</span>`);
            
            // 6. Correntes do Esquecimento: Remove se o inimigo esquivar
            if (p.items && p.items.some(i => i.id === 'correntes_esquecimento')) {
                if (target.battleState) target.battleState.damageReduction = 0;
                addLog(`⛓️ CORRENTES: As correntes se soltaram após a esquiva!`);
            }

            // Animação de Esquiva no Inimigo
            if (enemyCard) {
                enemyCard.classList.remove('anim-flash');
                void enemyCard.offsetWidth;
                enemyCard.classList.add('anim-flash');
            }
            
            // JOGADOR PERDE O TURNO APÓS ESQUIVA DO INIMIGO
            p.battleState.hasActed = true;
            
        } else {
            // Efeitos de Itens Ofensivos (Calculados com Nível do Item)
            if (p.items && p.items.length > 0) {
                p.items.forEach(item => {
                    const lvl = item.level || 1;
                    const itemDourado = (msg) => `<span style="color:gold; font-weight:bold;">${item.icon || '📦'} ${msg}</span>`;
                    
                    if (item.id === 'garra_obsidiana') dmg += (50 * lvl);
                    if (item.id === 'anel_fogo') {
                        if (Math.random() > 0.5) dmg += (15 * lvl); 
                    }
                    if (item.id === 'amolador_presas') dmg = Math.max(dmg, 80 + (5 * (lvl-1)));
                    if (item.id === 'cajado_runico' && p.isEvolved) dmg = Math.floor(dmg * (2.5 + (0.05 * (lvl-1))));
                    if (item.id === 'coroa_demonio') dmg += (30 * lvl);
                    if (item.id === 'espelho_mistico') {
                        if (Math.random() < (0.6 + (0.2 * (lvl-1)))) {
                            const enemyMaxAtk = Math.max(...target.attacks.map(a => a.max));
                            dmg += enemyMaxAtk;
                            addLog(itemDourado(`ESPELHO MÍSTICO: Refletiu +${enemyMaxAtk} de dano! (Nível ${lvl})`));
                        }
                    }
                    // Colar de Regeneração (Cura ao atacar)
                    if (item.id === 'colar_regeneracao') {
                        const heal = Math.floor(p.hp * (0.10 * lvl));
                        p.currentHp = Math.min(p.hp, p.currentHp + heal);
                        showFloatingValue(`player-fighter-${playerIndex}`, heal, 'heal');
                        addLog(itemDourado(`REGENERAÇÃO: ${p.name} recuperou ${heal} HP!`));
                        updateHpUI(`player-${playerIndex}`, p);
                    }

                    // 9. Sangramento (Aplica/Reseta duração ao atacar)
                    if (item.id === 'sangramento') {
                        if (!target.battleState) {
                            target.battleState = {
                                damageReduction: 0,
                                takenDamageIncrease: 0,
                                missChance: 0,
                                bleedTurns: 0,
                                turnCount: 0
                            };
                        }
                        target.battleState.bleedTurns = 3;
                        addLog(itemDourado(`SANGRAMENTO: ${target.name} está sangrando!`));
                    }
                });
            }

            target.currentHp -= dmg;
            showFloatingValue(`enemy-fighter-${enemyIndex}`, dmg, 'damage');
            const dmgStyle = dmg > 100 ? `color:#ff4d4d; font-size:1.1rem; font-weight:bold;` : `color:#ff4d4d;`;
            addLog(`${p.name} usou ${atk.name} em ${target.name}: <span style="${dmgStyle}">${dmg} dano!</span>`);

            // --- EFEITOS DE ITENS DO INIMIGO (REVIDE/DEFESA) ---
            if (target.items && target.items.length > 0 && !ignoreDefense) {
                target.items.forEach(item => {
                    const lvl = item.level || 1;
                    // Escudo de Espinhos no Inimigo
                    if (item.id === 'escudo_espinhos') {
                        const reflect = Math.floor(dmg * (0.08 + (0.02 * (lvl - 1))));
                        p.currentHp -= reflect;
                        addLog(`<span style="color:#ff4d4d;">🌵 ESPINHOS: ${target.name} refletiu ${reflect} de dano!</span>`);
                        showFloatingValue(`player-fighter-${playerIndex}`, reflect, 'damage');
                        updateHpUI(`player-${playerIndex}`, p);
                    }
                });
            }
            // --------------------------------------------------


            // --- CUSTOS DE ATAQUE ---
            // 1. Recoil (Dano ao usuário)
            if (atk.recoil) {
                const recoilDmg = Math.floor(p.hp * atk.recoil);
                p.currentHp -= recoilDmg;
                showFloatingValue(`player-fighter-${playerIndex}`, recoilDmg, 'damage');
                addLog(`⚠️ RECUO: ${p.name} recebeu ${recoilDmg} de dano pelo impacto!`);
                updateHpUI(`player-${playerIndex}`, p);
                if (playerCard) {
                    const fxRecoil = document.createElement('div');
                    fxRecoil.className = 'attack-effect-recoil';
                    playerCard.appendChild(fxRecoil);
                    setTimeout(() => {
                        if (fxRecoil.parentNode) fxRecoil.parentNode.removeChild(fxRecoil);
                    }, 450);
                }
            }

            // 2. Enemy Boost (Fortalece inimigos)
            if (atk.enemyBoost) {
                state.enemies.forEach(e => {
                    if (e.currentHp > 0) {
                        e.tempDmgBoost = (e.tempDmgBoost || 1) * atk.enemyBoost.dmg;
                    }
                });
                addLog(`🔥 PODER INIMIGO: O ataque de ${p.name} fortaleceu os inimigos!`);
                state.enemies.forEach((e, idx) => {
                    const eEl = document.getElementById(`enemy-fighter-${idx}`);
                    const eCard = eEl ? eEl.querySelector('.creature-card') : null;
                    if (eCard && e.currentHp > 0) {
                        const fxBoost = document.createElement('div');
                        fxBoost.className = 'attack-effect-boost';
                        eCard.appendChild(fxBoost);
                        setTimeout(() => {
                            if (fxBoost.parentNode) fxBoost.parentNode.removeChild(fxBoost);
                        }, 500);
                    }
                });
            }

            // 3. Cooldown (Recarga)
            if (atk.cooldown) {
                let cd = atk.cooldown;
                if (state.relics && state.relics.cooldownReduction) {
                    cd = Math.max(1, cd - state.relics.cooldownReduction);
                }
                if (state.curse && state.curse.cooldownPlusOne) {
                    cd += 1;
                }
                p.cooldowns[atk.name] = cd;
            }
            // ------------------------

            updateHpUI(`enemy-${enemyIndex}`, target);
            
            // Animação de Dano no Inimigo
            if (enemyCard) {
                enemyCard.classList.remove('anim-shake', 'anim-flash');
                void enemyCard.offsetWidth;
                enemyCard.classList.add('anim-shake', 'anim-flash');
                const fx = document.createElement('div');
                fx.className = 'attack-effect';
                enemyCard.appendChild(fx);
                setTimeout(() => {
                    if (fx.parentNode) fx.parentNode.removeChild(fx);
                }, 400);
            }
        } // Fim do else (se não esquivou)

        // Atualiza tooltip do inimigo (HP mudou)
        if (enemyEl) {
            const enemyTooltip = enemyEl.querySelector('.tooltiptext');
            if (enemyTooltip) enemyTooltip.innerHTML = getCreatureTooltip(target);
        }

        // --- VERIFICAÇÃO DE MORTE DO INIMIGO ---
        if (target.currentHp <= 0) {
            addLog(`${target.name} foi derrotado!`);
            handleEnemyDeath(target); // Loot
            if (enemyCard) enemyCard.classList.add('anim-death');
            else if (enemyEl) enemyEl.style.opacity = '0.3';
        }

        // --- VERIFICAÇÃO DE MORTE DO JOGADOR (Recoil/Espinhos) ---
        if (p.currentHp <= 0) {
            addLog(`${p.name} desmaiou pelo esforço!`);
            handlePlayerDeath(p); // Salva itens
            if (playerCard) playerCard.classList.add('anim-death');
        }

        // --- PROCESSA FIM DE TURNO DO JOGADOR ---
        p.battleState.hasActed = true;
        
        // Se estiver no modo boss, desativa apenas os botões desta criatura
        if (state.isBossBattle) {
            setupBattleUI(); // Redesenha para refletir cooldowns e hasActed
        }

        // SEMPRE passa o turno após o ataque, independentemente de outros jogadores terem agido
        console.log("DEBUG: skipTurnPass =", skipTurnPass);
        if (!skipTurnPass) {
            console.log("DEBUG: Chamando processTurnEnd('player')");
            processTurnEnd('player');
            
            // Verifica mortes pós-efeitos
            const alivePlayers = (state.activePlayerCreatures || []).filter(c => c.currentHp > 0);
            alivePlayers.forEach((player, idx) => {
                if (player.currentHp <= 0) {
                    addLog(`${player.name} sucumbiu aos efeitos colaterais.`);
                    handlePlayerDeath(player); // Salva itens
                    const pCard = document.querySelector(`#player-fighter-${idx} .creature-card`);
                    if (pCard) pCard.classList.add('anim-death');
                }
            });

            const teamDefeated = state.activePlayerCreatures.every(c => c.currentHp <= 0);
            if (teamDefeated) {
                toggleButtons(true);
                setTimeout(loseBattle, 1000);
                return;
            }

            toggleButtons(true);
            const allEnemiesDead = state.enemies.every(e => e.currentHp <= 0);
            if (allEnemiesDead) {
                setTimeout(winBattle, 1000);
            } else {
                updateTurnIndicator(false);
                console.log("DEBUG: Iniciando turno do inimigo em 1 segundo");
                setTimeout(() => {
                    console.log("DEBUG: Executando enemyTurn");
                    enemyTurn();
                }, 1000);
            }
        }
    }

    function enemyTurn() {
        console.log("DEBUG: enemyTurn() chamada");
        // Safety: Ensure enemies array exists
        if (!state.enemies) {
            console.log("DEBUG: state.enemies não existe, retornando");
            return;
        }

        const aliveEnemies = state.enemies.filter(e => e.currentHp > 0);
        if (aliveEnemies.length === 0) { winBattle(); return; }
        
        updateTurnIndicator(false);
        let currentAttackerIndex = 0;

        // FIX: Ensure activePlayerCreatures is populated
        if (!state.activePlayerCreatures || state.activePlayerCreatures.length === 0) {
            if (state.activePlayerCreature) {
                state.activePlayerCreatures = [state.activePlayerCreature];
            } else {
                console.warn("EnemyTurn: No active player creatures found!");
            }
        }
        
        function nextEnemyAttack() {
            if (currentAttackerIndex >= aliveEnemies.length) {
                // Ao final do turno dos inimigos
                processTurnEnd('enemy'); // Aplica sangramento, regeneração, etc.

                // Verifica se todos os jogadores morreram
                const allPlayersDead = (state.activePlayerCreatures || []).every(p => p.currentHp <= 0);
                if (allPlayersDead) {
                    addLog(`Toda a sua equipe foi derrotada.`);
                    if (state.activePlayerCreatures) {
                        state.activePlayerCreatures.forEach((p, idx) => {
                            const playerCard = document.querySelector(`#player-fighter-${idx} .creature-card`);
                            if (playerCard) playerCard.classList.add('anim-death');
                        });
                    }
                    setTimeout(loseBattle, 1000);
                    return;
                }

                // Verifica se todos os inimigos morreram por efeitos de fim de turno
                const allDead = state.enemies.every(e => e.currentHp <= 0);
                if (allDead) {
                    setTimeout(winBattle, 1000);
                    return;
                }
                
                updateTurnIndicator(true);
                setupBattleUI(); // Redesenha para resetar estados visuais dos botões
                toggleButtons(false);
                return;
            }
            
            const enemy = aliveEnemies[currentAttackerIndex];
            
            // --- VERIFICA PROVOCAR (Taunt) ---
            let potentialTargets = (state.activePlayerCreatures || [])
                .map((p, idx) => ({p, idx}))
                .filter(item => item.p && item.p.currentHp > 0);
            
            const tauntedTargets = potentialTargets.filter(item => item.p.battleState && item.p.battleState.taunt);
            
            if (tauntedTargets.length > 0) {
                // Se houver alguém provocando, foca nele(s)
                potentialTargets = tauntedTargets;
            }
            
            if (potentialTargets.length === 0) {
                currentAttackerIndex++;
                setTimeout(nextEnemyAttack, 100);
                return;
            }

            const targetItem = potentialTargets[Math.floor(Math.random() * potentialTargets.length)];
            const p = targetItem.p;
            const pIdx = targetItem.idx;

            // Garante que o battleState existe
            if (!enemy.battleState) {
                enemy.battleState = {
                    damageReduction: 0,
                    takenDamageIncrease: 0,
                    missChance: 0,
                    bleedTurns: 0,
                    turnCount: 0
                };
            }
            
            // --- VERIFICA MISS CHANCE (Névoa/Cegueira) ---
            if (enemy.battleState.missChance > 0) {
                if (Math.random() < enemy.battleState.missChance) {
                    addLog(`🌫️ ${enemy.name} errou o ataque devido à névoa!`);
                    // Animação de erro?
                    currentAttackerIndex++;
                    setTimeout(nextEnemyAttack, 1000);
                    return;
                }
            }

            // --- VERIFICA STUN (Suporte Polvo) ---
            if (enemy.battleState.stunned) {
                addLog(`✨ STUN: ${enemy.name} está atordoado e não pode atacar!`);
                enemy.battleState.stunned = false;
                currentAttackerIndex++;
                setTimeout(nextEnemyAttack, 1000);
                return;
            }

            if (p && !p.battleState) {
                p.battleState = {
                    consecutiveTurns: 0,
                    defenseBuff: 0,
                    tonicoTurns: (p.items && p.items.some(i => i.id === 'tonico_guerra')) ? 3 : 0,
                    targetLock: { index: -1, turns: 0 },
                    turnCount: 0,
                    hasActed: false
                };
            }

            // Bônus Sagrados para Inimigos: Cura 10% no início do seu ataque
            if (enemy.clan === 'sagrados') {
                const heal = Math.floor(enemy.hp * 0.1);
                enemy.currentHp = Math.min(enemy.hp, enemy.currentHp + heal);
                showFloatingValue(`enemy-fighter-${state.enemies.indexOf(enemy)}`, heal, 'heal');
                addLog(`SAGRADOS: ${enemy.name} regenerou ${heal} HP!`);
                updateHpUI(`enemy-${state.enemies.indexOf(enemy)}`, enemy);
            }

            playSound('hit');
            
            // Animação de Ataque do Inimigo
            const enemyIndex = state.enemies.indexOf(enemy);
            const enemyEl = document.getElementById(`enemy-fighter-${enemyIndex}`);
            const enemyCard = enemyEl ? enemyEl.querySelector('.creature-card') : null;
            if (enemyCard) {
                enemyCard.classList.remove('anim-attack-e');
                void enemyCard.offsetWidth;
                enemyCard.classList.add('anim-attack-e');
            }

            // IA: Escolhe o melhor ataque baseado no clã e cooldowns
            let atk;
            enemy.cooldowns = enemy.cooldowns || {};
            
            const attackerClan = CLANS[enemy.clan];
            const isWeak = attackerClan && p.clan && attackerClan.weakAgainst.includes(p.clan);
            
            // Filtra ataques disponíveis (sem cooldown)
            const availableAttacks = enemy.attacks.filter(a => (enemy.cooldowns[a.name] || 0) <= 0);
            const pool = availableAttacks.length > 0 ? availableAttacks : enemy.attacks;
            
            if (isWeak) {
                const ignoreAtks = pool.filter(a => a.ignoreClans);
                if (ignoreAtks.length > 0 && Math.random() < 0.7) {
                    atk = ignoreAtks[Math.floor(Math.random() * ignoreAtks.length)];
                    addLog(`🧠 ESTRATÉGIA: ${enemy.name} usa um ataque que ignora fraquezas!`);
                } else {
                    atk = pool[Math.floor(Math.random() * pool.length)];
                }
            } else {
                atk = pool[Math.floor(Math.random() * pool.length)];
            }
            
            // --- APLICA CUSTOS DE ATAQUE DO INIMIGO ---
            if (atk.cooldown) {
                enemy.cooldowns[atk.name] = atk.cooldown;
            }
            if (atk.recoil) {
                const recoilDmg = Math.floor(enemy.hp * atk.recoil);
                enemy.currentHp -= recoilDmg;
                showFloatingValue(`enemy-fighter-${enemyIndex}`, recoilDmg, 'damage');
                addLog(`⚠️ RECUO: ${enemy.name} recebeu ${recoilDmg} de dano pelo impacto!`);
                
                if (enemy.currentHp <= 0) handleEnemyDeath(enemy); // Inimigo morre por recuo

                updateHpUI(`enemy-${enemyIndex}`, enemy);
                const eEl = document.getElementById(`enemy-fighter-${enemyIndex}`);
                const eCard = eEl ? eEl.querySelector('.creature-card') : null;
                if (eCard) {
                    const fxRecoil = document.createElement('div');
                    fxRecoil.className = 'attack-effect-recoil';
                    eCard.appendChild(fxRecoil);
                    setTimeout(() => {
                        if (fxRecoil.parentNode) fxRecoil.parentNode.removeChild(fxRecoil);
                    }, 450);
                }
            }

            // Dano Base
            let dmg = Math.floor(Math.random() * (atk.max - atk.min + 1)) + atk.min;

            // --- SUPORTE SERPENTE: Toxina Debilitante ---
            if (enemy.battleState.maxDmgDebuff && enemy.battleState.maxDmgDebuff.turns > 0) {
                dmg = Math.max(atk.min, dmg - enemy.battleState.maxDmgDebuff.value);
            }

            // Correntes do Esquecimento: -5% dano por acúmulo
            if (enemy.battleState.damageReduction > 0) {
                dmg = Math.floor(dmg * (1 - enemy.battleState.damageReduction));
            }

            // Bônus Primordiais para Inimigos: +20% Dano
            if (enemy.clan === 'primordiais') {
                dmg = Math.floor(dmg * 1.2);
            }

            // Fraqueza Elemental
            if (isWeak) {
                if (!atk.ignoreClans) {
                    dmg = Math.floor(dmg * 0.7);
                    addLog(`🛡️ RESISTÊNCIA: ${p.name} resistiu ao ataque de ${enemy.name}!`);
                }
            } else if (p.clan && attackerClan && attackerClan.strongAgainst.includes(p.clan)) {
                if (!atk.ignoreClans) {
                    dmg = Math.floor(dmg * 1.5);
                    addLog(`💥 FRAQUEZA: ${p.name} é vulnerável a ${enemy.name}!`);
                }
            }

            // --- NÉVOA ENTORPECENTE ---
            if (enemy.battleState.missChance > 0) {
                if (Math.random() < enemy.battleState.missChance) {
                    showFloatingValue(`player-fighter-${pIdx}`, 'MISS', 'damage');
                    addLog(`<span style="color:#4dabff;">🌫️ NÉVOA: ${enemy.name} errou o ataque!</span>`);
                    currentAttackerIndex++;
                    setTimeout(nextEnemyAttack, 1000);
                    return;
                }
            }

            // Esquiva do Jogador (Base + Buffs + Itens)
            let dodgeBonus = 0;
            if (p.battleState && p.battleState.dodgeBuff) {
                dodgeBonus = (typeof p.battleState.dodgeBuff === 'object') ? p.battleState.dodgeBuff.value : p.battleState.dodgeBuff;
            }
            let dodgeChance = 0.05 + dodgeBonus;

            if (p.clan === 'sombras') dodgeChance += 0.20;
            if (p.items && p.items.some(i => i.id === 'capa_invisibilidade')) dodgeChance += 0.15;
            if (p.items && p.items.some(i => i.id === 'manto_esquiva')) dodgeChance += 0.25;
            
            if (Math.random() < dodgeChance) {
                showFloatingValue(`player-fighter-${pIdx}`, 'MISS', 'damage');
                addLog(`💨 ESQUIVA: ${p.name} desviou do ataque!`);
            } else {
                // Verifica Invulnerabilidade
                if (p.battleState && p.battleState.invulnerability) {
                    showFloatingValue(`player-fighter-${pIdx}`, 'BLOCKED', 'heal');
                    addLog(`🛡️ INVULNERÁVEL: ${p.name} não sofreu dano!`);
                    // Não aplica dano
                    currentAttackerIndex++;
                    setTimeout(nextEnemyAttack, 1000);
                    return;
                }

                // Redução de Dano (Defesa do Jogador)
                let defBonus = 0;
                if (p.battleState && p.battleState.damageReductionBuff) {
                    defBonus = (typeof p.battleState.damageReductionBuff === 'object') ? p.battleState.damageReductionBuff.value : p.battleState.damageReductionBuff;
                }
                let reduction = p.battleState ? (p.battleState.defenseBuff || 0) + defBonus : 0;
                if (p.items && p.items.some(i => i.id === 'escudo_espartano')) reduction += 0.15;
                
                dmg = Math.floor(dmg * (1 - Math.min(0.8, reduction)));
                
                // Relógio da Entropia: Turno par: +15% dano recebido
                if (p.items && p.items.some(i => i.id === 'relogio_entropia') && p.battleState.turnCount % 2 === 0) {
                    dmg = Math.floor(dmg * 1.15);
                }

                // --- ESCUDO (Barreira) ---
                let shieldBlocked = 0;
                if (p.battleState.shield && p.battleState.shield.value > 0) {
                    const shieldVal = p.battleState.shield.value;
                    if (dmg <= shieldVal) {
                        p.battleState.shield.value -= dmg;
                        shieldBlocked = dmg;
                        dmg = 0;
                        addLog(`🛡️ Escudo de ${p.name} absorveu todo o dano! (${p.battleState.shield.value} restante)`);
                    } else {
                        p.battleState.shield.value = 0;
                        shieldBlocked = shieldVal;
                        dmg -= shieldVal;
                        p.battleState.shield = null; // Escudo quebrou
                        addLog(`🛡️ Escudo de ${p.name} absorveu ${shieldVal} de dano e quebrou!`);
                    }
                }
                
                if (shieldBlocked > 0) {
                    showFloatingValue(`player-fighter-${pIdx}`, shieldBlocked, 'heal'); // Visual shield block
                }

                p.currentHp -= dmg;
                showFloatingValue(`player-fighter-${pIdx}`, dmg, 'damage');
                addLog(`⚔️ ATAQUE: ${enemy.name} usou ${atk.name} e causou ${dmg} de dano em ${p.name}!`);
                
                // Espinhos
                if (p.items && p.items.some(i => i.id === 'escudo_espinhos')) {
                    const reflect = Math.floor(dmg * 0.1);
                    enemy.currentHp -= reflect;
                    addLog(`🛡️ ESPINHOS: ${enemy.name} recebeu ${reflect} de dano de volta!`);
                    
                    if (enemy.currentHp <= 0) handleEnemyDeath(enemy); // Inimigo morre por espinhos
                    
                    updateHpUI(`enemy-${enemyIndex}`, enemy);
                }

                // Refletir (Skill Support)
                if (p.battleState && p.battleState.reflect) {
                    const reflect = Math.floor(dmg * p.battleState.reflect.value);
                    enemy.currentHp -= reflect;
                    addLog(`🌵 ESPINHOS: ${enemy.name} recebeu ${reflect} de dano refletido!`);
                    if (enemy.currentHp <= 0) handleEnemyDeath(enemy);
                    updateHpUI(`enemy-${enemyIndex}`, enemy);
                }

                // Abissais (Refletir)
                if (p.clan === 'abissais') {
                    const reflect = Math.floor(dmg * 0.25);
                    enemy.currentHp -= reflect;
                    addLog(`🌌 ABISSAIS: ${enemy.name} recebeu ${reflect} de dano refletido!`);
                    
                    if (enemy.currentHp <= 0) handleEnemyDeath(enemy); // Inimigo morre por reflexão

                    updateHpUI(`enemy-${enemyIndex}`, enemy);
                }

                const playerEl = document.getElementById(`player-fighter-${pIdx}`);
                const playerCard = playerEl ? playerEl.querySelector('.creature-card') : null;
                if (playerCard) {
                    playerCard.classList.remove('anim-shake', 'anim-flash');
                    void playerCard.offsetWidth;
                    playerCard.classList.add('anim-shake', 'anim-flash');
                }
                
                // Fênix de Bolso
                if (p.currentHp <= 0 && p.items && p.items.some(i => i.id === 'fenix_bolso')) {
                    const fIdx = p.items.findIndex(i => i.id === 'fenix_bolso');
                    p.items.splice(fIdx, 1);
                    p.currentHp = Math.floor(p.hp * 0.3);
                    addLog(`🔥 FÊNIX: ${p.name} renasceu das cinzas!`);
                }

                if (p.currentHp <= 0) handlePlayerDeath(p); // Jogador morre pelo dano

                updateHpUI(`player-${pIdx}`, p);
            }

            currentAttackerIndex++;
            setTimeout(nextEnemyAttack, 1000);
        }
        
        nextEnemyAttack();
    }

    function winBattle() {
        if (state.isBossBattle) playSound('victory');
        
        let totalXp = 0;
        let totalDiamantes = 0;
        let totalLux = 0;
        
        state.enemies.forEach(e => {
            totalXp += state.isBossBattle ? 500 : 40 + (e.level * 10);
            totalDiamantes += state.isBossBattle ? 200 : 20 + (e.level * 5);
            totalLux += state.isBossBattle ? 50 : 5 + (e.level * 2);
        });

        if (state.challengeActive) {
            totalXp = Math.floor(totalXp * 2);
            totalDiamantes = Math.floor(totalDiamantes * 2);
            totalLux = Math.floor(totalLux * 2);
        }
        if (state.eliteBattleActive) {
            totalDiamantes += 80;
            totalLux += 30;
            const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
            const bonusItem = JSON.parse(JSON.stringify(possibleItems[Math.floor(Math.random() * possibleItems.length)]));
            bonusItem.level = 1;
            state.battleLoot.push(bonusItem);
        }
        state.diamantes += totalDiamantes;
        state.lux += totalLux;
        addLog(`Vitória! +${totalXp} XP | +${totalDiamantes} 💎 | +${totalLux} Lux`);

        // Dá XP para todos os jogadores ativos
        state.activePlayerCreatures.forEach(p => {
            let xpForThisOne = totalXp;
            if (state.relics && state.relics.xpBonus) {
                xpForThisOne = Math.floor(xpForThisOne * (1 + state.relics.xpBonus));
            }
            // Bônus Grimório de Aprendiz (+60% XP)
            if (p.items && p.items.length > 0) {
                let xpBonus = 0;
                p.items.forEach(item => {
                    if (item.id === 'grimorio_aprendiz') xpBonus += 0.6;
                });
                if (xpBonus > 0) {
                    xpForThisOne = Math.floor(xpForThisOne * (1 + xpBonus));
                    addLog(`GRIMÓRIO (${p.name}): +${Math.round(xpBonus * 100)}% de XP bônus!`);
                }
            }
            gainXp(p, xpForThisOne);
        });

        // 1. Chance de Drop Aleatório (Sistema antigo) adicionado à fila
        const dropChance = state.isBossBattle ? 1.0 : 0.1;
        if (Math.random() < dropChance) {
            const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
            const randomDrop = JSON.parse(JSON.stringify(possibleItems[Math.floor(Math.random() * possibleItems.length)]));
            randomDrop.level = 1;
            randomDrop.name += " (Sorte)"; // Marcador visual
            state.battleLoot.push(randomDrop);
        }

        // Inicia o processamento do loot (Itens dos inimigos + Drop aleatório)
        if (state.battleLoot && state.battleLoot.length > 0) {
            processBattleLoot(state.battleLoot, 0, () => {
                finishBattleFlow();
            });
        } else {
            finishBattleFlow();
        }
    }

    function finishBattleFlow() {
        if (state.isBossBattle) { 
            setTimeout(showBossVictory, 1000); 
        } else {
            const lastEnemy = state.enemies[state.enemies.length - 1];
            showModal("Vitória!", `
                <div style="text-align:center;">
                    ${getCreatureImageOnly(lastEnemy, "80px")}<br>
                    Você venceu o combate contra <strong>${lastEnemy.name}</strong>!
                </div>`, [
                { text: "Capturar", action: () => captureCreature(lastEnemy) },
                { text: "Trocar", action: () => swapCreature(lastEnemy) },
                { text: "Deixar ir", action: endEvent }
            ]);
        }
    }

    function fuseItems(targetItem, sourceItem) {
        targetItem.level = (targetItem.level || 1) + 1;
        // O poder dos itens é calculado dinamicamente no combate agora
    }

    function gainXp(creature, amount) {
        creature.xp += amount;
        while (creature.xp >= creature.maxXp) {
            creature.xp -= creature.maxXp;
            levelUp(creature);
        }
        updateUI();
    }

    function levelUp(c) {
        c.level++;
        c.maxXp = Math.floor(c.maxXp * 1.2);
        
        // Casco de Adamante (+100 HP Max) - já deve estar somado se o item for equipado
        // Mas garantimos que o HP aumente ao subir de nível
        c.hp += 20;
        c.currentHp = c.hp;
        c.attacks.forEach(a => { a.min += 2; a.max += 4; });
        addLog(`LEVEL UP! ${c.name} atingiu nível ${c.level}!`);
        
        // Pedra da Evolução Precoce (Evolui 1 nível antes por item)
        let reduction = 0;
        if (c.items) {
            c.items.forEach(item => {
                if (item.id === 'pedra_evolucao') reduction += 2;
            });
        }
        const evoLevel = c.evo ? Math.max(1, c.evo.level - reduction) : 999;
        
        if (!c.isEvolved && c.evo && c.level >= evoLevel) {
            evolve(c);
        }
    }

    function evolve(c, onAfter) {
        playSound('evolution');
        const oldName = c.name;
        c.name = c.evo.name;
        c.emoji = c.evo.emoji;
        if (c.evo.image) c.image = c.evo.image;
        c.hp += c.evo.hpBoost;
        c.currentHp = c.hp;
        c.attacks.forEach(a => { a.min += c.evo.dmgBoost; a.max += c.evo.dmgBoost; });
        c.isEvolved = true;
        
        showModal("EVOLUÇÃO!", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(c, "100px")}
                <strong>${oldName}</strong> evoluiu para <strong>${c.name}</strong>!
            </div>`, [{ text: "Incrível!", action: () => { hideModal(); if (onAfter) onAfter(); } }]);
        addLog(`EVOLUÇÃO! ${oldName} evoluiu para ${c.name}!`);
    }

    function loseBattle() {
        playSound('defeat');
        
        if (state.isBossBattle) {
            // No modo boss, verifica se ainda há alguém vivo no time reserva
            const aliveReserves = state.team.filter(c => !state.activePlayerCreatures.includes(c) && c.currentHp > 0);
            
            if (aliveReserves.length > 0) {
                addLog("Toda a equipe ativa caiu! Escolha reforços!");
                showCreatureSelection((s) => { 
                    // Substitui a primeira criatura morta pela nova (ou apenas reseta se houver lógica complexa)
                    // Para simplificar: se o time de 3 caiu, o jogador pode escolher um para continuar (modo legado adaptado)
                    // Ou idealmente, substituir as 3. Mas vamos manter o fluxo de "escolha um" por enquanto.
                    state.activePlayerCreature = s; 
                    state.activePlayerCreatures = [s]; // Volta para modo single se escolher reforço? 
                    // Na verdade, vamos manter o modo Boss mas com apenas 1 se for reforço.
                    setupBattleUI(); 
                    toggleButtons(false); 
                }, "Quem deve entrar?");
            } else {
                gameOverBoss();
            }
            return;
        }

        const p = state.activePlayerCreature;
        
        // Verifica se a criatura tem auto-revivência
        if (p.battleState && p.battleState.autoRevive) {
            const reviveHp = Math.floor(p.hp * p.battleState.autoRevive.value);
            p.currentHp = reviveHp;
            p.defeated = false;
            addLog(`✨ RENASCIMENTO: ${p.name} renasce com ${reviveHp} HP!`);
            playSound('heal');
            
            // Remove o buff de auto-revivência
            delete p.battleState.autoRevive;
            updateUI();
            return; // Não termina a batalha
        }
        
        addLog(`${p.name} caiu!`);
        // Marca a criatura como derrotada em vez de removê-la da equipe
        p.defeated = true;
        p.currentHp = 0;
        updateUI();
        showModal("Derrota", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(p, "80px")}
                <strong>${p.name}</strong> foi derrotado...
                <div style="margin-top:10px;color:#888;font-size:14px;">
                    Use a skill "Reanimar" para reviver criaturas derrotadas!
                </div>
            </div>`, [{ text: "Continuar", action: () => { stopSound(); endEvent(); } }]);
    }

    // --- Events ---

    function startEncounter(enemy) {
        showModal("Encontro", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(enemy, "80px")}
                Um <strong>${enemy.name}</strong> nível ${enemy.level} apareceu!
            </div>`, [
            { text: "Capturar", action: () => tryCapture(enemy) },
            { text: "Lutar", action: () => startBattle(enemy) },
            { text: "Fugir", action: endEvent }
        ]);
    }

    function tryCapture(enemy) {
        let chance = 0.5;
        // Incenso Atrativo (+30% chance de captura por item)
        let bonus = 0;
        state.team.forEach(c => {
            if (c.items) {
                c.items.forEach(item => {
                    if (item.id === 'incenso_atrativo') bonus += 0.3;
                });
            }
        });
        
        if (bonus > 0) {
            chance = Math.min(0.95, chance + bonus);
            addLog(`INCENSO ATRATIVO: +${Math.round(bonus * 100)}% de chance de captura!`);
        }
        if (state.relics && state.relics.captureBonus) {
            chance = Math.min(0.95, chance + state.relics.captureBonus);
        }
        if (state.curse && state.curse.capturePenalty) {
            chance = Math.max(0.05, chance - state.curse.capturePenalty);
        }

        const roll = Math.random();
        const fleeChanceOnFail = 0.4; // 40% de chance de fuga quando a captura falha

        if (roll < chance) {
            showModal("Sucesso!", `
                <div style="text-align:center;">
                    ${getCreatureImageOnly(enemy, "80px")}
                    Capturou <strong>${enemy.name}</strong>!
                </div>`, [{ text: "Ok", action: () => captureCreature(enemy) }]);
        } else {
            const fleeRoll = Math.random();
            if (fleeRoll < fleeChanceOnFail) {
                showModal("Fugiu!", `
                    <div style="text-align:center;">
                        ${getCreatureImageOnly(enemy, "80px")}
                        <strong>${enemy.name}</strong> escapou antes de ser capturado!
                    </div>`, [{ text: "Ok", action: endEvent }]);
            } else {
                showModal("Falhou!", `
                    <div style="text-align:center;">
                        ${getCreatureImageOnly(enemy, "80px")}
                        A captura falhou e ele decidiu atacar!
                    </div>`, [{ text: "Lutar", action: () => startBattle(enemy) }]);
            }
        }
    }

    function showBossVictory() {
        const clanKey = state.currentBossClan || "sombras";
        const baseBoss = CLAN_BOSSES[clanKey] || CLAN_BOSSES.sombras;
        const capturedBoss = JSON.parse(JSON.stringify(baseBoss));
        capturedBoss.hp = Math.floor(capturedBoss.hp * 0.3);
        capturedBoss.attacks.forEach(a => {
            a.min = Math.floor(a.min * 0.3);
            a.max = Math.floor(a.max * 0.3);
        });
        capturedBoss.name = "Mini " + capturedBoss.name;

        if (typeof state.lux !== "number") state.lux = 0;
        if (typeof state.tcgFreePacks !== "number") state.tcgFreePacks = 0;
        const extraLux = 100;
        state.lux += extraLux;
        state.tcgFreePacks += 2;
        saveGame();

        showModal("VITÓRIA!", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(baseBoss, "100px")}
                Você derrotou o Chefe! Deseja capturá-lo com 30% do poder?<br><br>
                <span style="color:gold;">Recompensas: +${extraLux} Lux e +2 Pacotes TCG Grátis</span>
            </div>`, [
            { text: "Sim, Capturar!", action: () => { 
                captureCreature(capturedBoss, true);
            } },
            { text: "Não, Apenas Continuar", action: continueAfterBoss }
        ]);
    }

    function captureCreature(c, isBoss = false) {
        c.currentHp = c.hp; // Vida cheia ao capturar
        
        const nextAction = isBoss ? continueAfterBoss : endEvent;

        // Desbloqueia a criatura capturada
        state.unlockedCreatures[c.id] = true;
        saveGame(); // Salva o jogo após desbloquear
        
        // Mostra popup de criatura desbloqueada
        showModal("Criatura Desbloqueada!", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(c, "80px")}
                <strong>${c.name}</strong> foi desbloqueada!<br>
                <span style="color:gold;">Agora ela está disponível em sua coleção!</span>
            </div>
        `, [{ text: "Ótimo!", action: hideModal }]);

        // Verifica se já tem uma criatura igual para fusão
        const duplicate = state.team.find(t => t.id === c.id);
        if (duplicate) {
            showModal("Criatura Repetida!", `
                <div style="text-align:center;">
                    ${getCreatureImageOnly(c, "80px")}
                    Você já tem um <strong>${c.name}</strong>. O que deseja fazer?
                </div>`, [
                { text: "Fundir (Power Up!)", action: () => { fuseCreatures(duplicate, c); nextAction(); } },
                { text: "Manter Ambos", action: () => addToTeam(c, isBoss) }
            ]);
        } else {
            addToTeam(c, isBoss);
        }
    }



    function addToTeam(c, isBoss = false) {
        const nextAction = isBoss ? continueAfterBoss : endEvent;
        if (state.team.length < state.maxTeamSize) { 
            state.team.push(c); 
            healTeamOnCapture();
            nextAction(); 
        }
        else {
            showModal("Time Cheio", "Trocar uma?", [
                { text: "Trocar", action: () => swapCreature(c, isBoss) }, 
                { text: "Deixar ir", action: nextAction }
            ]);
        }
    }



    function swapCreature(newC, isBoss = false) {
        const nextAction = isBoss ? continueAfterBoss : endEvent;
        showCreatureSelection((s) => { 
            state.team[state.team.indexOf(s)] = newC; 
            nextAction(); 
        }, `
            <div style="text-align:center;">
                ${getCreatureImageOnly(newC, "64px")}
                Remover qual para dar lugar a <strong>${newC.name}</strong>?
            </div>`);
    }

    function fuseCreatures(existing, newC) {
        existing.level += 1;
        existing.hp += 30;
        existing.currentHp = existing.hp;
        existing.attacks.forEach(a => { a.min += 5; a.max += 8; });
        
        if (newC.level > 1) {
            existing.xp += 50;
            if (existing.xp >= existing.maxXp) gainXp(existing, 0);
        }
        
        addLog(`FUSÃO! ${existing.name} ficou mais forte!`);
        healTeamOnCapture();
    }

    function healTeamOnCapture() {
        state.team.forEach(creature => {
            const healAmount = creature.hp * 0.25;
            creature.currentHp = Math.min(creature.hp, creature.currentHp + healAmount);
        });
        addLog("Sua equipe recuperou um pouco de vida!");
    }

    function handleTreasure() {
        showModal("Baú de Tesouro", "Você encontrou um baú misterioso 🍀! Deseja abri-lo?", [
            { text: "Abrir Baú", action: openTreasure },
            { text: "Deixar para lá", action: endEvent }
        ]);
    }

    function startEliteBattle() {
        const e = getRandomEnemy();
        e.hp = Math.floor(e.hp * 1.3);
        e.attacks.forEach(a => { a.min = Math.floor(a.min * 1.2); a.max = Math.floor(a.max * 1.2); });
        if (!e.battleState) e.battleState = {};
        e.battleState.damageReduction = 0.15;
        state.eliteBattleActive = true;
        startBattle(e, false);
    }

    function handleChaosEvent() {
        let startedBattle = false;
        const effects = [
            () => { state.team.forEach(c => c.currentHp = Math.min(c.hp, Math.floor(c.currentHp + c.hp * 0.25))); addLog("Cura parcial aplicada."); return false; },
            () => { state.team.forEach(c => { if (c.currentHp > 0) { c.trapPoisonTurns = 2; c.trapPoisonDmg = 20; } }); addLog("Armadilha leve aplicada."); return false; },
            () => { state.team.forEach(c => { c.hp += 10; c.currentHp = Math.min(c.hp, c.currentHp + 10); c.attacks.forEach(a => { a.min += 1; a.max += 2; }); }); addLog("Bênção aplicada."); return false; },
            () => { const enemy = getRandomEnemy(); startBattle(enemy); return true; },
            () => { const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel); const it = JSON.parse(JSON.stringify(possibleItems[Math.floor(Math.random() * possibleItems.length)])); it.level = 1; addItemToInventory(it); addLog("Item obtido."); return false; },
            () => { state.team.forEach(c => { const lose = Math.floor(c.hp * 0.10); c.currentHp = Math.max(1, c.currentHp - lose); }); addLog("Drenagem leve aplicada."); return false; }
        ];
        const picks = [];
        while (picks.length < 2) {
            const f = effects[Math.floor(Math.random() * effects.length)];
            if (!picks.includes(f)) picks.push(f);
        }
        const body = `
            <div style="font-size:0.85rem; text-align:left;">
                Dois efeitos aleatórios acontecerão entre:
                <br>• Cura parcial da equipe
                <br>• Armadilha de veneno leve
                <br>• Bênção (buff leve)
                <br>• Batalha surpresa
                <br>• Item aleatório
                <br>• Drenagem leve de HP
            </div>
        `;
        showModal("Evento Caótico", body, [
            { text: "Ok", action: () => { picks.forEach(fn => { if (fn()) startedBattle = true; }); if (!startedBattle) endEvent(); } }
        ]);
    }

    function showWanderingMerchant() {
        const body = `
            <div style="text-align:center;">
                Escolha: comprar com 💎 ou ⚜️, trocar item raro, ou ignorar.
            </div>
        `;
        showModal("Mercador Errante", body, [
            { text: "Comprar (120 💎)", action: () => {
                if (state.diamantes >= 120) {
                    state.diamantes -= 120;
                    const it = randomShopItem();
                    addItemToInventory(it);
                    addLog(`Comprou ${it.name}.`);
                    playSound('shop');
                    showMerchantPurchaseResult(it);
                } else {
                    alert("Diamantes insuficientes!");
                }
            } },
            { text: "Comprar (40 ⚜️)", action: () => {
                if (state.lux >= 40) {
                    state.lux -= 40;
                    const it = randomShopItem();
                    addItemToInventory(it);
                    addLog(`Comprou ${it.name}.`);
                    playSound('shop');
                    showMerchantPurchaseResult(it);
                } else {
                    alert("Lux insuficiente!");
                }
            } },
            { text: "Trocar Raro", action: () => merchantTradeRare() },
            { text: "Ignorar", action: endEvent }
        ], true);
    }

    function randomShopItem() {
        const pool = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
        const found = JSON.parse(JSON.stringify(pool[Math.floor(Math.random() * pool.length)]));
        found.level = 1;
        return found;
    }

    function showMerchantPurchaseResult(item) {
        const lvl = item.level || 1;
        const imgHtml = item.image ? `
            <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin:0 auto 8px; display:block;" onerror="this.style.display='none'">
        ` : '';
        const body = `
            <div style="text-align:center;">
                ${imgHtml}
                <div style="font-size:1.6rem; margin-bottom:4px;">${item.icon || '🎒'}</div>
                <div style="font-weight:900; margin-bottom:4px;">${item.name}</div>
                <div style="font-size:0.8rem; color:#ffd700; margin-bottom:6px;">${getItemTypeLabel(item.type)} • Lvl ${lvl}</div>
                <div style="font-size:0.8rem; color:#ddd;">${item.desc || ''}</div>
            </div>
        `;
        showModal("Item Comprado", body, [
            { text: "Ok", action: endEvent },
            { text: "Abrir Inventário", action: () => showInventoryModal('all') }
        ]);
    }

    function merchantTradeRare() {
        const rares = state.inventory.filter(i => i.type === 'rare');
        if (rares.length === 0) {
            showModal("Troca", "Você não tem itens raros.", [{ text: "Ok", action: showWanderingMerchant }]);
            return;
        }
        const f = document.getElementById('modal-footer');
        f.innerHTML = '';
        rares.forEach(it => {
            const btn = document.createElement('button');
            btn.innerText = `Trocar ${it.name}`;
            btn.onclick = () => {
                removeItemFromInventory(it.uid);
                const newIt = randomShopItem();
                addItemToInventory(newIt);
                addLog(`Trocou por ${newIt.name}.`);
                endEvent();
            };
            f.appendChild(btn);
        });
    }

    function openSupportSelectionForCreature(creature) {
        if (!creature) return;
        const skills = creature.supportSkills || (creature.supportSkill ? [creature.supportSkill] : []);
        if (!skills || skills.length === 0) {
            addLog(`${creature.name} não possui habilidades de suporte.`);
            return;
        }

        const playerIndex = (state.activePlayerCreatures || []).indexOf(creature);

        if (state.challengeNoReserve && playerIndex < 0) {
            addLog("Este desafio não permite suporte da reserva.");
            return;
        }

        creature.cooldowns = creature.cooldowns || {};

        const body = `
            <div style="text-align:center; margin-bottom:8px; border-bottom: 1px solid rgba(124,252,0,0.3); padding-bottom: 10px;">
                <div style="font-size: 0.85rem; text-align: left;">${getCreatureTooltip(creature)}</div>
                <div style="font-weight:600; color:#7CFC00; margin-top: 10px;">Escolha uma habilidade:</div>
            </div>
        `;

        const buttons = [];

        skills.forEach((skill, index) => {
            const cd = creature.cooldowns[skill.name] || 0;
            const label = cd > 0 ? `${skill.name} (${cd})` : `✨ ${skill.name}`;
            buttons.push({
                text: label,
                action: () => {
                    hideModal();
                    if (cd > 0) {
                        addLog(`⏰ ${skill.name} está em recarga (${cd})!`);
                        return;
                    }
                    if (playerIndex >= 0) {
                        activateOwnSupportSkill(creature, playerIndex, index);
                    } else {
                        activateSupportSkill(creature, index);
                    }
                }
            });
        });

        buttons.push({ text: "Cancelar", action: hideModal });

        showModal("Suporte", body, buttons);
    }

    function showAncientAltar() {
        if (state.team.length <= 1) {
            endEvent();
            return;
        }
        showModal("Altar Antigo", "Escolha um sacrifício.", [
            { text: "HP da equipe", action: () => { state.team.forEach(c => { const dmg = Math.floor(c.hp * 0.15); c.currentHp = Math.max(1, c.currentHp - dmg); }); state.permanentBuffs = state.permanentBuffs || {}; state.permanentBuffs.atkBonus = (state.permanentBuffs.atkBonus || 0) + 0.05; addLog("Bônus permanente pequeno concedido."); endEvent(); } },
            { text: "Sacrificar criatura", action: () => {
                showCreatureSelection((s) => {
                    state.team.splice(state.team.indexOf(s), 1);
                    addLog(`⚰️ ${s.name} foi sacrificado no altar...`);
                    showAltarItemRewardSelection();
                }, "Quem será sacrificado?");
            } },
            { text: "Ignorar", action: endEvent }
        ]);
    }

    function showAltarItemRewardSelection() {
        const bossItemIds = ['coroa_demonio', 'fenix_bolso', 'espelho_mistico'];
        const allItems = Object.values(ITEMS);
        const pool = allItems.filter(it => !bossItemIds.includes(it.id));

        if (pool.length === 0) {
            addLog("O altar não tem itens disponíveis no momento.");
            endEvent();
            return;
        }

        const body = `
            <div style="text-align:center; margin-bottom:6px;">
                O altar aceita o sacrifício e oferece um item à sua escolha.
                <br><span style="color:#ffd700; font-size:0.8rem;">Todos vêm no nível 5 (exclui itens de chefes).</span>
            </div>
        `;

        showModal("Recompensa do Sacrifício", body, [
            { text: "Cancelar", action: endEvent }
        ]);

        const f = document.getElementById('modal-footer');
        f.innerHTML = '';

        pool.forEach(item => {
            const card = document.createElement('div');
            card.className = 'creature-card inventory-item-card tooltip';
            const lvl = 5;
            card.innerHTML = `
                <div class="creature-img-container" style="position:absolute; top:0; left:0; width:100%; height:100%;">
                    <img src="imagens/${item.image}" class="creature-img" style="object-fit:contain;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 3rem; opacity: 0.25;">${item.icon || '🎁'}</div>
                </div>
                <div class="creature-card-info" style="padding: 18px 8px 10px 8px;">
                    <div class="creature-card-name" style="font-size:0.9rem;">${item.name}</div>
                    <div class="creature-card-stats" style="font-size:0.65rem; color:#fff;">${getItemTypeLabel(item.type)} • Lvl ${lvl}</div>
                </div>
                <span class="tooltiptext"><strong>${item.name} (Lvl ${lvl})</strong><br><small style="color:#ccc;">${item.desc || ''}</small></span>
            `;
            card.onclick = () => {
                const inst = createItemInstance(item, lvl);
                addItemToInventory(inst);
                addLog(`🩸 O altar concedeu ${inst.name} (Lvl ${lvl}) como recompensa pelo sacrifício.`);
                endEvent();
            };
            f.appendChild(card);
        });
    }

    function startAmbush() {
        state.ambushActive = true;
        const enemyCount = Math.min(3, 1 + Math.floor(Math.random() * 2));
        const enemies = [];
        for (let i = 0; i < enemyCount; i++) enemies.push(getRandomEnemy());
        state.team.forEach(c => { if (!c.battleState) c.battleState = {}; c.battleState.dodgeBuff = (c.battleState.dodgeBuff || 0) - 0.10; });
        startBattle(enemies, false);
    }

    function handleLostRelic() {
        showModal("Relíquia Perdida", "Escolha uma relíquia.", [
            { text: "+5% XP", action: () => { state.relics = state.relics || {}; state.relics.xpBonus = 0.05; endEvent(); } },
            { text: "+5% captura", action: () => { state.relics = state.relics || {}; state.relics.captureBonus = 0.05; endEvent(); } },
            { text: "-1 cooldown (mínimo 1)", action: () => { state.relics = state.relics || {}; state.relics.cooldownReduction = 1; endEvent(); } }
        ]);
    }

    function applyCurse() {
        state.curse = { hpPenalty: 0.10, cooldownPlusOne: true, capturePenalty: 0.10 };
        addLog("Uma maldição persistente cai sobre você.");
        endEvent();
    }

    function showFutureVision() {
        const grid = document.getElementById('exploration-grid');
        const cards = Array.from(grid.children).filter(c => !c.classList.contains('revealed'));
        const reveal = cards.slice(0, Math.min(3, cards.length));
        const summary = reveal.map((c, i) => `${i + 1}: ${c.dataset.type}`).join('<br>');
        showModal("Visão do Futuro", `<div style="text-align:center;">${summary}</div>`, [
            { text: "Trocar 1↔2", action: () => { if (reveal.length >= 2) { const a = reveal[0].dataset.type; reveal[0].dataset.type = reveal[1].dataset.type; reveal[1].dataset.type = a; } endEvent(); } },
            { text: "Remover armadilha", action: () => { const t = reveal.find(c => c.dataset.type === 'TRAP'); if (t) t.dataset.type = 'TREASURE'; endEvent(); } },
            { text: "Fechar", action: endEvent }
        ]);
    }

    function startWildCall() {
        showModal("Chamado Selvagem", "Escolha lutar ou fugir.", [
            { text: "Lutar", action: () => { const count = 2 + Math.floor(Math.random() * 3); const enemies = []; for (let i = 0; i < count; i++) { const e = getRandomEnemy(); e.hp = Math.floor(e.hp * 0.6); e.attacks.forEach(a => { a.min = Math.floor(a.min * 0.6); a.max = Math.floor(a.max * 0.6); }); e.currentHp = e.hp; enemies.push(e); } startBattle(enemies, false); } },
            { text: "Fugir", action: endEvent }
        ]);
    }

    function showChallengeCard() {
        showModal("Desafio", "Escolha uma condição.", [
            { text: "Sem suporte", action: () => { state.challengeActive = true; state.challengeNoReserve = true; startBattle(getRandomEnemy(), false); } },
            { text: "Cooldowns +1", action: () => { state.challengeActive = true; state.challengeCooldownPlus = 1; state.curse = state.curse || {}; state.curse.cooldownPlusOne = true; startBattle(getRandomEnemy(), false); } },
            { text: "Inimigos mais rápidos", action: () => { state.challengeActive = true; state.challengeEnemiesFast = true; startBattle(getRandomEnemy(), false); } },
            { text: "Cancelar", action: endEvent }
        ]);
    }

    function openTreasure() {
        playSound('treasure');
        const r = Math.random();
        
        // Bolsa de Moedas (+100 diamantes)
        let bonusDiamantes = 0;
        state.team.forEach(c => {
            if (c.items && c.items.length > 0) {
                c.items.forEach(item => {
                    if (item.id === 'bolsa_moedas') bonusDiamantes += 100;
                });
            }
        });
        if (bonusDiamantes > 0) {
            addLog(`BOLSA DE MOEDAS: +${bonusDiamantes} diamantes bônus!`);
        }

        if (r < 0.25) {
            // Ganha Criatura (Ovo)
            const c = getRandomEnemy();
            showModal("Ovo encontrado!", `
                <div style="text-align:center;">
                    ${getCreatureImageOnly(c, "80px")}<br>
                    Ganhou um <strong>${c.name}</strong>!
                </div>`, [{ text: "Chocar", action: () => captureCreature(c) }]);
        
        } else if (r < 0.6) {
            // Ganha Diamantes
            const g = 50 + (state.level * 10) + bonusDiamantes;
            state.diamantes += g;
            showModal("Diamantes!", `Encontrou um baú com ${g} 💎!`, [{ text: "Pegar", action: endEvent }]);
        
        } else if (r < 0.8) {
            // Ganha Item
            const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
            const foundItem = JSON.parse(JSON.stringify(possibleItems[Math.floor(Math.random() * possibleItems.length)]));
            foundItem.level = 1;
            
            showModal("Item Encontrado!", `
                <div style="text-align:center;">
                    <img src="imagens/${foundItem.image}" style="width:64px; height:64px; object-fit:contain; margin-bottom:10px;"><br>
                    <strong>${foundItem.name}</strong><br>
                    <small>${foundItem.desc}</small>
                </div>`, [
                { text: "Equipar / Usar", action: () => {
                     // Lógica simplificada de equipar direta
                     addItemToInventory(foundItem);
                     addLog(`Você encontrou ${foundItem.name} e guardou na mochila.`);
                     endEvent();
                }},
                { text: "Guardar", action: () => {
                    addItemToInventory(foundItem);
                    addLog(`Você guardou ${foundItem.name} no inventário.`);
                    endEvent();
                }}
            ]);

        } else {
            // Cura o time
            state.team.forEach(c => { 
                c.hp += 10; c.currentHp = c.hp; 
                c.attacks.forEach(a => { a.min += 2; a.max += 2; });
            });
            showModal("Bênção", "Uma luz sagrada curou e fortaleceu seu time!", [{ text: "Amém!", action: endEvent }]);
        }
    }

    function openMysteryBox() {
        playSound('treasure');
        let bonusDiamantes = 0;
        if (Math.random() < 0.7) {
            bonusDiamantes = 1 + Math.floor(Math.random() * 75);
            state.diamantes += bonusDiamantes;
        }
        const itemDrops = [];
        const possibleItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);
        for (let i = 0; i < 3; i++) {
            if (Math.random() < 0.5) {
                const foundItem = JSON.parse(JSON.stringify(possibleItems[Math.floor(Math.random() * possibleItems.length)]));
                foundItem.level = 1;
                itemDrops.push(foundItem);
            }
        }
        if (bonusDiamantes === 0 && itemDrops.length === 0) {
            showModal("Caixa Vazia", "A caixa não continha nada...", [
                { text: "Ok", action: showShop }
            ]);
            return;
        }
        if (itemDrops.length === 0) {
            showModal("Recompensa", `Você ganhou ${bonusDiamantes} 💎 da caixa!`, [
                { text: "Ok", action: showShop }
            ]);
            return;
        }
        let header = "";
        if (bonusDiamantes > 0) header = `Você ganhou ${bonusDiamantes} 💎 e encontrou itens!`;
        else header = "Você encontrou itens na caixa!";
        equipBoxItemsSequentially(itemDrops, 0, header);
    }

    function equipBoxItemsSequentially(items, index, headerText) {
        if (index >= items.length) {
            showModal("Caixa Aberta", headerText, [
                { text: "Voltar à Loja", action: showShop }
            ]);
            return;
        }
        const foundItem = items[index];
        showModal("Item da Caixa", `
            <div style="text-align:center;">
                <img src="imagens/${foundItem.image}" style="width:64px; height:64px; object-fit:contain; margin-bottom:10px;"><br>
                <strong>${foundItem.name}</strong><br>
                <small>${foundItem.desc}</small>
            </div>`, [
            { text: (foundItem.type === 'consumable' ? 'Usar agora' : 'Equipar'), action: () => {
                if (foundItem.type === 'consumable') {
                    const fail = (msg) => {
                        showModal('Não foi possível usar', msg, [
                            { text: 'Ok', action: () => equipBoxItemsSequentially(items, index, headerText) }
                        ]);
                    };
                    tryUseEvolutionCrystal(foundItem, () => equipBoxItemsSequentially(items, index + 1, headerText), fail);
                    return;
                }
                showCreatureSelection((target) => {
                    if (!target.items) target.items = [];
                    const maxSlots = getMaxItemSlotsForLevel(target.level);
                    
                    if (target.items.length >= maxSlots) {
                        showModal("Espaços Cheios", `Escolha um item para substituir ou fundir (se for igual):`, [
                            ...target.items.map((it, idx) => ({
                                text: it.id === foundItem.id ? `Fundir (+Power)` : `Subst. ${it.name}`,
                                action: () => {
                                    if (it.id === foundItem.id) {
                                        fuseItems(it, foundItem);
                                        addLog(`${it.name} subiu de nível!`);
                                    } else {
                                        target.items[idx] = foundItem;
                                        addLog(`${target.name} equipou ${foundItem.name}!`);
                                    }
                                    equipBoxItemsSequentially(items, index + 1, headerText);
                                }
                            })),
                            { text: "Voltar", action: () => equipBoxItemsSequentially(items, index, headerText) }
                        ]);
                        return;
                    }
                    
                    target.items.push(foundItem);
                    addLog(`${target.name} equipou ${foundItem.name}!`);
                    equipBoxItemsSequentially(items, index + 1, headerText);
                }, "Quem deve equipar?");
            }},
            { text: "Guardar", action: () => {
                addItemToInventory(foundItem);
                addLog(`Você guardou ${foundItem.name} no inventário.`);
                equipBoxItemsSequentially(items, index + 1, headerText);
            }},
            { text: "Ignorar", action: () => equipBoxItemsSequentially(items, index + 1, headerText) }
        ]);
    }

    function endEvent() {
        hideModal();
        
        // Gerencia o descanso das criaturas
        state.team.forEach(c => {
            if (c.restCount > 0) c.restCount--;
            if (c.battleState && c.battleState.hpOriginal) {
                c.hp = c.battleState.hpOriginal;
                c.currentHp = Math.min(c.currentHp, c.hp);
                delete c.battleState.hpOriginal;
            }
        });

        saveGame();
        restoreStandardBattleUI();
        document.getElementById('battle-screen').style.display = 'none';
        document.body.classList.remove('battle-mode');
        document.getElementById('exploration-grid').classList.remove('hidden');
        updateUI();
        if (state.cardsRevealed >= getGridCardCount()) { 
            if (state.level < 50) {
                setTimeout(showShop, 500);
            } else {
                // Se já estiver no 50 (fim do jogo), não faz nada especial aqui
            }
        }
        state.ambushActive = false;
        state.eliteBattleActive = false;
        state.challengeActive = false;
        state.challengeNoReserve = false;
        state.challengeEnemiesFast = false;
        state.challengeCooldownPlus = 0;
    }

    function showShop() {
        const shopTitle = `Loja do Nível ${state.level}`;
        const shopBody = state.selectingLock ? 
            "<strong style='color: gold;'>MODO DE TRAVA: Clique no item que deseja fixar!</strong>" : 
            "Bem-vindo! Use seus diamantes para fortalecer sua equipe.";
        const rerollCost = 20;
        
        showModal(shopTitle, shopBody, [
            { text: `Novas Escolhas (${rerollCost} 💎)`, action: () => {
                if (state.diamantes >= rerollCost) {
                    state.diamantes -= rerollCost;
                    state.currentShopItems = null; // Limpa para gerar novos
                    updateUI();
                    showShop();
                    addLog("Loja renovada!");
                } else {
                    alert("Diamantes insuficientes!");
                }
            }},
            { text: state.lockedShopItem ? "Destravar Item" : "Travar Item", action: () => {
                if (state.lockedShopItem) {
                    state.lockedShopItem = null;
                    addLog("Item destravado!");
                    showShop();
                } else {
                    state.selectingLock = !state.selectingLock;
                    showShop();
                }
            }},
            { text: "Próximo Nível", action: () => { 
                hideModal(); 
                state.level++; 
                grantFreePackForLevel();
                state.currentShopItems = null; // Limpa para o próximo nível
                generateLevel(); 
            } }
        ], true);

        const f = document.getElementById('modal-footer');
        f.innerHTML = '';

        const healCost = 40 + (state.level * 10);
        createShopItem("Poção Grupal", "Cura 50% de HP de todos.", healCost, "skill_heal.png", "🧪", () => {
            if (state.selectingLock) return false;
            state.team.forEach(c => c.currentHp = Math.min(c.hp, c.currentHp + (c.hp * 0.5)));
            addLog("Equipe curada pela poção!");
            updateUI();
        });

        const boxCost = 150;
        createShopItem("Caixa Misteriosa", "Abra e receba até 3 equipamentos e até 75 💎.", boxCost, "caixa_misteriosa.png", "🎁", () => {
            if (state.selectingLock) return false;
            if (state.diamantes < boxCost) return false;
            state.diamantes -= boxCost;
            updateUI();
            openMysteryBox();
            return false;
        });

        if (!state.currentShopItems) {
            const allItems = Object.values(ITEMS).filter(i => !i.minLevel || state.level >= i.minLevel);

            const shopConsumables = allItems.filter(i => i.type === 'consumable');
            const shopEquipments = allItems.filter(i => i.type !== 'consumable');

            let selected = [];

            // Se houver um item travado, ele ocupa a primeira vaga
            if (state.lockedShopItem) {
                const lockedInPool = allItems.find(i => i.id === state.lockedShopItem.id);
                selected.push(lockedInPool || state.lockedShopItem);
            }

            const pickUnique = (pool) => {
                const available = pool.filter(it => !selected.some(s => s.id === it.id));
                if (available.length === 0) return null;
                return available[Math.floor(Math.random() * available.length)];
            };

            // Chance de aparecer consumível na loja (raro)
            const consumableChance = 0.22;

            while (selected.length < 3) {
                const useConsumable = shopConsumables.length > 0 && Math.random() < consumableChance;
                let candidate = useConsumable ? pickUnique(shopConsumables) : pickUnique(shopEquipments);

                if (!candidate) {
                    // Fallback se não tiver mais opções
                    candidate = pickUnique(allItems);
                }

                if (!candidate) break;
                selected.push(candidate);
            }

            state.currentShopItems = selected;
        }

        state.currentShopItems.forEach((item, idx) => {
            const isLocked = state.lockedShopItem && state.lockedShopItem.id === item.id;
            const nameDisplay = isLocked ? `🔒 ${item.name}` : item.name;

            createShopItem(nameDisplay, item.desc, item.cost, item.image, null, () => {
                // Se estiver no modo de seleção de trava
                if (state.selectingLock) {
                    state.lockedShopItem = item;
                    state.selectingLock = false;
                    addLog(`Item ${item.name} travado para a próxima visita!`);
                    showShop();
                    return false;
                }

                // Consumíveis (Itens): usar ou guardar (não equipa)
                if (item.type === 'consumable') {
                    showModal("Comprar Item", `
                        <div style="text-align:center;">
                            <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                            <div style="font-weight:900; color:gold;">${item.name}</div>
                            <div style="font-size:0.85rem; color:#ddd; margin-top:6px;">${item.desc}</div>
                            <div style="margin-top:10px; font-weight:900; color:#ffd700;">${item.cost} 💎</div>
                            <div style="margin-top:6px; font-size:0.75rem; color:#aaa;">Você pode usar agora ou guardar no inventário.</div>
                        </div>
                    `, [
                        { text: 'Usar agora', action: () => {
                            if (state.diamantes < item.cost) {
                                alert('Diamantes insuficientes!');
                                return;
                            }

                            const boughtItem = createItemInstance(item, 1);

                            const finalizePurchase = () => {
                                state.diamantes -= item.cost;
                                if (isLocked) state.lockedShopItem = null;
                                state.currentShopItems.splice(idx, 1);
                                updateUI();
                                showShop();
                            };

                            const fail = (msg) => {
                                showModal('Não foi possível usar', msg, [
                                    { text: 'Ok', action: () => showShop() }
                                ]);
                            };

                            if (isEvolutionCrystal(boughtItem)) {
                                tryUseEvolutionCrystal(boughtItem, () => finalizePurchase(), fail);
                            } else {
                                fail('Este item ainda não tem efeito implementado.');
                            }
                        }},
                        { text: 'Guardar', action: () => {
                            if (state.diamantes < item.cost) {
                                alert('Diamantes insuficientes!');
                                return;
                            }
                            const boughtItem = createItemInstance(item, 1);
                            addItemToInventory(boughtItem);
                            state.diamantes -= item.cost;
                            if (isLocked) state.lockedShopItem = null;
                            state.currentShopItems.splice(idx, 1);
                            addLog(`Você guardou ${item.name} no inventário.`);
                            updateUI();
                            showShop();
                        }},
                        { text: 'Voltar', action: () => showShop() }
                    ]);
                    return false;
                }

                showModal("Comprar Item", `
                    <div style="text-align:center;">
                        <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                        <div style="font-weight:900; color:gold;">${item.name}</div>
                        <div style="font-size:0.85rem; color:#ddd; margin-top:6px;">${item.desc}</div>
                        <div style="margin-top:10px; font-weight:900; color:#ffd700;">${item.cost} 💎</div>
                        <div style="margin-top:6px; font-size:0.75rem; color:#aaa;">Você pode equipar agora ou guardar no inventário.</div>
                    </div>
                `, [
                    { text: "Equipar", action: () => {
                        showCreatureSelection((target) => {
                            if (!target.items) target.items = [];
                            const maxSlots = getMaxItemSlotsForLevel(target.level);

                            const boughtItem = createItemInstance(item, 1);

                            const finalizePurchase = () => {
                                state.diamantes -= item.cost;
                                if (isLocked) state.lockedShopItem = null;
                                state.currentShopItems.splice(idx, 1);
                                updateUI();
                                showShop();
                            };

                            if (target.items.length >= maxSlots) {
                                showModal("Espaços Cheios", `Escolha um item para substituir ou fundir (se for igual):`, [
                                    ...target.items.map((it, itIdx) => ({
                                        text: it.id === boughtItem.id ? `Fundir (+Power)` : `Subst. ${it.name}`,
                                        action: () => {
                                            if (it.id === boughtItem.id) {
                                                fuseItems(it, boughtItem);
                                                addLog(`${it.name} subiu de nível!`);
                                            } else {
                                                // Move item antigo para o inventário
                                                removeImmediateItemEffects(target, it);
                                                const old = target.items[itIdx];
                                                target.items[itIdx] = boughtItem;
                                                applyImmediateItemEffects(target, boughtItem);
                                                addItemToInventory(old);
                                                addLog(`Item antigo foi para o inventário.`);
                                            }
                                            addLog(`${target.name} equipou ${boughtItem.name}!`);
                                            finalizePurchase();
                                        }
                                    })),
                                    { text: "Voltar", action: () => showShop() }
                                ]);
                                return;
                            }

                            applyImmediateItemEffects(target, boughtItem);
                            target.items.push(boughtItem);
                            addLog(`${target.name} equipou ${item.name}!`);
                            finalizePurchase();
                        }, "Quem deve equipar este item?");
                    }},
                    { text: "Guardar", action: () => {
                        const boughtItem = createItemInstance(item, 1);
                        addItemToInventory(boughtItem);
                        state.diamantes -= item.cost;
                        if (isLocked) state.lockedShopItem = null;
                        state.currentShopItems.splice(idx, 1);
                        addLog(`Você guardou ${item.name} no inventário.`);
                        updateUI();
                        showShop();
                    }},
                    { text: "Voltar", action: () => showShop() }
                ]);
                return false;
            });
        });

        // --- Treinos ---
        const atkCost = 60 + (state.level * 15);
        createShopItem("Treino Força", "+5 Dano (Min/Max) p/ um", atkCost, "skill_atk.png", "⚔️", () => {
            if (state.selectingLock) return false;
            showCreatureSelection((c) => {
                c.attacks.forEach(a => { a.min += 8; a.max += 8; });
                state.diamantes -= atkCost;
                addLog(`${c.name} aumentou seu dano!`);
                updateUI();
                showShop();
            }, "Escolha quem treinar:");
            return false;
        });

        const hpCost = 50 + (state.level * 10);
        createShopItem("Vida Máxima", "+40 HP Máximo p/ um", hpCost, "skill_hp.png", "💊", () => {
            if (state.selectingLock) return false;
            showCreatureSelection((c) => {
                c.hp += 40;
                c.currentHp += 40;
                state.diamantes -= hpCost;
                addLog(`${c.name} aumentou sua vida!`);
                updateUI();
                showShop();
            }, "Escolha quem fortalecer:");
            return false;
        });
    }

    function createShopItem(name, desc, cost, image, emoji, action) {
        const f = document.getElementById('modal-footer');
        const d = document.createElement('div');
        d.className = 'creature-card';
        if (state.diamantes < cost && !state.selectingLock) d.style.opacity = '0.5';
        
        let displayHtml = "";
        if (image) {
            displayHtml = `<div class="creature-img-container" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
                <img src="imagens/${image}" class="creature-img" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 2rem; opacity: 0.3;">${emoji || '❓'}</div>
            </div>`;
        } else {
            displayHtml = `<div class="creature-img-container" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center;">
                <span style="font-size: 2rem;">${emoji}</span>
            </div>`;
        }

        d.innerHTML = `
            ${displayHtml}
            <div class="creature-card-info" style="padding: 15px 5px 8px 5px; background: linear-gradient(transparent, rgba(0,0,0,0.9));">
                <div class="creature-card-name" style="font-size: 0.9rem;">${name}</div>
                <div class="creature-card-stats" style="font-size: 0.65rem; margin-bottom: 5px; max-height: 40px; overflow: hidden; line-height: 1.1;">${desc}</div>
                <div style="color: #ffd700; font-weight: bold; font-size: 1rem; text-shadow: 1px 1px 2px black;">${cost} 💎</div>
            </div>
        `;
        
        d.onclick = () => {
            if (state.diamantes >= cost || state.selectingLock) {
                const result = action();
                if (result !== false) {
                    state.diamantes -= cost;
                    updateUI();
                    showShop();
                }
            } else {
                alert("Diamantes insuficiente!");
            }
        };
        f.appendChild(d);
    }

    function startBossBattle() {
        document.getElementById('exploration-grid').classList.add('hidden');
        const bossAppearanceCount = Math.floor(state.level / 10);
        const multiplier = 1 + (bossAppearanceCount - 1) * 0.5;

        let bossClan = "sombras";
        if (state.team && state.team.length > 0) {
            const counts = { primordiais: 0, selvagens: 0, sagrados: 0, sombras: 0, abissais: 0 };
            state.team.forEach(c => {
                if (c && c.clan && counts.hasOwnProperty(c.clan)) {
                    counts[c.clan] += 1;
                }
            });
            let maxClan = bossClan;
            let maxCount = -1;
            Object.keys(counts).forEach(k => {
                if (counts[k] > maxCount) {
                    maxCount = counts[k];
                    maxClan = k;
                }
            });
            if (maxCount > 0) bossClan = maxClan;
        }

        state.currentBossClan = bossClan;

        const baseBoss = CLAN_BOSSES[bossClan] || CLAN_BOSSES.sombras;
        const currentBoss = JSON.parse(JSON.stringify(baseBoss));
        currentBoss.hp = Math.floor(currentBoss.hp * multiplier);
        currentBoss.attacks.forEach(a => {
            a.min = Math.floor(a.min * multiplier);
            a.max = Math.floor(a.max * multiplier);
        });

        const itemLevel = 6 + (bossAppearanceCount - 1);
        currentBoss.items = [
            { ...JSON.parse(JSON.stringify(ITEMS['garra_obsidiana'])), level: itemLevel },
            { ...JSON.parse(JSON.stringify(ITEMS['cajado_runico'])), level: itemLevel },
            { ...JSON.parse(JSON.stringify(ITEMS['coroa_demonio'])), level: itemLevel }
        ];
        
        currentBoss.hp = Math.floor(currentBoss.hp / 2);
        
        const clanName = CLANS[bossClan] ? CLANS[bossClan].name : bossClan;
        showModal(`CHEFE DO CLÃ ${clanName.toUpperCase()} (NÍVEL ${state.level})`, `
            <div style="text-align:center;">
                ${getCreatureImageOnly(currentBoss, "120px")}
                <small>Equipamentos Nível ${itemLevel}</small>
            </div>`, [{ text: "LUTAR", action: () => startBattle(initCreature(currentBoss), true) }]);
    }

    function gameOver() { showModal("FIM", "Perdeu tudo.", [{ text: "Reiniciar", action: () => { location.reload(); } }]); }

    function gameOverBoss() {
        const clanKey = state.currentBossClan || "sombras";
        const baseBoss = CLAN_BOSSES[clanKey] || CLAN_BOSSES.sombras;
        showModal("DERROTA", `
            <div style="text-align:center;">
                ${getCreatureImageOnly(baseBoss, "100px")}
                O Rei venceu. Escolha uma criatura para renascer:
                <div style="margin-top: 10px; font-size: 14px; color: #888;">
                    O jogo recomeçará apenas com a criatura escolhida, mantendo seu nível e itens.
                </div>
            </div>`, []);
        const f = document.getElementById('modal-footer');
        f.innerHTML = '';
        state.team.forEach(c => {
            const d = document.createElement('div'); 
            d.className = 'creature-card';
            d.innerHTML = `
                ${getCreatureDisplay(c)}
                <div class="creature-card-info">
                    <div class="creature-card-name">${c.name}</div>
                    <div class="creature-card-stats">Lvl ${c.level}</div>
                    <div class="creature-card-stats">${c.items ? c.items.length : 0} itens</div>
                </div>
            `;
            d.onclick = () => {
                stopSound();
                
                // Preserva a criatura escolhida com seu nível e itens
                const chosenCreature = JSON.parse(JSON.stringify(c));
                chosenCreature.currentHp = chosenCreature.hp; // Cura completamente
                
                // Remove todos os efeitos de batalha e estados temporários
                if (chosenCreature.battleState) {
                    delete chosenCreature.battleState;
                }
                chosenCreature.trapPoisonTurns = 0;
                chosenCreature.trapParalysisTurns = 0;
                chosenCreature.cooldowns = {};
                
                // Mantém apenas a criatura escolhida no time
                state.team = [chosenCreature];
                
                // Mantém o nível atual (não reduz como antes)
                // state.level permanece o mesmo
                
                // Mantém todos os itens do inventário
                // O inventário já está preservado no state
                
                hideModal(); 
                
                // Mostra mensagem de confirmação
                showModal("RENASCIMENTO", `
                    <div style="text-align:center;">
                        ${getCreatureImageOnly(chosenCreature, "80px")}
                        <div style="margin-top: 10px;">
                            ${chosenCreature.name} renasceu!<br>
                            Nível ${chosenCreature.level} mantido.<br>
                            ${chosenCreature.items ? chosenCreature.items.length : 0} itens preservados.
                        </div>
                    </div>
                `, [
                    { text: "Continuar", action: () => {
                        hideModal(); 
                        initGame(true);
                    }}
                ]);
            };
            f.appendChild(d);
        });
    }

    function continueAfterBoss() {
        if (state.curse) {
            state.curse = null;
            addLog("A maldição foi dissipada.");
        }
        if (state.level >= 50) {
            showModal("GRANDE VITÓRIA!", "Você completou todos os 50 níveis e dominou o mundo das criaturas!", [
                { text: "Jogar Novamente", action: () => location.reload() }
            ]);
            return;
        }

        showModal("Progresso", `Nível ${state.level} concluído! Prepare-se para o próximo desafio.`, [
            { text: "Continuar", action: () => {
                state.level++;
                grantFreePackForLevel();
                generateLevel();
                hideModal();
            }}
        ]);
    }

    // --- UI Utils ---
    function showFloatingValue(elementId, value, type = 'damage') {
        const target = document.getElementById(elementId);
        if (!target) return;

        const rect = target.getBoundingClientRect();
        const container = document.body;
        
        const floating = document.createElement('div');
        floating.className = `floating-value floating-${type}`;
        floating.textContent = type === 'heal' ? `+${value} HP` : value;
        
        // Posicionamento centralizado no elemento alvo
        const x = rect.left + rect.width / 2;
        const y = rect.top + rect.height / 2;
        
        floating.style.left = `${x}px`;
        floating.style.top = `${y}px`;
        
        container.appendChild(floating);
        
        // Remove após a animação
        setTimeout(() => floating.remove(), 1000);
    }

    function getMaxItemSlotsForLevel(level) {
        const lvl = level || 1;
        if (lvl >= 6) return 3;
        if (lvl >= 3) return 2;
        return 1;
    }

    function getCreatureDisplay(c) {
        // Badge do Clã em estilo pílula acima da carta
        let clanHtml = "";
        if (c.clan && CLANS[c.clan]) {
            const clan = CLANS[c.clan];
            clanHtml = `<div class="clan-badge-pill" style="background: ${clan.color};" title="${clan.name}: ${clan.bonus}">
                ${clan.emoji} ${clan.name}
            </div>`;
        }

        // Ícone de Habilidade de Suporte com Tooltip
        let supportHtml = "";
        if (c.supportSkill) {
            const skill = c.supportSkill;
            supportHtml = `<div class="support-skill-icon" title="Habilidade de Suporte: ${skill.name}\n${skill.desc || skill.description}">
                ${skill.icon || '🤝'}
            </div>`;
        }

        const maxSlots = getMaxItemSlotsForLevel(c.level);
        // Slots de Itens Laterais - Movidos para FORA do creature-img-container para garantir visibilidade
        let itemsHtml = `<div class="item-slots-sidebar">`;
        for (let i = 0; i < 3; i++) {
            const item = (c.items && c.items[i]) ? c.items[i] : null;
            if (i >= maxSlots) {
                itemsHtml += `<div class="item-slot locked" title="Slot bloqueado: aumente o nível para liberar"></div>`;
            } else if (item) {
                itemsHtml += `
                    <div class="item-slot" title="${item.name}\nAtk: +${item.atk || 0} | Def: +${item.def || 0} | HP: +${item.hp || 0}">
                        <img src="imagens/${item.image}" onerror="this.parentElement.innerHTML='<span style=\'font-size:12px;\'>${item.icon}</span>'">
                    </div>`;
            } else {
                itemsHtml += `<div class="item-slot empty" title="Slot Vazio"></div>`;
            }
        }
        itemsHtml += `</div>`;

        const imagePath = c.image ? `imagens/${c.image}` : "";
        
        return `
            ${clanHtml}
            ${itemsHtml}
            <div class="creature-img-container">
                ${supportHtml}
                ${c.image ? `
                    <img src="${imagePath}" alt="${c.name}" class="creature-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 3.5rem; opacity: 0.3;">${c.emoji}</div>
                ` : `
                    <div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size: 3.5rem;">${c.emoji}</div>
                `}
            </div>
        `;
    }

    function getCreatureImageOnly(c, size = "100px") {
        const height = parseInt(size) * 1.5 + "px";
        const imagePath = c.image ? `imagens/${c.image}` : "";
        const content = c.image ? 
            `<img src="${imagePath}" style="width:100%; height:100%; object-fit:cover; display:block;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
             <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 2rem; opacity: 0.3;">${c.emoji}</div>` :
            `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size: 2rem; opacity: 0.5;">${c.emoji}</div>`;
            
        const maxSlots = getMaxItemSlotsForLevel(c.level);
        let itemsHtml = `<div class="item-slots-sidebar" style="right: -25px; gap: 4px;">`;
        for (let i = 0; i < 3; i++) {
            const item = (c.items && c.items[i]) ? c.items[i] : null;
            if (i >= maxSlots) {
                itemsHtml += `<div class="item-slot locked" style="width: 18px; height: 18px; border-width: 1px;" title="Slot bloqueado"></div>`;
            } else if (item) {
                itemsHtml += `<div class="item-slot" style="width: 18px; height: 18px; border-width: 1px;"><img src="imagens/${item.image}" style="width:100%; height:100%; object-fit:contain;" onerror="this.parentElement.innerHTML='<span style=\'font-size:8px;\'>${item.icon}</span>'"></div>`;
            } else {
                itemsHtml += `<div class="item-slot empty" style="width: 18px; height: 18px; border-width: 1px;"></div>`;
            }
        }
        itemsHtml += `</div>`;

        return `
            <div class="creature-card mini" style="width: ${size}; height: ${height}; margin: 0 auto; cursor: default; pointer-events: none; overflow: visible;">
                ${itemsHtml}
                <div class="creature-img-container" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">
                    ${content}
                </div>
                <div class="creature-card-info">
                    <div class="creature-card-name" style="font-size: 0.7rem; padding: 2px;">${c.name}</div>
                </div>
            </div>
        `;
    }

    function getCreatureTooltip(c) {
        let attacksInfo = c.attacks ? c.attacks.map(a => `• ${a.name}: ${a.min}-${a.max}${a.ignoreClans ? ' <small>(Ignora Clãs)</small>' : ''}`).join('<br>') : 'Sem ataques';
        let itemsInfo = (c.items && c.items.length > 0) ? `<br>--- Equipamentos ---<br>${c.items.map(i => {
            const lvl = i.level || 1;
            return `<img src="imagens/${i.image}" style="width:20px; vertical-align:middle;"> <strong>${i.name} (Lvl ${lvl})</strong><br><small style="color:#ccc;">${i.desc}</small>`;
        }).join('<br>')}` : '';

        // Suporte para estrutura antiga (supportSkill) e nova (supportSkills)
        const skills = c.supportSkills || (c.supportSkill ? [c.supportSkill] : []);
        let supportInfo = skills.length > 0 ? `<br>--- Habilidades de Suporte ---<br>${skills.map(skill => `<span style="color:#ffd700;">🤝 ${skill.name}</span><br><small style="color:#ccc;">${skill.desc}</small><br><small style="color:#4dabff;">Cooldown: ${skill.cooldown} Turnos</small>`).join('<br>')}` : '';
        
        let tcgInfo = "";
        if (c.attachedEnergy !== undefined) {
            tcgInfo = `<br><span style="color: #00ffff;">⚡ Energia Ligada: ${c.attachedEnergy}</span>`;
        }
        if (c.type === 'evo' && c.baseId) {
            tcgInfo += `<br><small style="color: #ffaa00;">Evolução de: ${c.baseId}</small>`;
        }

        let clanInfo = "";
        if (c.clan && CLANS[c.clan]) {
            const clan = CLANS[c.clan];
            const strong = clan.strongAgainst.map(s => CLANS[s].emoji).join(' ');
            const weak = clan.weakAgainst.map(w => CLANS[w].emoji).join(' ');
            clanInfo = `
                <div style="border-left: 3px solid ${clan.color}; padding-left: 5px; margin-bottom: 5px;">
                    <span style="color:${clan.color}; font-weight:bold;">${clan.emoji} Clã ${clan.name}</span><br>
                    <small style="color:#ccc;">${clan.desc}</small><br>
                    <small style="color:${clan.color};">Bônus: ${clan.bonus}</small><br>
                    <small>💪 Forte contra: ${strong} | 🧊 Fraco contra: ${weak}</small>
                </div>
            `;
        }

        return `
            <strong>${c.name} ${c.level ? `(Lvl ${c.level})` : ''}</strong><br>
            ${clanInfo}
            HP: ${Math.floor(c.currentHp)}/${c.hp}${tcgInfo}<br>
            ${c.xp !== undefined ? `XP: ${c.xp}/${c.maxXp}<br>` : ''}--- Ataques ---<br>
            ${attacksInfo}${itemsInfo}${supportInfo}
        `;
    }

    function showModal(t, b, btns, showDiamonds = false) {
        playSound('click');
        document.getElementById('overlay').style.display = 'flex';
        document.getElementById('modal-title').innerHTML = t;
        
        const modalBody = document.getElementById('modal-body');
        modalBody.innerHTML = ''; // Limpa o conteúdo anterior
        if (typeof b === 'string') {
            modalBody.innerHTML = b;
        } else if (b instanceof HTMLElement) {
            modalBody.appendChild(b);
        }
        
        document.getElementById('modal-actions').innerHTML = '';
        const f = document.getElementById('modal-footer'); f.innerHTML = '';
        
        const diamondHud = document.getElementById('modal-diamonds-hud');
        if (showDiamonds) {
            diamondHud.classList.remove('hidden');
            document.getElementById('modal-diamonds-val').innerText = state.diamantes;
        } else {
            diamondHud.classList.add('hidden');
        }

        btns.forEach(btn => {
            const el = document.createElement('button'); el.innerText = btn.text; el.onclick = btn.action;
            document.getElementById('modal-actions').appendChild(el);
        });
    }

    function hideModal() { 
        playSound('click');
        document.getElementById('overlay').style.display = 'none'; 
    }

    // --- Inventory UI ---
    function getItemTypeLabel(type) {
        if (type === 'offense') return '⚔️ Ofensivo';
        if (type === 'defense') return '🛡️ Defensivo';
        if (type === 'utility') return '🧰 Utilitário';
        if (type === 'rare') return '⭐ Raro';
        if (type === 'consumable') return '🧿 Item';
        return '📦 Item';
    }

    function showInventoryModal(filter = 'all') {
        normalizeState();
        const total = state.inventory.length;

        const body = `
            <div class="inventory-header">
                <div style="text-align:center;">
                    <div style="font-weight:900; color: gold;">Mochila: ${total} item(s)</div>
                    <div style="font-size:0.75rem; color:#bbb;">Clique em um item para equipar / gerenciar.</div>
                </div>
            </div>
        `;

        showModal("🎒 Inventário", body, [
            { text: "Tudo", action: () => showInventoryModal('all') },
            { text: "⚔️ Ataque", action: () => showInventoryModal('offense') },
            { text: "🛡️ Defesa", action: () => showInventoryModal('defense') },
            { text: "🧰 Util", action: () => showInventoryModal('utility') },
            { text: "⭐ Raro", action: () => showInventoryModal('rare') },
            { text: "🧿 Itens", action: () => showInventoryModal('consumable') },
            { text: "Equipe", action: () => showEquippedInventoryModal() },
            { text: "Fechar", action: hideModal }
        ]);

        const f = document.getElementById('modal-footer');
        f.innerHTML = '';

        const filtered = state.inventory
            .filter(it => filter === 'all' ? true : (it.type === filter))
            .sort((a, b) => {
                const ta = (a.type || '');
                const tb = (b.type || '');
                if (ta !== tb) return ta.localeCompare(tb);
                return (a.name || '').localeCompare(b.name || '');
            });

        if (filtered.length === 0) {
            const empty = document.createElement('div');
            empty.style.cssText = 'color:#aaa; text-align:center; width:100%; padding:10px;';
            empty.innerHTML = 'Inventário vazio.';
            f.appendChild(empty);
            return;
        }

        filtered.forEach(item => {
            const card = document.createElement('div');
            card.className = 'creature-card inventory-item-card tooltip';
            const lvl = item.level || 1;
            card.innerHTML = `
                <div class="creature-img-container" style="position:absolute; top:0; left:0; width:100%; height:100%;">
                    <img src="imagens/${item.image}" class="creature-img" style="object-fit:contain;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 3rem; opacity: 0.25;">${item.icon || '🎒'}</div>
                </div>
                <div class="creature-card-info" style="padding: 18px 8px 10px 8px;">
                    <div class="creature-card-name" style="font-size:0.9rem;">${item.name}</div>
                    <div class="creature-card-stats" style="font-size:0.65rem; color:#fff;">${getItemTypeLabel(item.type)} • Lvl ${lvl}</div>
                </div>
                <span class="tooltiptext"><strong>${item.name} (Lvl ${lvl})</strong><br><small style="color:#ccc;">${item.desc}</small></span>
            `;
            card.onclick = () => showInventoryItemModal(item.uid, filter);
            f.appendChild(card);
        });
    }

    function showInventoryItemModal(uid, backFilter = 'all') {
        normalizeState();
        const item = state.inventory.find(i => i.uid === uid);
        if (!item) {
            showInventoryModal(backFilter);
            return;
        }
        const lvl = item.level || 1;

        // Consumíveis (Itens): usar ao invés de equipar
        if (item.type === 'consumable') {
            showModal(item.name, `
                <div style="text-align:center;">
                    <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                    <div style="color:#ffd700; font-weight:900;">${getItemTypeLabel(item.type)} • Lvl ${lvl}</div>
                    <div style="margin-top:6px; color:#ddd; font-size:0.85rem;">${item.desc}</div>
                </div>
            `, [
                { text: 'Usar', action: () => {
                    if (isEvolutionCrystal(item)) {
                        useEvolutionCrystalFromInventory(uid, backFilter);
                    } else if (item.id === 'pocao_grupal') {
                        usePocaoGrupalFromInventory(uid, backFilter);
                    } else {
                        showModal('Item', 'Este item ainda não tem efeito implementado.', [
                            { text: 'Ok', action: () => showInventoryItemModal(uid, backFilter) }
                        ]);
                    }
                }},
                { text: 'Descartar', action: () => {
                    removeItemFromInventory(uid);
                    addLog(`Você descartou ${item.name}.`);
                    updateUI();
                    showInventoryModal(backFilter);
                }},
                { text: 'Voltar', action: () => showInventoryModal(backFilter) }
            ]);
            return;
        }

        showModal(item.name, `
            <div style="text-align:center;">
                <img src="imagens/${item.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                <div style="color:#ffd700; font-weight:900;">${getItemTypeLabel(item.type)} • Lvl ${lvl}</div>
                <div style="margin-top:6px; color:#ddd; font-size:0.85rem;">${item.desc}</div>
            </div>
        `, [
            { text: "Equipar", action: () => {
                const picked = removeItemFromInventory(uid);
                if (!picked) return showInventoryModal(backFilter);
                showCreatureSelection((target) => {
                    equipItemToCreatureFromInventory(target, picked, () => {
                        addLog(`${target.name} equipou ${picked.name}!`);
                        updateUI();
                        showInventoryModal(backFilter);
                    }, backFilter);
                }, `Quem deve equipar <strong>${item.name}</strong>?`);
            }},
            { text: "Descartar", action: () => {
                removeItemFromInventory(uid);
                addLog(`Você descartou ${item.name}.`);
                updateUI();
                showInventoryModal(backFilter);
            }},
            { text: "Voltar", action: () => showInventoryModal(backFilter) }
        ]);
    }

    function equipItemToCreatureFromInventory(target, itemInstance, onDone, backFilter = 'all') {
        if (!target.items) target.items = [];
        const maxSlots = getMaxItemSlotsForLevel(target.level);

        // Se houver slot disponível
        if (target.items.length < maxSlots) {
            applyImmediateItemEffects(target, itemInstance);
            target.items.push(itemInstance);
            saveGame();
            onDone && onDone();
            return;
        }

        // Slots cheios: substituir ou fundir
        showModal("Espaços Cheios", `Escolha um item para substituir ou fundir (se for igual):`, [
            ...target.items.map((it, idx) => ({
                text: it.id === itemInstance.id ? `Fundir (+Power)` : `Subst. ${it.name}`,
                action: () => {
                    if (it.id === itemInstance.id) {
                        fuseItems(it, itemInstance);
                        addLog(`${it.name} subiu de nível!`);
                    } else {
                        // Move o item antigo para o inventário
                        removeImmediateItemEffects(target, it);
                        const old = target.items[idx];
                        target.items[idx] = itemInstance;
                        applyImmediateItemEffects(target, itemInstance);
                        addItemToInventory(old);
                        addLog(`Item antigo foi para o inventário.`);
                    }
                    updateUI();
                    onDone && onDone();
                }
            })),
            { text: "Cancelar", action: () => {
                // Devolve o item para o inventário
                addItemToInventory(itemInstance);
                showInventoryModal(backFilter);
            }}
        ]);
    }

    function showEquippedInventoryModal() {
        normalizeState();
        showModal("Equipe - Equipamentos", "Escolha uma criatura para gerenciar os itens equipados:", [
            { text: "Voltar", action: () => showInventoryModal('all') }
        ]);
        const f = document.getElementById('modal-footer');
        f.innerHTML = '';

        if (state.team.length === 0) {
            const empty = document.createElement('div');
            empty.style.cssText = 'color:#aaa; text-align:center; width:100%; padding:10px;';
            empty.innerHTML = 'Você ainda não tem criaturas.';
            f.appendChild(empty);
            return;
        }

        state.team.forEach(c => {
            const card = document.createElement('div');
            card.className = 'creature-card tooltip';
            card.innerHTML = `
                ${getCreatureDisplay(c)}
                <div class="creature-card-info">
                    <div class="creature-card-name">${c.name}</div>
                    <div class="creature-card-stats">Lvl ${c.level} • Itens: ${(c.items||[]).length}</div>
                </div>
                <span class="tooltiptext">${getCreatureTooltip(c)}</span>
            `;
            card.onclick = () => showCreatureEquipmentModal(c);
            f.appendChild(card);
        });
    }

    function showCreatureEquipmentModal(creature) {
        normalizeState();
        const maxSlots = getMaxItemSlotsForLevel(creature.level);
        const equipped = creature.items || [];

        const body = `
            <div style="text-align:center;">
                ${getCreatureImageOnly(creature, "110px")}
                <div style="margin-top:6px; color:#bbb;">Slots: ${equipped.length}/${maxSlots}</div>
                <div style="font-size:0.75rem; color:#aaa;">Clique em um item para remover para o inventário.</div>
            </div>
        `;

        showModal(`${creature.name} - Itens`, body, [
            { text: "Voltar", action: () => showEquippedInventoryModal() }
        ]);

        const f = document.getElementById('modal-footer');
        f.innerHTML = '';

        // Renderiza os slots
        for (let i = 0; i < maxSlots; i++) {
            const it = equipped[i] || null;
            const card = document.createElement('div');
            card.className = 'creature-card inventory-item-card';
            if (!it) card.style.opacity = '0.5';

            card.innerHTML = it ? `
                <div class="creature-img-container" style="position:absolute; top:0; left:0; width:100%; height:100%;">
                    <img src="imagens/${it.image}" class="creature-img" style="object-fit:contain;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                    <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size: 3rem; opacity: 0.25;">${it.icon || '🎒'}</div>
                </div>
                <div class="creature-card-info" style="padding: 18px 8px 10px 8px;">
                    <div class="creature-card-name" style="font-size:0.9rem;">${it.name}</div>
                    <div class="creature-card-stats" style="font-size:0.65rem; color:#fff;">${getItemTypeLabel(it.type)} • Lvl ${it.level || 1}</div>
                </div>
            ` : `
                <div class="creature-img-container" style="position:absolute; top:0; left:0; width:100%; height:100%; display:flex; align-items:center; justify-content:center;">
                    <div style="font-size:2.5rem; opacity:0.25;">📦</div>
                </div>
                <div class="creature-card-info" style="padding: 18px 8px 10px 8px;">
                    <div class="creature-card-name" style="font-size:0.9rem;">Vazio</div>
                    <div class="creature-card-stats" style="font-size:0.65rem; color:#bbb;">Slot ${i + 1}</div>
                </div>
            `;

            if (it) {
                card.onclick = () => {
                    showModal("Remover Item", `
                        <div style="text-align:center;">
                            <img src="imagens/${it.image}" style="width:72px; height:72px; object-fit:contain; margin-bottom:10px;" onerror="this.style.display='none'">
                            <div style="font-weight:900; color: gold;">${it.name} (Lvl ${it.level || 1})</div>
                            <div style="margin-top:6px; color:#ddd; font-size:0.85rem;">${it.desc}</div>
                        </div>
                    `, [
                        { text: "Remover p/ Inventário", action: () => {
                            const removed = creature.items.splice(i, 1)[0];
                            removeImmediateItemEffects(creature, removed);
                            addItemToInventory(removed);
                            addLog(`${creature.name} removeu ${removed.name}.`);
                            updateUI();
                            showCreatureEquipmentModal(creature);
                        }},
                        { text: "Voltar", action: () => showCreatureEquipmentModal(creature) }
                    ]);
                };
            }

            f.appendChild(card);
        }
    }

    function showCreatureSelection(cb, t = "Escolha:", filterResting = false, excludeIds = [], onCancel = null) {
        document.getElementById('overlay').style.display = 'flex';
        document.getElementById('modal-title').innerHTML = t;
        document.getElementById('modal-body').innerText = "";
        document.getElementById('modal-actions').innerHTML = '';
        const f = document.getElementById('modal-footer'); f.innerHTML = '';
        
        // Esconde HUD de diamantes se estiver vindo da loja para seleção
        document.getElementById('modal-diamonds-hud').classList.add('hidden');

        state.team.forEach(c => {
            if (excludeIds.includes(c.id)) return;

            const isResting = filterResting && c.restCount > 0;
            const d = document.createElement('div'); 
            d.className = 'creature-card' + (c.currentHp <= 0 ? ' dead' : '') + (isResting ? ' resting' : '');
            
            let statusText = "";
            if (c.currentHp <= 0) statusText = "<br><span style='color:red;'>DERROTADO</span>";
            else if (isResting) statusText = "<br><span style='color:orange;'>CANSADO</span>";

            d.innerHTML = `
                ${getCreatureDisplay(c)}
                <div class="creature-card-info">
                    <div class="creature-card-name">${c.name}</div>
                    <div class="creature-card-stats">Lvl ${c.level}${statusText}</div>
                </div>
            `;
            
            d.ondblclick = (e) => {
                e.stopPropagation();
                showModal("Status: " + c.name, getCreatureTooltip(c), [
                    { text: "Voltar", action: () => showCreatureSelection(cb, t, filterResting, excludeIds, onCancel) }
                ]);
            };

            if (c.currentHp > 0 && !isResting) d.onclick = () => { hideModal(); cb(c); };
            f.appendChild(d);
        });

        const actions = document.getElementById('modal-actions');
        const cancelBtn = document.createElement('button');
        cancelBtn.innerText = "Cancelar";
        cancelBtn.onclick = () => { hideModal(); if (typeof onCancel === 'function') onCancel(); };
        actions.appendChild(cancelBtn);
    }

    function addLog(m) {
        const screen = document.getElementById('battle-screen');
        if (!screen) return;

        const popup = document.createElement('div');
        popup.className = 'battle-log-popup';
        popup.innerHTML = m;
        screen.appendChild(popup);

        requestAnimationFrame(() => {
            popup.classList.add('show');
        });

        setTimeout(() => {
            popup.classList.remove('show');
            popup.classList.add('hide');
        }, 2200);

        setTimeout(() => {
            if (popup.parentNode) popup.parentNode.removeChild(popup);
        }, 3200);
    }

    function toggleButtons(d) { 
        if (d) {
            document.querySelectorAll('#battle-actions button').forEach(b => b.disabled = true);
        } else {
            // Se for para habilitar, redesenha a UI para respeitar cooldowns e hasActed
            setupBattleUI();
        }
        
        // Atualiza indicador de turno visualmente
        const indicator = document.getElementById('turn-indicator');
        if (indicator) {
            if (d) {
                indicator.innerText = "▶ Turno do Inimigo";
                indicator.style.color = "#ff4d4d"; // Vermelho
                indicator.style.borderColor = "#ff4d4d";
            } else {
                indicator.innerText = "▶ Seu Turno";
                indicator.style.color = "gold"; // Dourado
                indicator.style.borderColor = "var(--accent-color)";
            }
        }
    }

    // Ao carregar a página
    window.onload = () => {
        const saved = localStorage.getItem('aventura_save');
        if (saved) {
            document.getElementById('continue-btn').disabled = false;
        }
        
        // Ativa auto-escala para garantir que o jogo caiba na tela
        window.addEventListener('resize', autoScaleGame);
        autoScaleGame();
    };

    function autoScaleGame() {
        const container = document.getElementById('game-container');
        if (!container || container.style.display === 'none') return;

        // Se estiver em modo mobile nativo (pela opção do jogo), não escala
        if (state.options && state.options.mobileMode) {
            container.style.transform = '';
            container.style.transformOrigin = '';
            container.style.margin = '';
            return;
        }

        const windowW = window.innerWidth;
        const windowH = window.innerHeight;

        // Largura base do jogo (desktop expandido)
        const baseW = 1200;
        // Altura base do jogo (mais compacta para caber em telas menores)
        const baseH = 900;

        const scaleX = windowW / baseW;
        const scaleY = windowH / baseH;
        
        const isBattle = document.body.classList.contains('battle-mode');
        
        // Aplica uma redução adicional de 10% para garantir que sobre margem
        let scale = Math.min(scaleX, scaleY) * 0.9;
        
        // Limita a escala máxima para não ficar gigante em telas 4K, 
        // mas permite que diminua o quanto for necessário.
        scale = Math.min(scale, 1.0);

        if (scale < 1 || isBattle) {
            container.style.transform = `scale(${scale})`;
            container.style.transformOrigin = 'top center';
            container.style.margin = '0 auto';
            
            // Ajusta o body para não ter scroll horizontal por causa da escala
            document.body.style.overflowX = 'hidden';
        } else {
            container.style.transform = '';
            container.style.transformOrigin = '';
            container.style.margin = '';
        }
    }
